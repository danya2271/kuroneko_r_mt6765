...
{
	pr_no_notice("[Power/CPU_Thermal]%s doesn't exist\n", __func__);
}
...
void tscpu_thermal_cal_prepare(void)
{
	...
	pr_no_notice("[calibration] temp0=0x%x, temp1=0x%x, temp2=0x%x\n",
							temp0, temp1, temp2);
	/*
	 *   chip dependent
	 *   ADC_GE_T
	...
	pr_no_notice("[lvts_cal] 0: 0x%x, 1: 0x%x, 2: 0x%x\n", lvtsdevinfo1,
...
int tscpu_thermal_fast_init(int tc_num)
{
	...
	while (temp != THERMAL_INIT_VALUE && cunt < 20) {
		cunt++;
		/* pr_no_notice("[Power/CPU_Thermal]0 temp=%d,cunt=%d\n",
		 *					temp,cunt);
		 */
		temp = readl(offset + TEMPMSR0) & 0x0fff;
	}
	...
	while (temp != THERMAL_INIT_VALUE && cunt < 20) {
		cunt++;
		/* pr_no_notice("[Power/CPU_Thermal]1 temp=%d,cunt=%d\n",
		 *					temp,cunt);
		 */
		temp = readl(offset + TEMPMSR1) & 0x0fff;
	}
	...
	while (temp != THERMAL_INIT_VALUE && cunt < 20) {
		cunt++;
		/* pr_no_notice("[Power/CPU_Thermal]2 temp=%d,cunt=%d\n",
		 *					temp,cunt);
		 */
		temp = readl(offset + TEMPMSR2) & 0x0fff;
	}
	...
	while (temp != THERMAL_INIT_VALUE && cunt < 20) {
		cunt++;
		/* pr_no_notice("[Power/CPU_Thermal]3 temp=%d,cunt=%d\n",
		 *					temp,cunt);
		 */
		temp = readl(offset + TEMPMSR3) & 0x0fff;
	}
...
void tscpu_config_all_tc_hw_protect(int temperature, int temperature2)
{
	...
	/* Get milliseconds */
	pr_no_notice("resume time spent, sec : %lu , usec : %lu\n",
...
int get_io_reg_base(void)
{
	...
	if (!node) {
		pr_no_notice("[CLK_INFRACFG_AO] find node failed\n");
		return 0;
	}

	therm_clk_infracfg_ao_base = of_iomap(node, 0);
	if (!therm_clk_infracfg_ao_base) {
		pr_no_notice("[CLK_INFRACFG_AO] base failed\n");
		return 0;
	}
#endif
	return 1;
