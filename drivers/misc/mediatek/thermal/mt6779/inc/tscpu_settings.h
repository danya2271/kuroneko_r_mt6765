...
#define THERMAL_IS_TOO_LONG()   \
	do {                                    \
		...
		if (thermal_diff_us > THERMAL_TIME_TH) {                \
			pr_no_notice(TSCPU_LOG_TAG "%s: %llu us\n", __func__, \
							thermal_diff_us); \
		} else if (thermal_diff_us < 0) {	\
			pr_no_notice(TSCPU_LOG_TAG \
...
#define tscpu_dprintk(fmt, args...)	\
	do {					\
		if (tscpu_debug_log == 1) {	\
			pr_no_notice(TSCPU_LOG_TAG fmt, ##args);	\
		}	\
	} while (0)

#define tscpu_printk(fmt, args...)	pr_no_notice(TSCPU_LOG_TAG fmt, ##args)
#define tscpu_warn(fmt, args...)	pr_no_notice(TSCPU_LOG_TAG fmt, ##args)
...
#define lvts_reg_print(fmt, args...)	pr_no_notice(LVTS_LOG_REG_TAG fmt, ##args)
#define lvts_printk(fmt, args...)	pr_no_notice(LVTS_LOG_TAG fmt, ##args)
#define lvts_warn(fmt, args...)		pr_no_notice(LVTS_LOG_TAG fmt, ##args)
#if 0
#define lvts_dbg_printk(fmt, args...)  pr_no_notice(LVTS_LOG_TAG fmt, ##args)
#else
#define lvts_dbg_printk(fmt, args...)   \
	do {                                    \
		if (lvts_debug_log == 1) {                \
			pr_no_notice(LVTS_LOG_TAG fmt, ##args); \
		}                                   \
	} while (0)
#endif
#endif
