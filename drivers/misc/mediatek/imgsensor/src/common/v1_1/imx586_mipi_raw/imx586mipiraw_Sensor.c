...
static void write_shutter(kal_uint32 shutter)
{
	...
	if (longexposure_times > 0) {
		pr_no_debug("enter long exposure mode, time is %d",
			longexposure_times);
		...
		pr_no_debug("exit long exposure mode");
	}
	#endif
	/* Update Shutter */
	write_cmos_sensor_8(0x0104, 0x01);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < BASEGAIN || gain > max_gain) {
		pr_no_debug("Error max gain setting: %d\n", max_gain);
	...
	spin_unlock(&imgsensor_drv_lock);
	pr_no_debug("gain = %d, reg_gain = 0x%x, max_gain:0x%x\n ",
...
static kal_uint32 imx586_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	...
	pr_no_debug("[%s] ABS_GAIN_GR:%d, grgain_32:%d\n",
		__func__,
		pSetSensorAWB->ABS_GAIN_GR, grgain_32);
	pr_no_debug("[%s] ABS_GAIN_R:%d, rgain_32:%d\n",
		__func__,
		pSetSensorAWB->ABS_GAIN_R, rgain_32);
	pr_no_debug("[%s] ABS_GAIN_B:%d, bgain_32:%d\n",
		__func__,
		pSetSensorAWB->ABS_GAIN_B, bgain_32);
	pr_no_debug("[%s] ABS_GAIN_GB:%d, gbgain_32:%d\n",
		__func__,
...
{
	pr_no_debug("%s E! 4000*2256@30fps\n", __func__);
...
{
	pr_no_debug("%s full size 30 fps E!\n", __func__);
	/*************MIPI output setting************/
...
{
	pr_no_debug("%s 720p@240 fps E! currefps\n", __func__);
...
{
	pr_no_debug("%s fhd 240fps E! currefps\n", __func__);
	/*************MIPI output setting************/
...
{
	pr_no_debug("%s 3840*2160@60fps E! currefps\n", __func__);
	/*************MIPI output setting************/
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	if (!qsc_flag) {
		pr_no_debug("write_sensor_QSC Start\n");
		write_sensor_QSC();
		pr_no_debug("write_sensor_QSC End\n");
		qsc_flag = 1;
	}
	custom3_setting();
	set_mirror_flip(imgsensor.mirror);
