...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		for (i = 0; i < timeout; i++) {
			...
			if (framecnt == 0xFF) {
				pr_no_debug(" Stream Off OK at i=%d.\n", i);
				return ERROR_NONE;
			}
		}
		pr_no_debug("Stream Off Fail! framecnt= %d.\n", framecnt);
	}
	return ERROR_NONE;
}
...
static void preview_setting(void)
{
	pr_no_debug("start\n");
	if (bIsLongExposure) {
		pr_no_debug("enter normal shutter.\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = read_cmos_sensor(0x0000);
			pr_no_debug("read out sensor id 0x%x\n", *sensor_id);
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
		...
		case MSDK_SCENARIO_ID_CUSTOM3:
			...
			break;
		default:
			pr_no_debug("Warning: wrong ScenarioId setting");
			preview(image_window, sensor_config_data);
				...
				UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		pr_no_debug("SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY scenarioId:%lld\n",
				*feature_data);
 ...
 */
	...
	pr_no_debug("SENSOR_FEATURE_GET_AE_FRAME_MODE_FOR_LE\n");
		memcpy(feature_return_para_32,
		&imgsensor.ae_frm_mode, sizeof(struct IMGSENSOR_AE_FRM_MODE));
		break;
	case SENSOR_FEATURE_GET_AE_EFFECTIVE_FRAME_FOR_LE:
		pr_no_debug("SENSOR_FEATURE_GET_AE_EFFECTIVE_FRAME_FOR_LE\n");
