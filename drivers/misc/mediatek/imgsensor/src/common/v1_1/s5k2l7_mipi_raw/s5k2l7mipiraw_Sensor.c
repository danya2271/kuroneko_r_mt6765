...
static void write_cmos_sensor_twobyte(kal_uint32 addr, kal_uint32 para)
{
	...
	/* pr_no_debug("write_cmos_sensor_twobyte is %x,%x,%x,%x\n",
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	pr_no_debug("framerate = %d, min framelength should enable = %d\n",
...
{
	/* pr_no_debug("enter xxxx  set_shutter, shutter =%d\n", shutter); */
...
static void hdr_write_shutter(kal_uint16 le, kal_uint16 se)
{
	...
	pr_no_debug("HDR set shutter LE=%d, SE=%d, iRation=0x%x\n",
		le, se, iRation);
...
static kal_uint16 set_gain(kal_uint16 leGain, kal_uint16 seGain)
{
	...
	if (leGain < S5K2L7_GAIN_MIN) {
		pr_no_debug("Error gain setting:LE=%x\n", leGain);
	...
	if (seGain < S5K2L7_GAIN_MIN) {
		pr_no_debug("Error gain setting:SE=%x\n", seGain);
...
static BOOL is_module_v2(void)
{
	...
	if (module_id == 0) {
		...
		pr_no_debug("i2c write id: 0x%x, module id: 0x%x\n",
			 imgsensor.i2c_write_id,
			 module_id);
	}

	if (module_id == MODULE_V2_ID) {
		pr_no_debug("It is module v2");

		return TRUE;
	}
	pr_no_debug("It is module v1");
	return FALSE;
}
...
{
	pr_no_debug("E S5k2L7 sensor init (%d)\n", pdaf_sensor_mode);
...
{
	pr_no_debug("E S5k2L7 preview setting (%d)\n", pdaf_sensor_mode);
...
{
	pr_no_debug("E S5k2L7 Capture WDR, fps = %d (%d)\n",
...
{
	pr_no_debug("E S5k2L7 capture setting (%d)\n", pdaf_sensor_mode);
...
{
	pr_no_debug("E S5k2L7 HS video setting (%d)\n", pdaf_sensor_mode);
	KD_SENSOR_PROFILE_INIT();
...
{
	pr_no_debug("E S5k2L7 slim video setting (%d)\n", pdaf_sensor_mode);
	KD_SENSOR_PROFILE_INIT();
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_no_debug("%s pdaf sensor mode %d\n", __func__, pdaf_sensor_mode);
...
static kal_uint32 open(void)
{
    ...
    /* pr_no_debug("JEFF get_imgsensor_id-read sensor ID (0x%x)\n", sensor_id ); */
	...
	/* pr_no_debug("get_imgsensor_id-read chip_id (0x%x)\n", chip_id ); */

	if (sensor_id == imgsensor_info.sensor_id || sensor_id == 0x20C1) {
		pr_no_debug("i2c write id: 0x%x, sensor id: 0x%x, chip_id (0x%x)\n",
	...
	chip_id = read_cmos_sensor_twobyte(0x6F12);
	/* chip_id = read_cmos_sensor_twobyte(0x001A); */
	pr_no_debug("JEFF get_imgsensor_id-read chip_id (0x%x)\n", chip_id);
	...
	pr_no_debug("pangfei capture OIS setting\n");
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_SLIM_VIDEO:
		...
		break;
	default:
		pr_no_debug("Error ScenarioId setting");
		preview(image_window, sensor_config_data);
...
{
	pr_no_debug("enable = %d, framerate = %d\n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
		...
		enum MSDK_SCENARIO_ID_ENUM scenario_id,	MUINT32 framerate)
{
	...
	pr_no_debug("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	default:		/* coding with  preview scenario by default */
		...
		pr_no_debug("error scenario_id = %d, we use preview scenario\n",
			scenario_id);
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		pr_no_debug("streaming_off exit\n");
	}
	return ERROR_NONE;
}
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		...
		/* add for s5k2l7 pdaf */
		pr_no_debug("s5k2l7_read_otp_pdaf_data %x\n", sensor_id);
...
/* pr_no_debug("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%lld\n", *feature_data); */
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		pr_no_debug("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%lld\n",
			*feature_data);
	...
	case SENSOR_FEATURE_SET_HDR_SHUTTER:
		pr_no_debug("SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",
	...
	case SENSOR_FEATURE_GET_PDAF_TYPE:
		*feature_para = pdaf_sensor_mode;
		if (pdaf_sensor_mode == 1) {
			...
			if (ret < 0) {
				pr_no_info("Error! sprintf allocate 0, ret = %d",
				ret);
				return ret;
			}
		} else if (pdaf_sensor_mode == 2) {
		...
		pr_no_debug("get PDAF type = %d\n", pdaf_sensor_mode);
		break;

	case SENSOR_FEATURE_SET_PDAF_TYPE:
		if (strstr(&(*feature_para), "mode1")) {
			pr_no_debug("configure PDAF as mode 1\n");
			proc_pdaf_sensor_mode = 1;
		} else if (strstr(&(*feature_para), "mode3")) {
			pr_no_debug("configure PDAF as mode 3\n");
			proc_pdaf_sensor_mode = 3;
		} else if (strstr(&(*feature_para), "mode2")) {
			pr_no_debug("configure PDAF as mode 2\n");
			proc_pdaf_sensor_mode = 2;
		} else {
			pr_no_debug("configure PDAF as unknown mode\n");
			proc_pdaf_sensor_mode = 1;
		}
		break;
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		pr_no_debug("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		pr_no_debug("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
