...
static void write_shutter(kal_uint16 shutter)
{
	...
	if (imgsensor.autoflicker_en) {
		...
		/* Extend frame length */
		write_cmos_sensor(0x0340, imgsensor.frame_length);
		pr_no_debug("(else)imgsensor.frame_length = %d\n",
	...
	pr_no_debug("shutter =%d, framelength =%d\n",
				     ...
				     kal_uint16 target_frame_length)
{
	...
	//pr_no_debug("shutter: %d, target_frame_length: %d", shutter, target_frame_length);
	spin_lock(&imgsensor_drv_lock);
	...
	imgsensor.frame_length = imgsensor_info.custom1.framelength +
				 imgsensor.dummy_line;
	imgsensor.min_frame_length = imgsensor.frame_length;
	pr_no_debug("shutter: %d, frame_length: %d, custom1.framelength: %d dummy_line: %d\n",
		 shutter, imgsensor.frame_length,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < BASEGAIN || gain > 32 * BASEGAIN) {
		pr_no_debug("Error gain setting");
	...
	spin_unlock(&imgsensor_drv_lock);
	pr_no_debug("gain = %d , reg_gain = 0x%x\n", gain, reg_gain);
...
{
	pr_no_debug("capture_setting() E! currefps:%d\n", currefps);
...
{
	pr_no_debug("normal_video_setting() E! currefps:%d\n", currefps);
...
{
	pr_no_debug("hs_video_setting() E\n");
...
{
	pr_no_debug("slim_video_setting() E\n");
	/* 1080p 60fps */
...
{
#if STEREO_CUSTOM1_30FPS
	pr_no_debug("custom1_setting() 6 M*30 fps E!\n");
#else
	pr_no_debug("custom1_setting() 6 M*24 fps E!\n");
#endif
	pr_no_debug("custom1_setting() use custom1\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) |
			...
			pr_no_debug("Read sensor id fail, id: 0x%x\n",
				 imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
		i++;
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate) {
			pr_no_debug("Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_no_debug("get_info -> scenario_id = %d\n", scenario_id);
	...
	enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 *framerate)
{
	...
	case MSDK_SCENARIO_ID_CUSTOM1:
		*framerate = imgsensor_info.custom1.max_framerate;
		pr_no_debug("custom1 *framerate = %d\n", *framerate);
		break;
...
static kal_uint32 get_sensor_temperature(void)
{
	...
	/*pr_no_info("temp_c(%d), read_reg(%d), enable %d\n",
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		pr_no_debug("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n",
			 (UINT32) *feature_data);
	...
	case SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME:/*lzl*/
		pr_no_debug("SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME\n");
