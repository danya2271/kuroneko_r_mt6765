...
{
	pr_no_debug("le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
...
static void sensor_WDR_zhdr(void)
{
	if (imgsensor.hdr_mode == 9) {
		...
		pr_no_debug("hdr_mode is not zhdr!\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			pr_no_debug("Read sensor id fail, i2c_write_id: 0x%x\n",
...
/*const kal_uint8 i2c_addr[] = {IMGSENSOR_WRITE_ID_1, IMGSENSOR_WRITE_ID_2};*/
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_no_debug("Read sensor id fail, i2c write id: 0x%x\n",
				imgsensor.i2c_write_id);
			retry--;
		} while (retry > 0);
...
/* pr_no_debug("scenario_id = %d, framerate = %d\n", scenario_id, framerate); */
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			pr_no_debug("so use cap's setting: %d fps!\n",
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	/* pr_no_debug("feature_id = %d", feature_id); */
	switch (feature_id) {
...
pr_no_debug("feature_Control imgsensor.pclk = %d,imgsensor.current_fps = %d\n",
