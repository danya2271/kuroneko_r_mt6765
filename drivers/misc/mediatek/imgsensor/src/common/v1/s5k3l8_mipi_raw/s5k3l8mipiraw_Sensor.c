...
static void set_shutter(kal_uint16 shutter)
{
	...
	pr_no_info("Exit! shutter =%d, framelength =%d\n",
			...
			kal_uint16 frame_length)
{
	...
	write_cmos_sensor(0x0202, shutter & 0xffff);
	pr_no_info("Add for N3D! shutter =%d, framelength =%d\n",
		shutter, imgsensor.frame_length);
	...
	kal_uint16 *para, kal_uint32 len)
{
	...
	puSendCmd = kmalloc(
		...
		GFP_KERNEL);
	if (puSendCmd == NULL) {
		pr_no_info("allocate mem failed\n");
		return -ENOMEM;
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id == imgsensor_info.sensor_id) {
		pr_no_info("Read ok sensor id: 0x%x, img sensor_id:0x%x.\n",
			*sensor_id, imgsensor_info.sensor_id);
		return ERROR_NONE;
	}
	pr_no_info("Readsensoridfail: 0x%x, img_info.sensor_id:0x%x.\n",
		*sensor_id, imgsensor_info.sensor_id);
		retry--;
	} while (retry > 0);
	i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_no_info("Read sensor id fail, id: 0x%x, sensor id: 0x%x\n",
			imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	for (i = 0; i < 10; i++) {
		pr_no_info("delay time = %d, the frame no = %d\n",
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	imgsensor_info.cap1.max_framerate) {
		pr_no_info("cap115fps: use cap1's setting: %d fps!\n",
			imgsensor.current_fps/10);
		...
		pr_no_info("current_fps %d not support,use cap1\n",
				imgsensor.current_fps/10);
		imgsensor.pclk = imgsensor_info.cap1.pclk;
		imgsensor.line_length = imgsensor_info.cap1.linelength;
		imgsensor.frame_length = imgsensor_info.cap1.framelength;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
	    pr_no_info("GET_CROP_INFO scenarioId:%d\n",
			(UINT32)*feature_data);
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		pr_no_info("PDAF_CAPACITY scenarioId:%llu\n", *feature_data);
		switch (*feature_data) {
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		pr_no_info("GET_PDAF_INFO scenarioId:%llu\n", *feature_data);
