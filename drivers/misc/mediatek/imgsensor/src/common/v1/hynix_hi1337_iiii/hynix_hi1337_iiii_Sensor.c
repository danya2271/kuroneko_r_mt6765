...
static int hi1337_vendor_id_read(int addr)
{
	...
	pr_no_info("hynix_hi1337_IIII  read vendor id , form 0x01 is: 0x%x\n", flag);
	return flag;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if( flag != HI1337_VENDOR_ID) {
        pr_no_info("hynix_hi1337_iiii match vendor id fail, reead vendor id is: 0x%x,expect vendor id is 0x07 \n", flag);
        return ERROR_SENSOR_CONNECT_FAIL;
    }else{
        hi1337_fusion_id_read();
    }
    pr_no_info("hynix_hi1337_IIII match vendor id successed, reead vendor id is: 0x%x,expect vendor id is 0x07 \n", flag);
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		pr_no_info("Read id fail,sensor id: 0x%x\n", *sensor_id);
...
{
    pr_no_info("[hi1337] preview mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_no_info("[hi1337] capture mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_no_info("[hi1337] normal video mode start\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_no_info("[hi1337] hs video mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_no_info("[hi1337] slim video mode start\n");
    spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    pr_no_info("[hi1337] custom1 mode start\n");
    spin_lock(&imgsensor_drv_lock);
