...
static void write_shutter(kal_uint16 shutter)
{
	...
	pr_no_info("shutter =%d, framelength =%d\n",
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			pr_no_info("Read id fail,i2c_write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_no_info("R sensorid fail,write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap.max_framerate) {
		...
		pr_no_info("current_fps %d not support, so cap1's: %d fps!\n",
		...
		pr_no_info("current_fps %d not support, so use cap2's: %d fps!\n",
			imgsensor.current_fps,
			...
			MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			if (imgsensor.current_fps !=
				...
				pr_no_info("%d fps not support,cap's: %d fps\n",
					framerate,
	...
	default:
		...
		pr_no_info("error scenario_id = %d, preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	/*pr_no_info("feature_id = %d", feature_id);*/
	switch (feature_id) {
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
		pr_no_info("imgsensor.pclk = %d,imgsensor.current_fps = %d\n",
	...
	case SENSOR_FEATURE_SET_HDR:
		pr_no_info("Warning! Not Support IHDR Feature");
		spin_lock(&imgsensor_drv_lock);
		imgsensor.ihdr_en = KAL_FALSE;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_GET_CROP_INFO:
		pr_no_info("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%llu\n",
			*feature_data);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		pr_no_info("SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
