				...
				kal_uint32 addr, u8 *reg_buf, kal_uint32 size)
{
	...
	for (i = 0; i < size; i += MAX_READ_WRITE_SIZE) {
		...
		if (ret < 0)
			pr_no_info("write burst reg into sensor failed!\n");
...
static int imx386_read_otp(u16 addr, u8 *buf)
{
	...
	if (ret < 0)
		pr_no_info("read data from imx386 primax otp e2prom failed!\n");
...
static void imx386_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	...
	for (i = 0; i < regNum; i++) {
		...
		/* pr_no_info("%x %x", regDa[idx], regDa[idx + 1]); */
	}
}
...
static void load_imx386_spc_data(void)
{
	...
	0x7D4C + i, imx386_primax_otp_buf[spc_start_addr - OTP_START_ADDR + i]);
		...
		pr_no_info("SPC_Data[0x%x] = 0x%x\n",
			0x7D4C + i, read_cmos_sensor(0x7D4C + i));

		pr_no_info("OTP[0x%x] = 0x%x\n", spc_start_addr + i,
	...
	for (i = 0; i < OTP_DATA_SIZE; i++) {
		pr_no_info("========imx386_otp idx:0x%4x val:0x%x======\n", i,
...
{
	pr_no_info("dummyline = %d, dummypixels = %d\n",
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	pr_no_info("exit IMX386MIPI_sensorGainMapping function\n");
				...
				kal_uint16 le, kal_uint16 se, kal_uint16 gain)
{
	...
	pr_no_info("le:0x%x, se:0x%x, gain:0x%x, auto_extend=%d\n",
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	case IMAGE_HV_MIRROR:
		...
		break;
	default:
		pr_no_info("Error image_mirror setting\n");
	}
...
static void capture_setting(kal_uint16 currefps)
{
	...
	pr_no_info("start streamming. 0x0100 =%d\n", read_cmos_sensor(0x0100));
}				/* capture setting */
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap.max_framerate)
		pr_no_info("capture30fps: use cap30FPS's setting: %d fps!\n",
...
{
	pr_no_info("streaming_enable(0=Sw tandby,1=streaming): %d\n", enable);
	if (enable)
		write_cmos_sensor(0x0100, 0X01);
	else
		write_cmos_sensor(0x0100, 0x00);
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		pr_no_info("current fps :%d\n", *feature_data_32);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.current_fps = (UINT16)*feature_data_32;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_SET_HDR:
		pr_no_info("ihdr enable :%d\n", *feature_data_32);
		spin_lock(&imgsensor_drv_lock);
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		pr_no_info("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%d\n",
			(UINT32) *feature_data);
	...
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		pr_no_info("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
