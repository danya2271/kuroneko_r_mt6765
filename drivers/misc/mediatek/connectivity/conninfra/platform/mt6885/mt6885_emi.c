...
int consys_emi_mpu_set_region_protection(void)
{
	...
	pr_no_info("setting MPU for EMI share memory\n");
	return 0;
}
...
#else
	pr_no_info("[%s] ECCCI Driver is not supported.\n", __func__);
#endif
	if (ret && mdPhy) {
		pr_no_info("MCIF base=0x%llx size=0x%x", mdPhy, ret);
		if (base)
			*base = mdPhy;
		if (size)
			*size = ret;
	} else {
		pr_no_info("MCIF is not supported");
		if (base)
			*base = 0;
		if (size)
			*size = 0;
