...
static void consys_bus_hang_dump_c(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(debug_setting); i++) {
		...
		pr_no_info("[CONN_BUS_C] addr=0x%x value=0x%08x", debug_setting[i], value);
	}
}
...
static void consys_bus_hang_dump_a(void)
{
	...
	pr_no_info("[CONN_BUS_A] %s %s [%x][%x][%x][%x][%x][%x] [%x][%x] [%x][%x][%x]", tmp_buf,
...
static void consys_bus_hang_dump_b(void)
{
	...
	pr_no_info("[CONN_BUS_B] infra_off %s [%x][%x] [%x][%x][%x] [%x] [%x]"
				, tmp_buf, r1, r2, r3, r4, r5, r6, r7);
	pr_no_info("[CONN_BUS_B] 0x1806_0294:[0x%08x] 0x1806_0220:[0x%08x]\n",
...
static void consys_bus_hang_dump_d(void)
{
	...
	if (((r1 >> 8) & 0x01) == 0) {
		pr_no_info("[CONN_BUS_D] conninfra off is not on");
		return;
	}
	 ...
	 */
	pr_no_info("[CONN_BUS_D] [%x][%x] [%x][%x][%x] [%x][%x] [%x][%x] [%x][%x] [%x]",
...
int consys_dump_cpupcr(enum conn_dump_cpupcr_type type, int times, unsigned long interval_us)
{
	...
	for (i = 0; i < times; i++) {
		pr_no_info("%s: wm pc=0x%x, wm lp=0x%x, bt pc=0x%x",
			__func__,
...
static int consys_is_bus_hang(void)
{
	...
	if ((r & 0x1) != 0x1) {
		pr_no_info("conninfra off bus might hang cirq=[0x%08x]", r);
		consys_bus_hang_dump_c();
	...
	if (ret) {
		pr_no_info("[%s] ret=[%x]", __func__, ret);
		consys_bus_hang_dump_c();
		return ret;
	}
#endif
...
int consys_check_reg_readable(void)
{
	...
	while (retry > 0 && spent < max_wait) {
		...
		if ((r & TOP_BUS_MUC_STAT_HCLK_FR_CK_DETECT_BIT) == 0 ||
			...
			retry--;
			if (retry == 0)
				pr_no_info("[%s] retry=0 r=[%x]", __func__, r);
			else
				delay_time = 10;
	...
	if (retry == 0 || spent >= max_wait) {
		pr_no_info("[%s] readable fail = bus clock retry=[%d] spent=[%d]", __func__,
					retry, spent);
		return 0;
	}
...
unsigned long consys_reg_validate_idx_n_offset(unsigned int idx, unsigned long offset)
{
	...
	if (idx < 0 || idx >= CONSYS_BASE_ADDR_MAX) {
		pr_no_info("ConsysReg failed: No support the base %d\n", idx);
		return 0;
	}
	...
	if (res == 0) {
		pr_no_info("ConsysReg failed: No support the base idx is 0 index=[%d]\n", idx);
		return 0;
	}

	if (offset >= conn_reg.reg_base_addr[idx].size) {
		pr_no_info("ConnReg failed: index(%d), offset(%d) over max size(%llu) %s\n",
...
int consys_reg_init(struct platform_device *pdev)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			...
			if (ret) {
				pr_no_info("Get Reg Index(%d-%s) failed",
			...
			pr_no_info("Get Index(%d-%s) phy(0x%zx) baseAddr=(0x%zx) size=(0x%zx)",
				i, consys_base_addr_index_to_str[i], base_addr->phy_addr,
				base_addr->vir_addr, base_addr->size);
		}

	} else {
		pr_no_info("[%s] can't find CONSYS compatible node\n", __func__);
		return ret;
	}
	return 0;
