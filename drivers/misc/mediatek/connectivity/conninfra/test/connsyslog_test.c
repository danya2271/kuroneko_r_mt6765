...
{
	ssize_t read = 0;
	pr_no_info("connlog_test_event_handler\n");
...
int connlog_test_init(void)
{
	...
	if (ret) {
		pr_no_info("connsys_dedicated_log_path_apsoc_init should fail\n");
		return 2;
	} else {
		pr_no_info("connsys_dedicated_log_path_apsoc_init return fail as expection\n");
	}

	ret = connsys_log_init(CONN_DEBUG_TYPE_WIFI);
	if (ret) {
		pr_no_info("Init connsys log failed\n");
		return 3;
	}
