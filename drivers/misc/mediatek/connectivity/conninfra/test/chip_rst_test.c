...
int chip_rst_test(void)
{
	...
	pr_no_info("[%s] cb init [%p][%p]", __func__,
	...
	if (ret)
		...
		pr_no_info("Trigger chip reset success. Test pass.");
	osal_sleep_ms(10);

	pr_no_info("Try to trigger whole chip reset when reset is ongoing. It should be fail.");
	ret = conninfra_trigger_whole_chip_rst(CONNDRV_TYPE_BT, "test reset");
	pr_no_info("Test %s. ret = %d.", ret == 1? "pass": "fail", ret);

	pr_no_info("Try to funcion on when reset is ongoing. It should be fail.");
	ret = conninfra_pwr_on(CONNDRV_TYPE_WIFI);
	pr_no_info("Test %s. ret = %d.", ret == CONNINFRA_ERR_RST_ONGOING ? "pass": "fail", ret);

	osal_sleep_ms(3000);

	conninfra_sub_drv_ops_unregister(CONNDRV_TYPE_BT);
	conninfra_sub_drv_ops_unregister(CONNDRV_TYPE_WIFI);
	pr_no_info("chip_rst_test finish");
	return 0;
}
