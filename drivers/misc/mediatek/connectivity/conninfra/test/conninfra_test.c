...
int core_tc_pwr_on(void)
{
	...
	pr_no_info("Power on test start");
	iret = conninfra_core_power_on(CONNDRV_TYPE_BT);
	pr_no_info("BT power on %s (result = %d)", iret? "fail" : "pass", iret);
	osal_sleep_ms(100);
	iret = conninfra_core_power_on(CONNDRV_TYPE_FM);
	pr_no_info("FM power on %s (result = %d)", iret? "fail" : "pass", iret);
	osal_sleep_ms(100);
	iret = conninfra_core_power_on(CONNDRV_TYPE_GPS);
	pr_no_info("GPS power on %s (result = %d)", iret? "fail" : "pass", iret);
	osal_sleep_ms(100);
	iret = conninfra_core_power_on(CONNDRV_TYPE_WIFI);
	pr_no_info("Wi-Fi power on %s (result = %d)", iret? "fail" : "pass", iret);
	osal_sleep_ms(200);
...
int core_tc_pwr_off(void)
{
	...
	pr_no_info("Wi-Fi power off %s (result = %d)", iret? "fail" : "pass", iret);
	osal_sleep_ms(100);
	iret = conninfra_core_power_off(CONNDRV_TYPE_GPS);
	pr_no_info("GPS power off %s (result = %d)", iret? "fail" : "pass", iret);
	osal_sleep_ms(100);
	iret = conninfra_core_power_off(CONNDRV_TYPE_BT);
	pr_no_info("BT power off %s (result = %d)", iret? "fail" : "pass", iret);
	osal_sleep_ms(100);
	iret = conninfra_core_power_off(CONNDRV_TYPE_FM);
	pr_no_info("FM power off %s (result = %d)", iret? "fail" : "pass", iret);
...
int core_tc(int par1, int par2, int par3)
{
	...
	if (par2 == 0) {
		...
		if (par3 == 15) {
			...
			pr_no_info("Power on %s test start\n", driver_name[par3]);
			iret = conninfra_core_power_on(par3);
			pr_no_info("Power on %s test, return = %d\n", driver_name[par3], iret);
		} else {
			pr_no_info("No support parameter\n");
		}
	} else if (par2 == 2) {
		if (par3 == 15) {
			iret = core_tc_pwr_off();
		} else if (par3 >= CONNDRV_TYPE_BT && par3 <= CONNDRV_TYPE_WIFI) {
			pr_no_info("Power off %s test start\n", driver_name[par3]);
			iret = conninfra_core_power_off(par3);
			pr_no_info("Power off %s test, return = %d\n", driver_name[par3], iret);
		} else {
			pr_no_info("No support parameter\n");
		}
	} else if (par2 == 3) {
	...
	//pr_no_info("core_tc %s (result = %d)", iret? "fail" : "pass", iret);
	return 0;
}
...
static int emi_tc(int par1, int par2, int par3)
{
	...
	pr_no_info("[%s] start", __func__);
	conninfra_get_phy_addr(&addr, &size);
	if (addr == 0 || size == 0) {
		pr_no_info("[%s] fail! addr=[0x%x] size=[%u]", __func__, addr, size);
		ret = -1;
	} else
		pr_no_info("[%s] pass. addr=[0x%x] size=[%u]", __func__, addr, size);

	pr_no_info("[%s] end", __func__);
...
static int thermal_tc(int par1, int par2, int par3)
{
	...
	ret = core_tc_pwr_on();
	if (ret) {
		pr_no_info("pwr on fail");
		return -1;
	}
	ret = conninfra_core_thermal_query(&temp);
	pr_no_info("[%s] thermal res=[%d][%d]", __func__, ret, temp);
...
static int log_tc(int par1, int par2, int par3)
{
	...
	if (par2 == 0) {
		if (log_status != 0) {
			pr_no_info("log has been init.\n");
			return 0;
		}
		/* init */
		ret = connlog_test_init();
		if (ret)
			pr_no_info("FW log init fail! ret=%d\n", ret);
		else {
			log_status = 1;
			pr_no_info("FW log init finish. Check result on EMI.\n");
		}
	} else if (par2 == 1) {
		/* add fake log */
		/* read log */
		...
		/* deinit */
		if (log_status == 0) {
			pr_no_info("log didn't init\n");
			return 0;
		}
		ret = connlog_test_deinit();
		if (ret)
			pr_no_info("FW log deinit fail! ret=%d\n", ret);
		else
			log_status = 0;
	}
	return ret;
