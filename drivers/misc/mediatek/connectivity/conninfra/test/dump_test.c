...
static int coredump_test_start(void)
{
	...
	if (!g_dumpCtx) {
		pr_no_info("coredump contxt is not init\n");
		return 1;
	}
#if 0
	pr_no_info("[%s] tc[0] test case\n", __func__);
	/* Trigger interrupt */
	...
	pr_no_info("[%s] tc[1] test case\n", __func__);
	/* Trigger interrupt */
	connsys_coredump_setup_tc(g_dumpCtx, 1);
	/* Start to dump */
	connsys_coredump_start(g_dumpCtx, 0);

	pr_no_info("[%s] tc[2] test case\n", __func__);
	/* Trigger interrupt */
...
static int coredump_test_init(void)
{
	...
	if (g_dumpCtx) {
		pr_no_info("log handler has been init. Need to deinit.\n");
		return 1;
	}

	conninfra_get_phy_addr(&emi_addr, &emi_size);
	if (!emi_addr || !emi_size) {
		pr_no_info("EMI init fail.\n");
		return 2;
	}

	pr_no_info("emi_addr=0x%08x, size=%d", emi_addr, emi_size);

	g_dumpCtx = connsys_coredump_init(
		CONN_DEBUG_TYPE_WIFI, &g_cb);

	if (!g_dumpCtx) {
		pr_no_info("Coredump init fail.\n");
		return 3;
	}
...
int opfunc_test_1(struct msg_op_data *op)
{
	...
	pr_no_info("[%s] param=[%d]", __func__, tc);
	switch (tc) {
		case 0:
...
{
	pr_no_info("Disable coredump test in SQC\n");
	init = 1;
	return 0;
#if 0
	int ret = 0;
