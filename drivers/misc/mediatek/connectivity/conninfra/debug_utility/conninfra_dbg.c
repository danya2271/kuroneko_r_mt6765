...
int conninfra_dbg_reg_read(int par1, int par2, int par3)
{
	...
	if (ret < 0) {
		pr_no_info("read chip register (0x%08x) with mask (0x%08x) error(%d)\n",
			par2, par3, ret);
	} else
		pr_no_info("%s", buf);
...
int conninfra_dbg_reg_write(int par1, int par2, int par3)
{
	...
	pr_no_info("write chip register (0x%08x) with value (0x%08x) %s\n",
...
int conninfra_dbg_force_conninfra_wakeup(int par1, int par2, int par3)
{
	...
	if (ret)
		pr_no_info("conninfra wakup fail\n");
	else
		pr_no_info("conninfra wakup success\n");
...
int conninfra_dbg_force_conninfra_sleep(int par1, int par2, int par3)
{
	...
	if (ret)
		pr_no_info("conninfra sleep fail\n");
	else
		pr_no_info("conninfra sleep success\n");
...
int conninfra_dbg_efuse_read(int par1, int par2, int par3)
{
	...
	pr_no_info("read combo chip efuse (0x%08x) with mask (0x%08x) %s, value = 0x%08x\n",
		      par2, par3, iRet != 0 ? "failed" : "succeed", iRet != 0 ? -1 : value);
#endif
	return 0;
}
...
int conninfra_dbg_efuse_write(int par1, int par2, int par3)
{
	...
	pr_no_info("write combo chip efuse (0x%08x) with value (0x%08x) %s\n",
...
static int conninfra_dbg_ap_reg_read(int par1, int par2, int par3)
{
	...
	pr_no_info("AP register read, reg address:0x%x\n", par2);
	ap_reg_base = ioremap_nocache(par2, 0x4);
	if (ap_reg_base) {
		value = readl(ap_reg_base);
		pr_no_info("AP register read, reg address:0x%x, value:0x%x\n", par2, value);
		iounmap(ap_reg_base);
	} else
		pr_no_info("AP register ioremap fail!\n");
...
static int conninfra_dbg_ap_reg_write(int par1, int par2, int par3)
{
	...
	pr_no_info("AP register write, reg address:0x%x, value:0x%x\n", par2, par3);

	ap_reg_base = ioremap_nocache(par2, 0x4);
	if (ap_reg_base) {
		writel(par3, ap_reg_base);
		value = readl(ap_reg_base);
		pr_no_info("AP register write done, value after write:0x%x\n", value);
		iounmap(ap_reg_base);
	} else
		pr_no_info("AP register ioremap fail!\n");
...
static int conninfra_dbg_fw_log_ctrl(int par1, int onoff, int level)
{
	...
	pr_no_info("Configuring firmware log: type:%d, on/off:%d, level:%d\n",
...
int conninfra_dbg_thermal_query(int par1, int count, int interval)
{
	...
	pr_no_info("[Conninfra_Thermal_Query] ret=[%d] temp=[%d]", ret, temp);
...
int conninfra_dbg_thermal_ctrl(int par1, int par2, int par3)
{
	...
	if (par2 == 0) {
		if (par3 >= 99) {
			pr_no_info("Can`t set temp threshold greater or queal 99\n");
			return -1;
		}
...
static int conninfra_dbg_connsys_emi_dump(int par1, int par2, int par3)
{
	...
	if (par2 & 0xf) {
		pr_no_info("EMI dump fail: wrong offset(0x%x), should be 16-byte alignment\n", par2);
		return -1;
	}
	...
	if (buf == NULL) {
		pr_no_info("[%s] allocate buffer fail\n", __func__);
		return -1;
	}

	pr_no_info("EMI dump, offset=0x%x(physical addr=0x%x), size=0x%x\n", par2, start, size);
...
static int conninfra_dbg_connsys_coredump_ctrl(int par1, int par2, int par3)
{
	...
	pr_no_info("Setup coredump mode from %d to %d\n", orig_mode, par2);
...
static int conninfra_dbg_connsys_coredump_mode_query(int par1, int par2, int par3)
{
	...
	pr_no_info("Connsys coredump mode is [%d]\n", orig_mode);
	return orig_mode;
}
...
ssize_t conninfra_dbg_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (ret) {
		pr_no_info("copy to dump info buffer failed, ret:%d\n", ret);
		ret = -EFAULT;
		goto exit;
	}
	...
	*f_pos += dump_len;
	...
	pr_no_info("conninfra_dbg:after read,wmt for dump info buffer len(%d)\n", g_dump_buf_len);
...
ssize_t conninfra_dbg_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	pr_no_info("write parameter len = %d\n\r", (int) len);
	if (len >= osal_sizeof(buf)) {
		pr_no_info("input handling fail!\n");
	...
	buf[len] = '\0';
	pr_no_info("write parameter data = %s\n\r", buf);
	...
	pr_no_info("x(0x%08x), y(0x%08x), z(0x%08x)\n\r", x, y, z);
	...
	if (0 == dbg_enabled && (x != 0x13) && (x != 0x14)) {
		pr_no_info("please enable conninfra debug first\n\r");
		return len;
	}
	...
	if (osal_array_size(conninfra_dev_dbg_func) > x && NULL != conninfra_dev_dbg_func[x])
		...
		pr_no_info("no handler defined for command id(0x%08x)\n\r", x);
...
int conninfra_dev_dbg_init(void)
{
	...
	if (g_conninfra_dbg_entry == NULL) {
		pr_no_info("Unable to create / wmt_aee proc entry\n\r");
		i_ret = -1;
	}
