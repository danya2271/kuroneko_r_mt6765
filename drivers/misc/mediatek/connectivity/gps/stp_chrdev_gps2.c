...
ssize_t GPS2_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (count > 0) {
...
#if GPS2_DEBUG_DUMP
		{
			...
			pr_no_info("--[GPS2-WRITE]--");
			for (k = 0; k < 10; k++) {
				if (k % 16 == 0)
					pr_no_info("\n");
				pr_no_info("0x%02x ", o_buf2[k]);
			}
			pr_no_info("\n");
		}
#endif
...
ssize_t GPS2_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    /* pr_no_info("GPS2_read(): count %d pos %lld\n", count, *f_pos); */
	if (rstflag2 == 1) {
...
#if GPS2_DEBUG_DUMP
	{
		...
		pr_no_info("--[GPS2-READ]--");
		for (k = 0; k < 10; k++) {
			if (k % 16 == 0)
				pr_no_info("\n");
			pr_no_info("0x%02x ", i_buf2[k]);
		}
		pr_no_info("--\n");
	}
#endif
