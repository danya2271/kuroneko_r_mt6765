...
#define BT_LOG_PRT_DBG(fmt, arg...)	\
	do { if (gBtDbgLevel >= BT_LOG_DBG) pr_no_info(PFX "%s: " fmt, __func__, ##arg); } while (0)
#define BT_LOG_PRT_INFO(fmt, arg...)	\
	do { if (gBtDbgLevel >= BT_LOG_INFO) pr_no_info(PFX "%s: " fmt, __func__, ##arg); } while (0)
#define BT_LOG_PRT_WARN(fmt, arg...)	\
	do { if (gBtDbgLevel >= BT_LOG_WARN) pr_no_info(PFX "%s: " fmt, __func__, ##arg); } while (0)
#define BT_LOG_PRT_ERR(fmt, arg...)	\
	do { if (gBtDbgLevel >= BT_LOG_ERR) pr_no_info(PFX "%s: " fmt, __func__, ##arg); } while (0)
				...
				if (gBtDbgLevel >= BT_LOG_DBG) { \
					...
					for (cnt_ = 0; cnt_ < len_; ++cnt_) {	\
						if (snprintf(raw_buf+5*cnt_, 6, "0x%02X ", ptr[cnt_]) < 0) {	\
							pr_no_info("snprintf error\n");	\
							break;	\
						}	\
					}	\
					raw_buf[5*cnt_] = '\0'; \
					if (l <= RAW_MAX_BYTES) {	\
						pr_no_info(PFX" "fmt"%s\n", ##__VA_ARGS__, raw_buf);	\
					} else {	\
						pr_no_info(PFX" "fmt"%s (prtail)\n", ##__VA_ARGS__, raw_buf); \
					}	\
				}	\
			} while (0)
