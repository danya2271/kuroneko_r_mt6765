...
MTK_WCN_BOOL mtk_wcn_stp_psm_dbg_level(INT32 dbglevel)
{
	if (dbglevel >= 0 && dbglevel <= 4) {
		...
		STP_PSM_pr_no_info("gPsmDbgLevel = %d\n", gPsmDbgLevel);
		return true;
	}
	STP_PSM_pr_no_info("invalid psm debug level. gPsmDbgLevel = %d\n", gPsmDbgLevel);
	return false;
}
#if 0
...
static INT32 _stp_psm_handler(MTKSTP_PSM_T *stp_psm, P_STP_OP pStpOp)
{
	...
	if (ret) {
		STP_PSM_pr_no_info("--->lock psm_thread_lock failed ret=%d\n", ret);
		return ret;
	}
	...
	case STP_OPID_PSM_SLEEP:
		if (stp_psm_check_sleep_enable(stp_psm) > 0)
			...
			STP_PSM_pr_no_info("cancel sleep request\n");
		break;
	...
	default:
		STP_PSM_pr_no_info("invalid operation id (%d)\n", pStpOp->opId);
		ret = -1;
		break;
	}
...
static P_OSAL_OP _stp_psm_get_op(MTKSTP_PSM_T *stp_psm, P_OSAL_OP_Q pOpQ)
{
	...
	if (!pOp)
		STP_PSM_pr_no_info("RB_GET fail\n");
	...
	if (pOpQ == &stp_psm->rActiveOpQ) {
		...
		while ((read_idx & RB_MASK(pOpQ)) != (write_idx & RB_MASK(pOpQ))) {
			...
			if (opId < STP_OPID_PSM_NUM)
				...
				STP_PSM_pr_no_info("Unknown OP Id\n");
			++read_idx;
		}
		STP_PSM_PR_DBG("Active op list:--\n");
	} else
	...
	if (pOpQ == &stp_psm->rActiveOpQ) {
		...
		while (RB_COUNT(pOpQ) > 2) {
			...
			if ((prev_opId == STP_OPID_PSM_SLEEP
				...
				|| (prev_opId == STP_OPID_PSM_HOST_AWAKE
				    && prev_prev_opId == STP_OPID_PSM_WAKEUP)) {
				STP_PSM_pr_no_info("prev_opId(%d), prev_prev_opId(%d)\n", prev_opId,
						  prev_prev_opId);
				RB_GET(pOpQ, pOp);
				RB_PUT(pFreeOpQ, pOp);
			} else {
...
static INT32 _stp_psm_put_op(MTKSTP_PSM_T *stp_psm, P_OSAL_OP_Q pOpQ, P_OSAL_OP pOp)
{
	...
	/* if (!pOpQ || !pOp) */
	/* { */
	/* STP_PSM_pr_no_info("pOpQ = 0x%p, pLxOp = 0x%p\n", pOpQ, pOp); */
	/* return 0; */
	/* } */
	ret = 0;
	...
	if (pOpQ == &stp_psm->rActiveOpQ) {
		if (!_stp_psm_is_redundant_active_op(pOp, pOpQ)) {
			...
			if (!RB_FULL(pOpQ)) {
				...
				STP_PSM_pr_no_info("************ Active Queue Full ************\n");
				ret = -1;
			}
	...
	if (ret) {
		STP_PSM_pr_no_info("RB_FULL, RB_COUNT=%d , RB_SIZE=%d\n", RB_COUNT(pOpQ),
				  RB_SIZE(pOpQ));
...
INT32 _stp_psm_put_act_op(MTKSTP_PSM_T *stp_psm, P_OSAL_OP pOp)
{
	...
	do {
		if (!stp_psm || !pOp) {
			STP_PSM_pr_no_info("stp_psm = %p, pOp = %p\n", stp_psm, pOp);
			break;
		}
		...
		if (bRet == 0) {
			STP_PSM_pr_no_info("+++++++++++ Put op Active queue Fail\n");
		...
		if (!wait_ret) {
			STP_PSM_pr_no_info("wait completion timeout\n");
			/* TODO: how to handle it? retry? */
		} else {
			if (pOp->result)
				STP_PSM_pr_no_info("op(%d) result:%d\n", pOp->op.opId, pOp->result);
	...
	if (pOp && atomic_dec_and_test(&pOp->ref_count)) {
		...
		if (bRet == 0)
			STP_PSM_pr_no_info("+++++++++++ Put op active free fail, maybe disable/enable psm\n");
	}
...
static INT32 _stp_psm_proc(PVOID pvData)
{
	...
	if (!stp_psm) {
		STP_PSM_pr_no_info("!stp_psm\n");
		return -1;
	}
/* STP_PSM_pr_no_info("wmtd starts running: pWmtDev(0x%p) [pol, rt_pri, n_pri, pri]=[%d, %d, %d, %d]\n", */
/* stp_psm, current->policy, current->rt_priority, current->normal_prio, current->prio); */
	...
	for (;;) {
		...
		if (osal_thread_should_stop(&stp_psm->PSMd)) {
			STP_PSM_pr_no_info("should stop now...\n");
		...
		if (id >= STP_OPID_PSM_NUM) {
			STP_PSM_pr_no_info("abnormal opid id: 0x%x\n", id);
			result = -1;
			goto handler_done;
		}
...
handler_done:
		...
		if (result)
			STP_PSM_pr_no_info("opid id(0x%x)(%s) error(%d)\n", id,
...
static inline INT32 _stp_psm_get_state(MTKSTP_PSM_T *stp_psm)
{
	...
	STP_PSM_pr_no_info("work_state = %d, invalid\n", stp_psm->work_state);
	return -STP_PSM_OPERATION_FAIL;
}
...
static inline INT32 _stp_psm_set_state(MTKSTP_PSM_T *stp_psm, const MTKSTP_PSM_STATE_T state)
{
	...
	if (stp_psm->work_state < STP_PSM_MAX_STATE) {
		_stp_psm_get_time();
		/* STP_PSM_pr_no_info("work_state = %s --> %s\n",
...
_stp_psm_hold_data(MTKSTP_PSM_T *stp_psm, const PUINT8 buffer, const UINT32 len, const UINT8 type)
{
	...
	/* STP_PSM_pr_no_info("*******FIFO Available(%d), Need(%d)\n",
	 * available_space, needed_space);
	 */
	if (available_space < needed_space) {
		STP_PSM_pr_no_info("FIFO Available!! Reset FIFO\n");
...
INT32 _stp_psm_release_data(MTKSTP_PSM_T *stp_psm)
{
	...
	/* STP_PSM_pr_no_info("++++++++++release data++len=%d\n", osal_fifo_len(&stp_psm->hold_fifo)); */
	while ((osal_fifo_len(&stp_psm->hold_fifo) && i > 0) || winspace_flag > 0) {
		...
		if (winspace_flag == 0) {
			...
			if (len > STP_PSM_PACKET_SIZE_MAX) {
				STP_PSM_pr_no_info("***psm packet's length too Long!****\n");
				STP_PSM_pr_no_info("***reset psm's fifo***\n");
			} else {
		...
		if (delimiter[0] == 0xbb && delimiter[1] == 0xbb) {
			...
			STP_PSM_pr_no_info("***psm packet fifo parsing fail****\n");
			STP_PSM_pr_no_info("***reset psm's fifo***\n");
		...
		if (winspace_flag > 0 && winspace_flag < 10)
			...
			STP_PSM_pr_no_info("***More than 20ms no winspace available***\n");
			break;
		}
	}
	return STP_PSM_OPERATION_SUCCESS;
...
static inline INT32 _stp_psm_notify_wmt_wakeup_wq(MTKSTP_PSM_T *stp_psm)
{
	...
	bRet = _stp_psm_put_act_op(stp_psm, pOp);
	if (bRet == 0) {
		STP_PSM_pr_no_info("OPID(%d) type(%zd) bRet(%s)\n\n",
...
static inline INT32 _stp_psm_reset(MTKSTP_PSM_T *stp_psm)
{
	...
	if (ret) {
		STP_PSM_pr_no_info("--->lock stp_psm->user_lock failed, ret=%d\n", ret);
		return ret;
	}
	/* --> disable psm <--// */
	stp_psm->flag.data = 0;
...
static inline INT32 _stp_psm_wait_wmt_event_wq(MTKSTP_PSM_T *stp_psm)
{
	...
	if (mtk_wcn_stp_get_wmt_trg_assert() == 1 || mtk_wcn_stp_coredump_start_get() == 1) {
		STP_PSM_pr_no_info("Host/Fw already triggered assert. Skip psm operation.\n");
		return STP_PSM_OPERATION_FAIL;
	}
	...
	if (osal_test_bit(STP_PSM_WMT_EVENT_WAKEUP_EN, &stp_psm->flag)) {
		...
		if (_stp_psm_get_state(stp_psm) == ACT_INACT) {
			...
			_stp_psm_set_state(stp_psm, INACT);
			STP_PSM_pr_no_info("[WARNING]PSM state rollback due too wakeup fail\n");
		}
	} else if (osal_test_bit(STP_PSM_RESET_EN, &stp_psm->flag)) {
		osal_clear_bit(STP_PSM_WMT_EVENT_ROLL_BACK_EN, &stp_psm->flag);
		_stp_psm_dbg_dmp_in(g_stp_psm_dbg, stp_psm->flag.data, __LINE__);
	} else {
		STP_PSM_pr_no_info("state = %d, flag = %ld<== Abnormal flag be set!!\n\r",
	...
	case ACT_INACT:
		if (action == SLEEP) {
			...
			if (action < STP_PSM_MAX_ACTION) {
				STP_PSM_pr_no_info("Action = %s, state = %d, flag = %ld, abnormal!\n",
						g_psm_action[action],
						stp_psm->work_state, stp_psm->flag.data);
			} else {
				STP_PSM_pr_no_info("Invalid Action!!\n\r");
			}
			_stp_psm_dbg_out_printk(g_stp_psm_dbg);
...
static inline INT32 _stp_psm_notify_wmt(MTKSTP_PSM_T *stp_psm, const MTKSTP_PSM_ACTION_T action)
{
	...
	case ACT:
		...
		if (action == SLEEP) {
			...
			if (osal_test_bit(STP_PSM_WMT_EVENT_DISABLE_MONITOR, &stp_psm->flag)) {
				STP_PSM_pr_no_info("psm monitor disabled, can't do sleep op\n");
				osal_unlock_sleepable_lock(&stp_psm->user_lock);
				return STP_PSM_OPERATION_FAIL;
			}
			...
			if (stp_psm->wmt_notify) {
				...
				if (!ret)
					...
					STP_PSM_pr_no_info("stp_psm->wmt_notify return fail\n");

			} else {
				STP_PSM_pr_no_info("stp_psm->wmt_notify = NULL\n");
	...
	case INACT:
		...
		if (action == WAKEUP) {
			...
			STP_PSM_pr_no_info("In INACT state, dont do SLEEP again\n");
		} else {
			STP_PSM_pr_no_info("Action = %s, state = %d, flag = %ld, abnormal!\n",
					g_psm_action[action],
					stp_psm->work_state, stp_psm->flag.data);
...
static inline INT32 _stp_psm_deinit_monitor(MTKSTP_PSM_T *stp_psm)
{
	if (!stp_psm)
		return STP_PSM_OPERATION_FAIL;
	STP_PSM_pr_no_info("deinit monitor\n");
	osal_timer_stop_sync(&stp_psm->psm_timer);
	return 0;
}
...
#define POLL_WAIT_TIME 2000
	...
	while (_stp_psm_get_state(stp_psm) != state && i < limit && mtk_wcn_stp_is_enable()) {
		i++;
		if (i < 3)
			STP_PSM_pr_no_info("STP is waiting state for %s, i=%d, state = %d\n",
					  g_psm_state[state], i, _stp_psm_get_state(stp_psm));
		osal_sleep_ms(POLL_WAIT);
		if (i == 10) {
			STP_PSM_pr_no_info("-Wait for %s takes %d msec\n", g_psm_state[state], i * POLL_WAIT);
			_stp_psm_opid_dbg_out_printk(g_stp_psm_opid_dbg);
		}
	}
	if (mtk_wcn_stp_is_enable() == 0) {
		STP_PSM_pr_no_info("STP disable, maybe do chip reset");
		return STP_PSM_OPERATION_FAIL;
	}

	if (i == limit) {
		STP_PSM_pr_no_info("-Wait for %s takes %llu usec\n", g_psm_state[state], osal_elapsed_us(sec, usec));
		mtk_wcn_wmt_dump_wmtd_backtrace();
		_stp_psm_opid_dbg_out_printk(g_stp_psm_opid_dbg);
		return STP_PSM_OPERATION_FAIL;
	}
	if (i > 0)
		STP_PSM_pr_no_info("+Total waits for %s takes %llu usec\n",
					g_psm_state[state], osal_elapsed_us(sec, usec));
	return STP_PSM_OPERATION_SUCCESS;
}
...
static inline INT32 _stp_psm_do_wakeup(MTKSTP_PSM_T *stp_psm)
{
	...
	while (!RB_EMPTY(&stp_psm->rActiveOpQ)) {
		...
		if (pOp != NULL && !RB_FULL(pOpQ))
			...
			STP_PSM_pr_no_info("clear up active queue fail, freeQ full\n");
	}
	osal_unlock_unsleepable_lock(&(stp_psm->wq_spinlock));
	...
	do {
		...
		if (ret == STP_PSM_OPERATION_SUCCESS) {
			...
			/* STP_PSM_pr_no_info("<< wait ret = %d, num of activeQ = %d\n", ret,
			 * RB_COUNT(&stp_psm->rActiveOpQ));
			 */
			if (ret == STP_PSM_OPERATION_SUCCESS)
				break;
		} else
			STP_PSM_pr_no_info("_stp_psm_notify_wmt_wakeup_wq fail, retry = %d !!\n", retry);
		/* STP_PSM_pr_no_info("retry = %d\n", retry); */
		retry--;
		if (retry == 0)
			break;
	} while (1);
...
static inline INT32 _stp_psm_disable(MTKSTP_PSM_T *stp_psm)
{
	...
	if (ret == STP_PSM_OPERATION_SUCCESS)
		...
		STP_PSM_pr_no_info("***PSM Disable Fail***\n");
...
static inline INT32 _stp_psm_enable(MTKSTP_PSM_T *stp_psm, INT32 idle_time_to_sleep)
{
	...
	if (ret == STP_PSM_OPERATION_SUCCESS) {
		...
		STP_PSM_pr_no_info("***PSM Enable Fail***\n");
	osal_unlock_sleepable_lock(&stp_psm->user_lock);
	return ret;
}
...
*/
	...
	if (dir == 0) {		/* tx */
		...
		if (((long)jiffies - (long)tx_end_time >= 0) || (is_tx_first)) {
			...
			STP_PSM_pr_no_info("tx cnt = %d in the previous 3 sec,tx_th = %d\n", tx_cnt,
					  tx_cnt_th);
		...
		if (((long)jiffies - (long)rx_end_time >= 0) || (is_rx_first)) {
			...
			STP_PSM_pr_no_info("rx cnt = %d in the previous 3 sec, rx_th = %d\n", rx_cnt,
					  rx_cnt_th);
	...
	if (sample_start) {
		...
		if (((tv_now.tv_sec == tv_end.tv_sec) && (tv_now.tv_usec > tv_end.tv_usec)) ||
			...
			if ((rx_sum_len + tx_sum_len) > RTX_SPEED_THRESHOLD) {
				...
				STP_PSM_pr_no_info("Low speed,Enable monitor\n");
...
INT32 stp_psm_set_sleep_enable(MTKSTP_PSM_T *stp_psm)
{
	...
	if (stp_psm) {
		...
		STP_PSM_pr_no_info("Null pointer\n");
		ret = -1;
	}
...
static INT32 _stp_psm_dbg_out_printk(STP_PSM_RECORD_T *stp_psm_dbg)
{
	...
	if (!stp_psm_dbg) {
		STP_PSM_pr_no_info("NULL g_stp_psm_dbg reference\n");
		return -1;
	}
	osal_lock_unsleepable_lock(&stp_psm_dbg->lock);
	...
	STP_PSM_pr_no_info("loged record size = %d, in(%d), out(%d)\n", dumpSize, inIndex, outIndex);
	while (dumpSize > 0) {
...
static INT32 _stp_psm_opid_dbg_out_printk(P_STP_PSM_OPID_RECORD p_opid_dbg)
{
	...
	if (!p_opid_dbg) {
		STP_PSM_pr_no_info("NULL p_opid_dbg reference\n");
		return -1;
	}
	osal_lock_unsleepable_lock(&p_opid_dbg->lock);
...
MTKSTP_PSM_T *stp_psm_init(VOID)
{
	...
	if (ret < 0) {
		STP_PSM_pr_no_info("FIFO INIT FAILS\n");
		goto ERR_EXIT4;
	}
...
/* osal_unsleepable_lock_init(&stp_psm->flagSpinlock); */
	...
	if (ret < 0) {
		STP_PSM_pr_no_info("osal_thread_create fail...\n");
		goto ERR_EXIT5;
	}
	...
	if (err) {
		STP_PSM_pr_no_info("__stp_psm_init ERROR\n");
		goto ERR_EXIT6;
	}
	/* Start STPd thread */
	ret = osal_thread_run(&stp_psm->PSMd);
	if (ret < 0) {
		STP_PSM_pr_no_info("osal_thread_run FAILS\n");
		goto ERR_EXIT6;
	}
...
ERR_EXIT6:
	...
	if (ret < 0) {
		STP_PSM_pr_no_info("osal_thread_destroy FAILS\n");
		goto ERR_EXIT5;
	}
ERR_EXIT5:
...
INT32 stp_psm_deinit(MTKSTP_PSM_T *stp_psm)
{
	...
	if (ret < 0)
		STP_PSM_pr_no_info("_stp_psm_deinit_monitor ERROR\n");
