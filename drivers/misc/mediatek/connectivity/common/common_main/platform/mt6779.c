			...
			if ((CONSYS_REG_READ(check_coredump_reg) == 0)
				...
				while (retry-- > 0) {
					...
					if (check_sleep == MCU_SLEEP_DONE) {
						WMT_PLAT_pr_no_info("consys is ready to sleep\n");
						break;
					}
					WMT_STEP_DO_ACTIONS_FUNC
							(STEP_TRIGGER_POINT_POWER_OFF_HANDSHAKE);
					WMT_PLAT_pr_no_info("check_sleep_reg=(0x%x)\n", check_sleep);
					msleep(20);
				}
...
static INT32 polling_consys_chipid(VOID)
{
	...
	if (consys_reg_base) {
		...
		WMT_PLAT_pr_no_info("connsys identify adie cr address(0x%x) ioremap fail!\n",
...
static VOID consys_afe_reg_setting(VOID)
{
	WMT_PLAT_pr_no_info("No need to do afe");
}
	...
	if (enable) {
		...
		if (reg_VCN33_1_BT) {
			...
			if (regulator_enable(reg_VCN33_1_BT))
				WMT_PLAT_pr_no_info("WMT do BT PMIC on fail!\n");
			}
		}
	} else {
		if (mtk_wcn_consys_get_adie_chipid() == PRIMARY_ADIE) {
		...
		if (enable) {
			...
			if (reg_VCN33_2_WIFI) {
				...
				if (regulator_enable(reg_VCN33_2_WIFI))
					WMT_PLAT_pr_no_info("[%s::%d] WMT do VCN33_2 PMIC on fail!\n", __func__, __LINE__);
			}
#if (!COMMON_KERNEL_PMIC_SUPPORT)
...
#endif
			WMT_PLAT_pr_no_info("turn on vcn33_2 for fm/gps usage in co-clock mode\n");
		} else {
			...
			WMT_PLAT_pr_no_info("turn off vcn33_2 for fm/gps usage in co-clock mode\n");
		}
	}
#endif
...
static INT32 consys_dump_osc_state(P_CONSYS_STATE state)
{
	...
	WMT_PLAT_pr_no_info("0x%08x: 0x%x\n", state->lp[0], state->lp[1]);
	...
	if (addr)
		...
		WMT_PLAT_pr_no_info("ioremap fail\n");
	iounmap(addr);
#endif
