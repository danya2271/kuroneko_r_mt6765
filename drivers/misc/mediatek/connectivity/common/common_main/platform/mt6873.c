...
INT32 mtk_wcn_consys_jtag_set_for_mcu(VOID)
{
	...
	if (gJtagCtrl) {
		...
		if (remap_addr3 == 0) {
			WMT_PLAT_pr_no_info("remap jtag_addr3 fail!\n");
			ret =  -1;
			goto error;
		}
...
static INT32 consys_hw_power_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		if (iRet)
			WMT_PLAT_pr_no_info("pm_runtime_get_sync() fail(%d)\n", iRet);
		else
			WMT_PLAT_pr_no_info("pm_runtime_get_sync() CONSYS ok\n");

		iRet = device_init_wakeup(&connsys_pdev->dev, true);
		if (iRet)
			WMT_PLAT_pr_no_info("device_init_wakeup(true) fail.\n");
		else
			WMT_PLAT_pr_no_info("device_init_wakeup(true) CONSYS ok\n");
#else
		...
		if (iRet)
			WMT_PLAT_pr_no_info("device_init_wakeup(false) fail.\n");
		else
			WMT_PLAT_pr_no_info("device_init_wakeup(false) CONSYS ok\n");

		iRet = pm_runtime_put_sync(&connsys_pdev->dev);
		if (iRet)
			WMT_PLAT_pr_no_info("pm_runtime_put_sync() fail.\n");
		else
			WMT_PLAT_pr_no_info("pm_runtime_put_sync() CONSYS ok\n");
#else
...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		if (consys_is_rc_mode_enable()) {
			/* RC mode */
			WMT_PLAT_pr_no_info("Turn on VCN18, VCN13 in RC mode\n");
			/* VCN18 */
#if (!COMMON_KERNEL_PMIC_SUPPORT)
		...
		WMT_PLAT_pr_no_info("Turn off VCN13, VCN18\n");
		if (reg_VCN13)
		...
		if (reg_VCN18) {
			if (regulator_disable(reg_VCN18))
				WMT_PLAT_pr_no_info("disable VCN_1V8 fail!\n");
			else
...
static INT32 consys_hw_bt_vcn33_ctrl(UINT32 enable)
{
	if (enable) {
		...
		if (consys_is_rc_mode_enable()) {
			WMT_PLAT_pr_no_info("Do nothing for reg_VCN33_1_BT under RC mode when disable\n");
		} else {
#if (!COMMON_KERNEL_PMIC_SUPPORT)
