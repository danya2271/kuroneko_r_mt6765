...
{
	WMT_PLAT_pr_no_info("Does not support on combo\n");
	return 0;
}
...
static VOID mtk_wcn_get_regmap(struct platform_device *pdev)
{
	...
	if (!pmic_node) {
		WMT_PLAT_pr_no_info("get pmic_node fail\n");
		return;
	}

	pmic_pdev = of_find_device_by_node(pmic_node);
	if (!pmic_pdev) {
		WMT_PLAT_pr_no_info("get pmic_pdev fail\n");
		return;
	}

	chip = dev_get_drvdata(&(pmic_pdev->dev));
	if (!chip) {
		WMT_PLAT_pr_no_info("get chip fail\n");
		return;
	}
	...
	if (IS_ERR_VALUE(g_regmap)) {
		g_regmap = NULL;
		WMT_PLAT_pr_no_info("get regmap fail\n");
	}
}
#endif
...
static INT32 wmt_allocate_connsys_emi(struct platform_device *pdev)
{
	...
	if (!np) {
		WMT_PLAT_pr_no_info("no memory-region, np is NULL\n");
		return -1;
	}
	...
	if (!rmem) {
		WMT_PLAT_pr_no_info("no memory-region\n");
		return -1;
	}
...
static INT32 wmt_thermal_register(struct platform_device *pdev)
{
	...
	if (IS_ERR(tz)) {
		ret = PTR_ERR(tz);
		WMT_PLAT_pr_no_info("Failed to register thermal zone device %d\n", ret);
		return -1;
	}
	WMT_PLAT_pr_no_info("Register thermal zone device.\n");
	return 0;
}
...
static INT32 mtk_wmt_probe(struct platform_device *pdev)
{
	...
	if (pdev)
		...
		WMT_PLAT_pr_no_info("pdev is NULL\n");
		return -1;
	}
	...
	if (gConEmiPhyBase) {
		...
		WMT_PLAT_pr_no_info("Clearing Connsys EMI (virtual(0x%p) physical(0x%pa)) %llu bytes\n",
		...
		WMT_PLAT_pr_no_info("consys emi memory address gConEmiPhyBase invalid\n");
	}

#ifdef CONFIG_MTK_HIBERNATION
	WMT_PLAT_pr_no_info("register connsys restore cb for complying with IPOH function\n");
	...
	if (IS_ERR(consys_pinctrl)) {
		WMT_PLAT_pr_no_info("cannot find consys pinctrl.\n");
		consys_pinctrl = NULL;
	}
	...
	if (consys_pinctrl) {
		...
		if (pinctl_node) {
			...
			if (pins_node) {
				...
				WMT_PLAT_pr_no_info("GPS LNA gpio pin number:%d, pinmux:0x%08x.\n",
	...
	WMT_PLAT_pr_no_info("ant swap pin number:%d\n", wifi_ant_swap_gpio_pin_num);
...
static INT32 mtk_wmt_suspend(struct device *dev)
{
	WMT_PLAT_pr_no_info(" mtk_wmt_suspend !!");
...
static void plat_resume_handler(struct work_struct *work)
{
	...
	if (wmt_lib_dmp_consys_state(&dmp_info, dmp_cnt, 0) == MTK_WCN_BOOL_TRUE) {
		...
		if ((wmt_lib_get_drv_status(WMTDRV_TYPE_WMT) != DRV_STS_FUNC_ON)
			...
			WMT_PLAT_pr_no_info("TOP:0,0;MCU:0,0;BT:0,0;WIFI:0,0;GPS:0,0\n");
		}
	}
}
...
static INT32 mtk_wmt_resume(struct device *dev)
{
	WMT_PLAT_pr_no_info(" mtk_wmt_resume !!");
...
INT32 mtk_wcn_consys_hw_reg_ctrl(UINT32 on, UINT32 co_clock_type)
{
	...
	WMT_PLAT_pr_no_info("CONSYS-HW-REG-CTRL(0x%08x),start\n", on);
	...
	if (on) {
		...
		if (co_clock_type) {
			WMT_PLAT_pr_no_info("co clock type(%d),turn on clk buf\n", co_clock_type);
	...
	WMT_PLAT_pr_no_info("CONSYS-HW-REG-CTRL(0x%08x),finish\n", on);
	return iRet;
}
...
INT32 mtk_wcn_consys_hw_efuse_paldo_ctrl(UINT32 enable, UINT32 co_clock_type)
{
	if (co_clock_type) {
		if (wmt_consys_ic_ops->consys_ic_hw_vcn28_ctrl)
			wmt_consys_ic_ops->consys_ic_hw_vcn28_ctrl(enable);
		if (enable)
			WMT_PLAT_pr_no_info("turn on vcn28 for efuse usage in co-clock mode\n");
		else
			WMT_PLAT_pr_no_info("turn off vcn28 for efuse usage in co-clock mode\n");
	}
	return 0;
}
...
INT32 mtk_wcn_consys_detect_adie_chipid(UINT32 co_clock_type)
{
	...
	if (wmt_consys_ic_ops && wmt_consys_ic_ops->consys_ic_adie_chipid_detect) {
		WMT_PLAT_pr_no_info("CONSYS A-DIE DETECT start\n");
		chipid = wmt_consys_ic_ops->consys_ic_adie_chipid_detect();
		if (chipid > 0) {
			g_adie_chipid = chipid;
			WMT_PLAT_pr_no_info("Set a-die chipid = %x\n", chipid);
		} else
			WMT_PLAT_pr_no_info("Detect a-die chipid = %x failed!\n", chipid);
		WMT_PLAT_pr_no_info("CONSYS A-DIE DETECT finish\n");
	}
...
INT32 mtk_wcn_consys_hw_pwr_on(UINT32 co_clock_type)
{
	...
	WMT_PLAT_pr_no_info("CONSYS-HW-PWR-ON, start\n");
	WMT_STEP_DO_ACTIONS_FUNC(STEP_TRIGGER_POINT_POWER_ON_START);
	if (!gConEmiPhyBase) {
		WMT_PLAT_pr_no_info("EMI base address is invalid, CONNSYS can not be powered on!");
		return -WMT_ERRCODE_EMI_NOT_READY;
	}
	iRet = mtk_wcn_consys_hw_reg_ctrl(1, co_clock_type);
	if (iRet) {
		WMT_PLAT_pr_no_info("CONSYS-HW-PWR-ON, failed!(%d)\n", iRet);
		return iRet;
	}
	iRet = mtk_wcn_consys_hw_gpio_ctrl(1);
	mtk_wcn_consys_jtag_set_for_mcu();

	WMT_PLAT_pr_no_info("CONSYS-HW-PWR-ON, finish(%d)\n", iRet);
...
INT32 mtk_wcn_consys_hw_rst(UINT32 co_clock_type)
{
	...
	WMT_PLAT_pr_no_info("CONSYS-HW, hw_rst start, eirq should be disabled before this step\n");
	...
	WMT_PLAT_pr_no_info("CONSYS-HW, hw_rst finish, eirq should be enabled after this step\n");
	return iRet;
}
...
INT32 mtk_wcn_consys_hw_init(VOID)
{
	...
	if (iRet)
		WMT_PLAT_pr_no_info("WMT platform driver registered failed(%d)\n", iRet);
	else {
		while (atomic_read(&g_probe_called) == 0 && retry < 100) {
			osal_sleep_ms(50);
			retry++;
			WMT_PLAT_pr_no_info("g_probe_called = 0, retry = %d\n", retry);
		}
	}
...
PUINT8 mtk_wcn_consys_emi_virt_addr_get(UINT32 ctrl_state_offset)
{
	...
	if (!pEmibaseaddr) {
		WMT_PLAT_pr_no_info("EMI base address is NULL\n");
		return NULL;
	}
...
INT32 mtk_wcn_consys_set_dbg_mode(UINT32 flag)
{
	...
	if (!vir_addr) {
		WMT_PLAT_pr_no_info("get vir address fail\n");
		return -2;
	}
	if (flag) {
		ret = 0;
	...
	WMT_PLAT_pr_no_info("fw dbg mode register value(0x%08x)\n", CONSYS_REG_READ(vir_addr));
	return ret;
}
...
INT32 mtk_wcn_consys_set_dynamic_dump(PUINT32 str_buf)
{
	...
	WMT_PLAT_pr_no_info("dynamic dump register value(0x%08x)\n", CONSYS_REG_READ(vir_addr));
	return 0;
}
		...
		PUINT32 value)
{
	...
	if (!reg_info) {
		WMT_PLAT_pr_no_info("reg_info osal_malloc fail\n");
		goto fail;
	}
	...
	if (node) {
		if (of_property_read_u32_array(node, "reg", reg_info, reg_info_size)) {
			WMT_PLAT_pr_no_info("get reg from DTS fail!!\n");
			goto fail;
		}
	} else {
	...
	if (reg_info[index*4 + 3] < offset) {
		WMT_PLAT_pr_no_info("Access overflow of address(0x%x), offset(0x%x)!\n",
...
UINT64 mtk_wcn_consys_get_options(VOID)
{
	...
	WMT_PLAT_pr_no_info("Please implement consys_ic_get_options!");
