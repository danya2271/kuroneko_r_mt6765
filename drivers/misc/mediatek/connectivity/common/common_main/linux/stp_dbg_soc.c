...
static _osal_inline_ INT32 stp_dbg_soc_put_emi_dump_to_nl(PUINT8 data_buf, INT32 dump_len)
{
	...
	if (dump_len > 0) {
		...
		STP_DBG_pr_no_info("dump entry length is 0\n");
...
static _osal_inline_ INT32 stp_dbg_soc_paged_dump(INT32 dump_sink)
{
	...
	if (chip_reset_only) {
		STP_DBG_pr_no_info("is chip reset only\n");
		ret = -3;
		return ret;
	}
	...
	if (dump_num != 0) {
		packet_num = dump_num;
		STP_DBG_pr_no_info("get consys dump num packet_num(%d)\n", packet_num);
	} else {
		dump_num = CORE_DUMP_NUM;
		STP_DBG_pr_no_info("can not get consys dump num and default num is %d\n", CORE_DUMP_NUM);
	}
	...
	while (1) {
		...
		if (wmt_plat_get_dump_info(p_ecsi->p_ecso->emi_apmem_ctrl_assert_flag) == 2) {
			STP_DBG_pr_no_info("coredump handshake start\n");
			break;
	...
	do {
...
#if !WMT_DBG_SUPPORT
				STP_DBG_pr_no_info("Wait Timeout: %llu > %d\n", elapsed_time, EMI_SYNC_TIMEOUT);
		...
		if (!dump_phy_addr) {
			STP_DBG_pr_no_info("get paged dump phy address fail\n");
			ret = -1;
			break;
		}
		...
		if (dump_len <= 32 * 1024) {
			...
			case 1:
				...
				if (ret == 0)
					...
					STP_DBG_pr_no_info("aee send timeout!\n");
					abort = 1;
					goto paged_dump_end;
				} else
					STP_DBG_pr_no_info("aee send error!\n");
				break;
			case 2:
				...
				if (ret == 0)
					...
					STP_DBG_pr_no_info("dump send timeout!\n");
					abort = 1;
					goto paged_dump_end;
				} else
					STP_DBG_pr_no_info("dump send error!\n");
				break;
			default:
				STP_DBG_pr_no_info("unknown sink %d\n", dump_sink);
				return -1;
			}
		} else
...
paged_dump_end:
		wmt_plat_set_host_dump_state(STP_HOST_DUMP_NOT_START);
		STP_DBG_pr_no_info("++ counter(%d) packet_num(%d) page_counter(%d) g_paged_dump_len(%d) fw_state(%d)++\n",
		...
		if (wmt_plat_get_dump_info(p_ecsi->p_ecso->emi_apmem_ctrl_chip_paded_dump_end) ||
			...
			if (stp_dbg_get_coredump_timer_state() == CORE_DUMP_DOING) {
				STP_DBG_pr_no_info("paged dump end by emi flag\n");
				if (dump_sink == 1)
				...
				STP_DBG_pr_no_info("paged dump end\n");
			ret = 0;
			break;
		} else if (abort || stp_dbg_get_coredump_timer_state() == CORE_DUMP_TIMEOUT) {
			STP_DBG_pr_no_info("paged dump fail, generate fake coredump message\n");
...
static _osal_inline_ INT32 stp_dbg_soc_paged_trace(VOID)
{
	...
	p_ecsi = wmt_plat_get_emi_phy_add();
	if (p_ecsi == NULL) {
		STP_DBG_pr_no_info("get EMI info failed.\n");
		return -1;
	}
	...
	do {
		...
		if (loop_cnt1 >= 10) {
			STP_DBG_pr_no_info("polling CTRL STATE fail\n");
			ret = -1;
			break;
		}
		...
		STP_DBG_pr_no_info("paged trace buffer addr(%08x),buffer_len(%d)\n", buffer_start,
				buffer_idx);
		dump_vir_addr = wmt_plat_get_emi_virt_add(buffer_start - p_ecsi->emi_phy_addr);
		if (!dump_vir_addr) {
			STP_DBG_pr_no_info("get vir dump address fail\n");
			ret = -2;
			break;
		}
...
INT32 stp_dbg_soc_core_dump(INT32 dump_sink)
{
	...
	if (dump_sink == 0 || chip_reset_only == 1) {
		if (chip_reset_only) {
			STP_DBG_pr_no_info("Chip reset only\n");
			chip_reset_only = 0;
		}
		...
		if (ret)
			STP_DBG_pr_no_info("stp_dbg_soc_paged_trace fail: %d!\n", ret);
...
PUINT8 stp_dbg_soc_id_to_task(UINT32 id)
{
	...
	if (id >= STP_DBG_TASK_ID_MAX) {
		STP_DBG_pr_no_info("task id(%d) overflow(%d)\n", id, STP_DBG_TASK_ID_MAX);
		return NULL;
	}
	...
	if (patch_name == NULL) {
		STP_DBG_pr_no_info("patch_name is null\n");
	...
	if (osal_strncmp(patch_name, ROM_V2_PATCH, strlen(ROM_V2_PATCH)) == 0) {
		...
		STP_DBG_pr_no_info("id = %d, gen two task_id = %d\n", id, temp_id);
	} else if (osal_strncmp(patch_name, ROM_V3_PATCH, strlen(ROM_V3_PATCH)) == 0) {
		temp_id = soc_gen_three_task_id_adapter[id];
		STP_DBG_pr_no_info("id = %d, gen three task_id = %d\n", id, temp_id);
	} else if (osal_strncmp(patch_name, ROM_V4_PATCH, strlen(ROM_V4_PATCH)) == 0) {
		temp_id = soc_gen_three_task_id_adapter[id];
		STP_DBG_pr_no_info("id = %d, gen three (ROMv4) task_id = %d\n", id, temp_id);
	} else {
		temp_id = id;
		STP_DBG_pr_no_info("id = %d, CONNAC project task_id = %d\n", id, temp_id);
	}
...
#define CONSYS_REG_READ(addr) (*((volatile UINT32 *)(addr)))
	...
	if (cr == CONNSYS_EMI_REMAP) {
		if (emi_phy_addr != NULL && emi_phy_addr->emi_remap_offset)
			...
			STP_DBG_pr_no_info("EMI remap has no value\n");
	}
