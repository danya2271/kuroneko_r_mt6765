		...
		struct mm_freq_step_config *step_config, u32 step)
{
	...
	if (step_config->clk_mux == NULL ||
		...
		pr_no_notice("CCF handle can't be NULL during MMDVFS\n");
		return -EINVAL;
	}
	...
	if (ret) {
		pr_no_notice("prepare clk(%d): %s-%u\n",
			ret, name, step);
		return -EFAULT;
	}
		...
		struct mm_freq_step_config *step_config, u32 step)
{
	...
	if (ret)
		pr_no_notice("hopping rate(%d):(%u)-0x%08x, %s-%u\n",
	...
	struct mm_freq_config *config, u32 step, s32 old_step)
{
	...
	if (step_config->clk_type == CLK_TYPE_NONE) {
		pr_no_notice("No need to change clk of %s\n", config->prop_name);
		return 0;
	}
...
static u32 mmdvfs_get_limit_status(u32 pm_qos_class)
{
	...
	if (i >= ARRAY_SIZE(all_freqs)) {
		pr_no_notice("[GET]Invalid class: %u\n", pm_qos_class);
		return false;
	}
...
static void update_step(u32 pm_qos_class, s32 src_mux_id)
{
	...
	if (!mmdvfs_enable || !mmdvfs_autok_enable) {
		pr_no_notice("mmdvfs qos is disabled(%d)\n", pm_qos_class);
		return;
	}

	if (!step_size) {
		pr_no_notice("no step available skip\n");
		return;
	}
...
static void simulate_dvfsrc(s32 next_hrt_bw)
{
	...
	if ((is_up && next_opp < current_opp) ||
		...
		if (log_level & 1 << log_bw)
			pr_no_notice("up=%d copp=%d nopp=%d cbw=%d nbw=%d\n",
...
static void log_hrt_bw_info(u32 master_id)
{
	...
	if (log_level & 1 << log_bw)
		pr_no_notice("%s larb=%d p1=%d ccu=%d disp=%d ddr_opp=%d\n",
...
static void update_hrt_bw_to_dvfsrc(s32 next_hrt_bw)
{
	...
	if (log_level & 1 << log_bw)
		pr_no_notice("%s report dvfsrc mm_hrt_bw=%d\n",
	...
	struct mm_qos_request *req, u32 smi_master_id)
{
	...
	if (larb_id >= MAX_LARB_COUNT || port_id >= MAX_PORT_COUNT) {
		pr_no_notice("mm_add(0x%08x) Invalid master_id\n", smi_master_id);
		return -EINVAL;
	}
	if (req->init) {
	...
	if (log_level & 1 << log_bw) {
		pr_no_notice("mm_add larb=%u port=%d\n", larb_id, port_id);
		pr_no_notice("req=%p\n", req);
	}
	return 0;
}
	...
	u32 hrt_value, u32 comp_type)
{
	...
	if (!req->init || larb >= MAX_LARB_COUNT ||
		...
		pr_no_notice("mm_set(0x%08x) init=%d larb=%d port=%d comp=%d\n",
			req->master_id, req->init, larb, port, comp_type);
		dump_stack();
		return -EINVAL;
	}
	if (!larb_req[larb].port_count || !larb_req[larb].ratio[port]) {
		pr_no_notice("mm_set(0x%08x) invalid port_cnt=%d ratio=%d\n",
	...
	if (req->hrt_value == hrt_value &&
		...
		if (log_level & 1 << log_bw)
			pr_no_notice("mm_set(0x%08x) no change\n", req->master_id);
		return 0;
	}
	...
	if (log_level & 1 << log_bw) {
		pr_no_notice("set=0x%08x comp=%u,%u\n", req->master_id,
		comp_type, req->comp_type);
		pr_no_notice("set=0x%08x bw=%u,%u total_bw=%d\n", req->master_id,
		bw_value, req->bw_value, larb_req[larb].total_bw_data);
		pr_no_notice("set=0x%08x hrt=%u,%u total_hrt=%d\n", req->master_id,
		hrt_value, req->hrt_value, larb_req[larb].total_hrt_data);
		pr_no_notice("set=0x%08x o_mix=%u total_mix=%d\n", req->master_id,
		old_larb_mix_value, larb_req[larb].total_mix_limit);
	}
	...
	if (log_level & 1 << log_bw)
		pr_no_notice("mm_set=0x%08x bw=%u ostd=%u hrt=%u comp=%u\n",
...
void mm_qos_update_all_request(struct plist_head *owner_list)
{
	...
	if (is_camera_larb(req->master_id)) {
		...
		if (cam_bw > camera_max_bw) {
			pr_no_notice("cam_bw(%d) > camera_max_bw(%d)\n",
		...
		if (cam_scen_change) {
			pr_no_notice("scenario change time=%u cam_bw=%d\n",
		...
		if (total_hrt_bw != UNINITIALIZED_VALUE &&
			...
			pr_no_notice("hrt bw overflow used=%d avail=%d\n",
				get_total_used_hrt_bw(), total_hrt_bw);
	}
	...
	plist_for_each_entry(req, owner_list, owner_node) {
		...
		if (log_level & 1 << log_bw)
			pr_no_notice("update(0x%08x) ostd=%d value=%d hrt=%d\n",
	...
	if (!skip_smi_config) {
		...
		if (log_level & 1 << log_bw)
			pr_no_notice("config SMI (%d) cost: %llu us\n",
	...
	for (comm = 0; comm < MAX_COMM_NUM; comm++) {
		...
		for (i = 0; i < MAX_CH_COUNT; i++) {
			...
			if (log_level & 1 << log_smi_freq)
				pr_no_notice("comm:%d chn:%d s_bw:%d h_bw:%d\n",
		...
		if (log_level & 1 << log_smi_freq)
			pr_no_notice("comm:%d smi_srt_clk:%d smi_hrt_clk:%d\n",
...
static int notify_bw_throttle_blocking(void *data)
{
	...
	wake_up(&hrt_wait);
	pr_no_notice("decrease lock_cam_count=%d\n",
...
void mmdvfs_set_max_camera_hrt_bw(u32 bw)
{
	...
	pr_no_notice("middleware set max camera hrt bw:%d\n", bw);
#endif
}
	...
	u32 index, struct clk **clk_module)
{
	...
	result = of_property_read_string_index(dev->of_node, "clock-names",
		index, &clk_name);
	if (unlikely(result)) {
		pr_no_notice("Cannot get module name of index (%u), result (%d)\n",
			index, result);
		return;
	}
	...
	*clk_module = devm_clk_get(dev, clk_name);
	if (IS_ERR(*clk_module)) {
		/* error status print */
		pr_no_notice("Cannot get module clock: %s\n", clk_name);
		*clk_module = NULL;
	} else {
		/* message print */
		pr_no_notice("Get module clock: %s\n", clk_name);
	}
}
...
static void mmdvfs_get_larb_node(struct device *dev, u32 larb_id)
{
	...
	if (larb_id >= MAX_LARB_COUNT) {
		pr_no_notice("larb_id:%d is over MAX_LARB_COUNT:%d\n",
	...
	if (!count)
		pr_no_notice("no data in larb (%s)\n", larb_name);
	else
		init_larb_list(larb_id);
}
	...
	const char *freq_name, struct mm_freq_step_config step_configs[])
{
	...
	pr_no_notice("start get step node of %s\n", freq_name);
	of_property_for_each_string(dev->of_node, freq_name, prop, name) {
		if (count >= MAX_FREQ_STEP) {
			pr_no_notice("freq setting %s is over the MAX_STEP (%d)\n",
				freq_name, MAX_FREQ_STEP);
			break;
		}
		pr_no_notice(" node name %s\n", name);
	...
	if (count != step_size)
		pr_no_notice("freq setting %s is not same as vcore_steps (%d)\n",
			freq_name, step_size);
	pr_no_notice("%s: step size:%u\n", freq_name, step_size);
}
	...
	struct mm_freq_limit_config *limit_config)
{
	...
	for (i = 0; i < limit_size; i++) {
		...
		if (result < 0) {
			pr_no_notice("snprintf error(%d) limit name:%s id:%d\n",
				result, freq_name, i);
			continue;
		}
		pr_no_notice("[limit]%s-%d: %s\n", freq_name, i, ext_name);
		mmdvfs_get_step_array_node(dev, ext_name,
			limit_config->limit_steps[i]);
	}
#else
	pr_no_notice("MMDVFS limit is off\n");
#endif
}
...
static int mmdvfs_probe(struct platform_device *pdev)
{
	...
	pr_no_notice("vcore_steps: [%u, %u, %u, %u, %u, %u], count:%u\n",
	...
	of_property_for_each_string(node, "comm_freq", prop, mux_name) {
		if (comm_count >= MAX_COMM_NUM) {
			pr_no_notice("comm_count > MAX_COMM_NUM (%d)\n",
				MAX_COMM_NUM);
			break;
		}
		for (i = 0; i < ARRAY_SIZE(all_freqs); i++) {
		...
		if (i == ARRAY_SIZE(all_freqs)) {
			pr_no_notice("wrong comm_freq name:%s\n", mux_name);
			break;
		}
	...
	if (!vcore_reg_id)
		pr_no_info("regulator_get vcore_reg_id failed\n");
	return 0;
...
static int __init mmdvfs_pmqos_late_init(void)
{
	...
	mmdvfs_enable = false;
	pr_no_notice("force set step0 when late_init\n");
#else
	mmdvfs_qos_force_step(0);
	mmdvfs_qos_force_step(-1);
	pr_no_notice("force flip step0 when late_init\n");
#endif
	...
	enum mmdvfs_limit_source source)
{
	...
	if (unlikely(i >= ARRAY_SIZE(all_freqs))) {
		pr_no_notice("[%d]Invalid class=%u %d\n",
			source, pm_qos_class, old_level);
		return;
	}

	if (!all_freqs[i]->limit_config.limit_size) {
		pr_no_notice("[%d]Not support limit: %u\n", source, pm_qos_class);
		return;
	}

	if (log_level & log_limit)
		pr_no_notice("[%d][%d]limit score update=(%d, %u, %u)\n",
	...
	if (old_level != all_freqs[i]->limit_config.limit_level) {
		pr_no_notice("MMDVFS limit level changed for %s %d->%d\n",
...
int mmdvfs_qos_force_step(int step)
{
	if (step >= (s32)step_size || step < STEP_UNREQUEST) {
		pr_no_notice("force set step invalid: %d\n", step);
		return -EINVAL;
	}
	force_step = step;
...
int set_force_step(const char *val, const struct kernel_param *kp)
{
	...
	if (result) {
		pr_no_notice("force set step failed: %d\n", result);
		return result;
	}
...
{
	pr_no_notice("%s: step_size=%d current_max_step=%d\n",
	...
	if (enable && step_size > 0)
		mmdvfs_qos_force_step(-1);
	pr_no_notice("mmdvfs_autok enabled? %d\n", enable);
}
...
int set_enable(const char *val, const struct kernel_param *kp)
{
	...
	if (result) {
		pr_no_notice("force set enable: %d\n", result);
		return result;
	}
...
void mmdvfs_prepare_action(enum mmdvfs_prepare_event event)
{
	if (event == MMDVFS_PREPARE_CALIBRATION_START) {
		...
		pr_no_notice("mmdvfs service is disabled for calibration\n");
	} else if (event == MMDVFS_PREPARE_CALIBRATION_END) {
		mmdvfs_autok_qos_enable(true);
		pr_no_notice("mmdvfs service is enabled after calibration\n");
	} else {
		pr_no_notice("%s: unknown event code:%d\n", __func__, event);
	}
}
...
void mmdvfs_print_larbs_info(void)
{
	...
	if (log_str) {
		...
		if (len > 0) {
			...
			pr_no_notice("no larbs info to print\n");
		kfree(log_str);
	} else
		pr_no_notice("kmalloc fails!\n");
}
...
{
	pr_no_notice("ut test notifier: value=%lu\n", value);
...
int mmdvfs_qos_ut_set(const char *val, const struct kernel_param *kp)
{
	...
	pr_no_notice("ut with (case_id,req_id,master,value)=(%d,%u,%#x,%d)\n",
	...
	case 6:
		...
		if (IS_ERR(pKThread))
			pr_no_notice("create cam hrt bw thread failed\n");
		...
		wait_for_completion(&comp);
		pr_no_notice("wait time should > 2000 msecs:%u\n",
			jiffies_to_msecs(jiffies-start_jiffies));
...
int mmdvfs_ut_set(const char *val, const struct kernel_param *kp)
{
	...
	pr_no_notice("%s (case_id, value): (%d,%d)\n",
	...
	case 0:
		result = sscanf(val, "%d %d %d", &mmdvfs_ut_case,
			&value1, &value2);
		if (result != 3) {
			pr_no_notice("invalid arguments: %s\n", val);
			break;
		}
		pr_no_notice("limit test score: %d\n", value2);
		pr_no_notice("limit initial: %d\n",
		...
		pr_no_notice("limit enable then opp up: %d freq=%llu MHz\n",
			mmdvfs_get_limit_status(value1),
			mmdvfs_qos_get_freq(value1));
		/* limit disable when opp0 */
		mmdvfs_qos_limit_config(value1, 0, MMDVFS_LIMIT_THERMAL);
		pr_no_notice("limit disable when opp up: %d freq=%llu MHz\n",
			mmdvfs_get_limit_status(value1),
			mmdvfs_qos_get_freq(value1));
		/* limit enable when opp0 */
		mmdvfs_qos_limit_config(value1, 1, MMDVFS_LIMIT_THERMAL);
		pr_no_notice("limit enable when opp up: %d freq=%llu MHz\n",
			mmdvfs_get_limit_status(value1),
			mmdvfs_qos_get_freq(value1));
		/* limit disable then opp0 -> opp1 */
		mmdvfs_qos_limit_config(value1, 0, MMDVFS_LIMIT_THERMAL);
		mtk_pm_qos_update_request(&disp_req, 0);
		pr_no_notice("limit disable then opp down: %d freq=%llu MHz\n",
			mmdvfs_get_limit_status(value1),
			mmdvfs_qos_get_freq(value1));
		/* limit enable when opp1 */
		mmdvfs_qos_limit_config(value1, 1, MMDVFS_LIMIT_THERMAL);
		mtk_pm_qos_update_request(&disp_req, 0);
		pr_no_notice("limit enable when opp down: %d freq=%llu MHz\n",
			mmdvfs_get_limit_status(value1),
			mmdvfs_qos_get_freq(value1));
		/* limit disable when opp1 */
		mmdvfs_qos_limit_config(value1, 0, MMDVFS_LIMIT_THERMAL);
		mtk_pm_qos_update_request(&disp_req, 0);
		pr_no_notice("limit disable when opp down: %d freq=%llu MHz\n",
			mmdvfs_get_limit_status(value1),
			mmdvfs_qos_get_freq(value1));
	...
	case 1:
		...
		pr_no_notice("limit test score: %d\n", value2);
		pr_no_notice("limit initial: %d freq=%llu MHz\n",
			mmdvfs_get_limit_status(value1),
			mmdvfs_qos_get_freq(value1));
		mmdvfs_qos_limit_config(value1, 1, MMDVFS_LIMIT_THERMAL);
		mmdvfs_qos_limit_config(value1, value2, MMDVFS_LIMIT_CAM);
		pr_no_notice("limit now: %d freq=%llu MHz\n",
			mmdvfs_get_limit_status(value1),
			mmdvfs_qos_get_freq(value1));
...
int set_disp_bw_ceiling(const char *val, const struct kernel_param *kp)
{
	...
	pr_no_notice("%s (disp_bw, wait): (%d,%d)\n",
	...
	pr_no_notice("disp_bw_ceiling=%d total_hrt_bw=%d disp_avail_hrt_bw=%d\n",
