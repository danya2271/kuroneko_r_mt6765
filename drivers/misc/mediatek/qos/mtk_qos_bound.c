...
int qos_notifier_call_chain(unsigned long val, void *v)
{
	...
	if (v == NULL) {
		pr_no_info("detect bound null ptr(%d)\n",
	...
	if (is_qos_bound_log_enabled()) {
		...
		pr_no_info("idx: %hu, state: %hu, num: %hu, event: %hu\n",
				idx, state,
				stat->num, stat->event);
		for (i = 0; i < NR_QOS_EMIBM_TYPE; i++)
			pr_no_info("emibw [%d]: mon: %hu, req: %hu\n", i,
					stat->emibw_mon[i],
					stat->emibw_req[i]);
		for (i = 0; i < NR_QOS_SMIBM_TYPE; i++)
			pr_no_info("smibw [%d]: mon: %hu, req: %hu\n", i,
					stat->smibw_mon[i],
					stat->smibw_req[i]);
		for (i = 0; i < NR_QOS_LAT_TYPE; i++)
			pr_no_info("lat [%d]: mon: %hu\n", i, stat->lat_mon[i]);
	}
