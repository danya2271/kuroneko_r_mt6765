	...
	const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if (rc < 0) {
		...
		tag_pr_no_info("echo oppidx > /proc/cpufreq/%s/cpufreq_oppidx\n",
		p->name);
	} else {
	...
	const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if (rc < 0) {
		...
		if (ret)
			tag_pr_no_info("%s err to set_cur_volt_wrapper ret = %d\n",
				    __func__, ret);
#endif
		cpufreq_unlock(flags);
	}
...
int cpufreq_procfs_init(void)
{
	...
	if (!dir) {
		tag_pr_no_notice("fail to create /proc/cpufreq @ %s()\n",
								__func__);
		return -ENOMEM;
	}
	...
	for (i = 0; i < ARRAY_SIZE(entries); i++) {
		if (!proc_create
			...
			tag_pr_no_notice("%s(), create /proc/cpufreq/%s failed\n",
