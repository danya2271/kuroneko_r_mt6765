	...
	struct mt_cpu_dvfs *p, unsigned int target_khz, int log)
{
	...
	if (!policy) {
		tag_pr_no_notice("Can't get policy of %s\n",
		cpu_dvfs_get_name(p));
		goto out;
	}
...
{				/* volt: vproc (mv*100) */
	...
	if (cur_vproc == 0) {
		tag_pr_no_notice("@%s():%d, can not use ext buck!\n",
		__func__, __LINE__);
		return -1;
	}
...
static void dump_all_opp_table(void)
{
	...
	for_each_cpu_dvfs(i, p) {
		tag_pr_no_notice("[%s/%d] available = %d, oppidx = %d (%u, %u)\n",
			      p->name, p->cpu_id,
	...
	struct mt_cpu_dvfs *p, unsigned int target_khz, int log)
{
	...
	if (new_opp_idx == -1) {
		tag_pr_no_info("%s cant find freq idx new_opp_idx = %d\n",
...
static int _mt_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	if (dev_pm_opp_of_get_sharing_cpus(cpu_dev, policy->cpus))
		tag_pr_no_notice("failed to share opp framework table\n");

	if (dev_pm_opp_of_cpumask_add_table(policy->cpus))
		tag_pr_no_notice("failed to add opp framework table\n");

	pd_ret = em_register_perf_domain(policy->cpus, NR_FREQ, &em_cb);

	tag_pr_no_notice("energy model register result %d\n", pd_ret);

	if (pd_ret)
		tag_pr_no_notice("energy model regist fail\n");
	if (ret)
		tag_pr_no_notice("failed to setup frequency table\n");
...
static int __init _mt_cpufreq_pdrv_init(void)
{
	...
	for (i = 0; i < cluster_num; i++) {
		...
		tag_pr_no_info("cluster_id = %d, cluster_cpuid = %d\n",
		i, cpu_dvfs[i].cpu_id);
	}
	...
	if (ret) {
		tag_pr_no_notice("fail to register cpufreq device @ %s()\n",
		__func__);
	}
#endif
	...
	if (ret) {
		tag_pr_no_notice("fail to register cpufreq driver @ %s()\n",
		__func__);
#ifndef CPU_DVFS_DT_REG
