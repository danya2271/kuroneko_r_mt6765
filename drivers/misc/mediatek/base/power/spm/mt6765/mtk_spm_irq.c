...
static void mtk_spm_get_edge_trigger_irq(void)
{
	...
	pr_no_info("[SPM] edge trigger irqs:\n");
	for (i = 0; i < IRQ_NUMBER; i++) {
		node = of_find_compatible_node(NULL, NULL, list[i].name);
		if (!node) {
			pr_no_info("[SPM] find '%s' node failed\n",
				list[i].name);
			continue;
		}
...
static irqreturn_t spm_irq0_handler(int irq, void *dev_id)
{
	...
	if (isr & (ISRS_SW_INT1)) {
		pr_no_info("[SPM] IRQ0 (ISRS_SW_INT1) HANDLER SHOULD NOT BE EXECUTED (0x%x)\n",
			isr);
	...
	if (isr & (ISRS_SW_INT0 | ISRS_PCM_RETURN))
		pr_no_info("[SPM] IRQ0 HANDLER SHOULD NOT BE EXECUTED (0x%x)\n",
			isr);
	...
	for (i = 0; i < ARRAY_SIZE(irqdesc); i++) {
		if (cpu_present(i)) {
			err = request_irq(irqdesc[i].irq, irqdesc[i].handler,
				...
				"SPM", NULL);
			if (err) {
				pr_no_info("[SPM] FAILED TO REQUEST IRQ%d (%d)\n",
					i, err);
				r = -EPERM;
			}
		}
