...
{
#ifdef DCM_BRINGUP
	dcm_pr_no_info("%s: skipped for bring up\n", __func__);
	return 1;
#else
	return 0;
#endif
...
int __init mt_dcm_dts_map(void)
{
	...
	if (!node) {
		dcm_pr_no_info("error: cannot find node %s\n", INFRACFG_AO_NODE);
		return -1;
	}
	dcm_infracfg_ao_base = (unsigned long)of_iomap(node, 0);
	if (!dcm_infracfg_ao_base) {
		dcm_pr_no_info("error: cannot iomap %s\n", INFRACFG_AO_NODE);
		return -1;
	}
	...
	if (of_address_to_resource(node, 0, &r)) {
		dcm_pr_no_info("error: cannot get phys addr %s\n", MCUCFG_NODE);
		return -1;
	}
...
void dcm_set_hotplug_nb(void)
{
	...
	if (register_cpu_notifier(&dcm_hotplug_nb))
		dcm_pr_no_info("[%s]: fail to register_cpu_notifier\n", __func__);
#endif /* #ifdef CONFIG_HOTPLUG_CPU */
}
