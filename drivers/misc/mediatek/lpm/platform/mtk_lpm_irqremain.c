	...
	while ((np = of_parse_phandle(parent, MTK_LPM_IRQREMAIN_NODE, idx))) {
		...
		do {
			...
			if (!(irqtype & IRQ_TYPE_EDGE_BOTH))
				pr_no_info("[name:mtk_lpm][P] - %pOF irq(%d) type = 0x%x ?\n",
						tar_np, irqidx, irqtype);
			if (irq_do) {
				irq_set_irq_type(irqnum, irqtype);
				pr_no_info("[name:mtk_lpm][P] - %pOF set irq(%d) as type = 0x%x\n",
						tar_np, irqidx, irqtype);
			}
			...
			if (irqnum >= 0) {
				...
				if (irqnode) {
					...
					pr_no_info("[name:mtk_lpm][P] - irq_%u, wakeup-src=0x%x (%pOF) (%s:%d)\n",
						irqnode->irq,
						irqnode->wakeup_src,
						tar_np,
						__func__, __LINE__);
				...
				pr_no_info("[name:mtk_lpm][P] - invalid irq, erro=%d (%s:%d)\n",
