...
int mtk_lpm_model_percpu_set(int cpu, struct mtk_lpm_module_reg *p)
{
	...
	if (!drv) {
		pr_no_info("[name:mtk_lpm][P] - cpuidle drv is null (%s:%d)\n",
					__func__, __LINE__);
		return -EINVAL;
	}
	...
	for (idx = 0; idx < drv->state_count; ++idx) {
		if (!IS_MTK_LPM_MODS_VALID(idx)) {
			pr_no_info("[name:mtk_lpm][P] - mod(%s) out of index (%d) (%s:%d)\n",
...
int mtk_lpm_module_register_blockcall(int cpu, void *p)
{
	...
	if (!reg || (reg->magic != MTK_LPM_MODULE_MAGIC)) {
		pr_no_info("[name:mtk_lpm][P] - registry(%d) fail (%s:%d)\n",
...
static int __init mtk_lpm_init(void)
{
	...
	if (mtk_lpm) {
		...
		if (pMethod) {
			...
			pr_no_info("[name:mtk_lpm][P] - suspend-method:%s (%s:%d)\n",
	...
	if (mtk_lpm_system.suspend.flag & MTK_LP_REQ_NOSUSPEND) {
		...
		if (!mtk_lpm_lock) {
			pr_no_info("[name:mtk_lpm][P] - initialize mtk_lpm_lock wakeup source fail\n");
			return -1;
		}
		__pm_stay_awake(mtk_lpm_lock);
		pr_no_info("[name:mtk_lpm][P] - device not support kernel suspend\n");
	}
