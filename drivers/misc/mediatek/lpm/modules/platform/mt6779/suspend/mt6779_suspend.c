...
int mt6779_suspend_prompt(int cpu, const struct mtk_lpm_issuer *issuer)
{
	...
	pr_no_info("[name:spm&][%s:%d] - prepare suspend enter\n",
			__func__, __LINE__);
	...
	pr_no_info("[name:spm&][%s:%d] - suspend enter\n",
			__func__, __LINE__);
...
{
	pr_no_info("[name:spm&][%s:%d] - prepare suspend resume\n",
			__func__, __LINE__);

	mt6779_suspend_common_resume(mt6779_suspend_status);
	mt6779_do_mcusys_prepare_on();

	pr_no_info("[name:spm&][%s:%d] - suspend resume\n",
			__func__, __LINE__);
