...
static int disp_probe_1(void)
{
	...
	pr_no_info("disp driver(1) disp_probe_1 begin\n");
#if (defined(CONFIG_TEE) || \
	defined(CONFIG_TRUSTONIC_TEE_SUPPORT)) && \
	defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
	...
	if (ret) {
		pr_no_info("disp: fail to create mtk_disp node\n");
	...
	pr_no_info("disp driver(1) disp_probe_1 end\n");
...
static int disp_probe(struct platform_device *pdev)
{
	...
	if (disp_probe_cnt != 0)
		return 0;
	pr_no_info("disp driver(1) disp_probe begin\n");
	...
	pr_no_info("disp driver(1) disp_probe end\n");
...
static int __init disp_late(void)
{
	...
	if (ret < 0)
		pr_no_info("get dsv_pos fail, ret = %d\n", ret);
