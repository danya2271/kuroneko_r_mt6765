...
#define DISPERR(string, args...)					\
	do {								\
		dprec_logger_pr(DPREC_LOGGER_ERROR, string, ##args);	\
		pr_no_info("[DISP][%s #%d]ERROR:"string,			\
				__func__, __LINE__, ##args);		\
	} while (0)
...
#define disp_aee_print(string, args...) do {	\
	...
	pr_no_info("DISP error: "string, ##args);				\
} while (0)
#else
#define disp_aee_print(string, args...) do {				\
	char disp_name[100];						\
...
#else
#define disp_aee_db_print(string, args...) pr_no_info("DISP error:"string, ##args)
#endif
