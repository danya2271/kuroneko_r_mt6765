...
static int mt_parse_dt(struct mt6370_chip *chip, struct device *dev)
{
	...
	if (!np) {
		pr_no_notice("%s find node type_c_port0 fail\n", __func__);
		return -ENODEV;
	}
	dev->of_node = np;
				...
				const struct i2c_device_id *id)
{
	...
	pr_no_info("mt6370_chipID = 0x%0x\n", chip_id);
	...
	if (ret < 0) {
		pr_no_err("mt6370 init alert fail\n");
		goto err_irq_init;
	}
	...
	if (client) {
		...
		if (chip) {
			...
			if (chip->tcpc->pd_wait_hard_reset_complete) {
				pr_no_info("%s WAITING HRESET(%d) - NO SUSPEND\n",
				    __func__,
				    chip->tcpc->pd_wait_hard_reset_complete);
				return -EAGAIN;
			}
			pr_no_info("%s WAIT HRESET DONE(%d) - SUSPEND\n",
				__func__,
				chip->tcpc->pd_wait_hard_reset_complete);
#endif
			down(&chip->suspend_lock);
