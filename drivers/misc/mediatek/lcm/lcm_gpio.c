...
static int _lcm_gpio_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(_lcm_gpio)) {
		...
		pr_no_info("[LCM][ERROR] Cannot find _lcm_gpio!\n");
		return ret;
	}
	...
	if (IS_ERR(_lcm_gpio_mode_default)) {
		...
		pr_no_info("[LCM][ERROR] Cannot find lcm_mode_default %d!\n",
			ret);
	}
	for (mode = LCM_GPIO_MODE_00; mode < MAX_LCM_GPIO_MODE; mode++) {
		...
		if (IS_ERR(_lcm_gpio_mode[mode]))
			pr_no_info("[LCM][ERROR] Cannot find lcm_mode:%d! skip it.\n",
			mode);
	...
	if (dev->of_node) {
		...
		if (!match) {
			pr_no_info("[LCM][ERROR] No device match found\n");
			return -ENODEV;
		}
	}
	GPIO_LCD_PWR_EN =
	...
	if (ret < 0)
		pr_no_info("[LCM][ERROR] Unable to request GPIO_LCD_PWR_EN\n");
	ret = gpio_request(GPIO_LCD_BL_EN, "lcm_bl_gpio");
	if (ret < 0)
		pr_no_info("[LCM][ERROR] Unable to request GPIO_LCD_BL_EN\n");
...
static int __init _lcm_gpio_init(void)
{
	...
	if (platform_driver_register(&_lcm_gpio_driver) != 0) {
		pr_no_info("unable to register LCM GPIO driver.\n");
		return -1;
	}
	return 0;
}
...
enum LCM_STATUS lcm_gpio_set_data(char type, const struct LCM_DATA_T1 *t1)
{
	...
	if (_lcm_gpio_check_data(type, t1) == LCM_STATUS_OK) {
		...
		pr_no_info("[LCM][ERROR] %s: 0x%x, 0x%x\n",
