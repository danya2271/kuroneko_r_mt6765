...
int setMaxbrightness(int max_level, int enable)
{
	...
	if (enable == 1) {
		...
		if (current_level != 0) {
			pr_no_info("Control temperature close:limit=%d\n",
				       limit);
...
#else
	pr_no_info("Set max brightness go through AAL\n");
			   ...
			   enum led_brightness level)
{
	...
	if (retval)
		pr_no_debug("Request I2C gpio149 failed\n");

	if (strcmp(led_data->cust.name, "lcd-backlight") == 0) {
		if (level == 0) {
			pr_no_debug("Level = %d, close the power\n", level);
			i2c_smbus_write_byte_data(client, 0x00, 0);
			gpio_direction_output(I2C_SET_FOR_BACKLIGHT, 0);
		}
		if (!last_level1 && level) {
			pr_no_debug("Level = %d, open the power\n", level);
			       ...
			       enum led_brightness level)
{
	...
	value = i2c_smbus_read_byte_data(g_client, 0x10);
	pr_no_debug("LEDS:mt65xx_led_set:0x10 = %d\n", value);
	...
	pr_no_debug("light type: #%d, level: %d\n", type, level);
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	if (!cust_led_list) {
		pr_no_info("Get dts fail! Probe exit.\n");
		ret = -1;
		goto err_dts;
	}
	...
	if (i2c_add_driver(&led_i2c_driver)) {
		pr_no_debug("Unable to add led-i2c driver.\n");
		return -1;
	}
	#endif
	pr_no_debug("Probe begain!\n");
	get_div_array();
...
static void mt65xx_leds_shutdown(struct platform_device *pdev)
{
	...
	pr_no_debug("Turn off backlight\n");
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		case MT65XX_LED_MODE_PMIC:
			pr_no_debug("not support set brightness through pmic!!1\n");
			break;
		case MT65XX_LED_MODE_CUST_LCM:
			pr_no_debug("Backlight control through LCM!!1\n");
#ifdef CONFIG_MTK_AAL_SUPPORT
		...
		case MT65XX_LED_MODE_CUST_BLS_PWM:
			pr_no_debug("Backlight control through BLS!!1\n");
#ifdef CONFIG_MTK_AAL_SUPPORT
			disp_aal_notify_backlight_changed(0);
#else
