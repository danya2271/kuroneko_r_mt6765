	...
	char __user *user_buf, size_t len, loff_t *offset)
{
	...
	if (emi_base == NULL) {
		pr_no_info("[DRAMC] can't find EMI base\n");
		return -1;
	}
	...
	unsigned int *bank, unsigned int *col, unsigned int *ch)
{
	...
	if (rank_max > 2) {
		pr_no_info("[DRAMC] invalid rank num (rank_max = %u)\n", rank_max);
		return -1;
	}
	...
	*ch = 0;
	...
	if ((ddr_type == TYPE_LPDDR4) || (ddr_type == TYPE_LPDDR4X))
		...
		pr_no_info("[DRAMC] undefined DRAM type\n");
		return -1;
	}
...
static phys_addr_t memtest_user_v2p(unsigned long va)
{
	...
	if (current == NULL) {
		pr_no_info("warning: memorytest_user_v2p, current is NULL!\n");
		return 0;
	}
	if (current->mm == NULL) {
		pr_no_info("warning: memorytest_user_v2p, current->mm is NULL!\n");
		pr_no_info("tgid=0x%x, name=%s\n", current->tgid, current->comm);
		return 0;
	}

	pgd = pgd_offset(current->mm, va);       /* what is tsk->mm */
	if (pgd_none(*pgd) || pgd_bad(*pgd)) {
		pr_no_info("memorytest_user_v2p, va=0x%lx, pgd invalid!\n", va);
		return 0;
	}

	pud = pud_offset(pgd, va);
	if (pud_none(*pud) || pud_bad(*pud)) {
		pr_no_info("memorytest_user_v2p, va=0x%lx, pud invalid!\n", va);
		return 0;
	}

	pmd = pmd_offset(pud, va);
	if (pmd_none(*pmd) || pmd_bad(*pmd)) {
		pr_no_info("(memorytest_user_v2p, va=0x%lx, pmd invalid!\n", va);
		return 0;
	}
	...
	pr_no_info("memorytest_user_v2p, va=0x%lx, pte invalid!\n", va);
	return 0;
}
...
static void mmap_mem_close(struct vm_area_struct *vma)
{
	...
	/*
	 *pr_no_info("%s(): 0x%lx~0x%lx pgoff=0x%lx %x\n", __func__,
...
static int test_mem_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	/* pr_no_info("%s(0): pgoff=0x%lx 0x%lx %lu\n", __func__, */
	...
	pfn = *addr >> PAGE_SHIFT;
	vma->vm_pgoff += pfn;
	/* pr_no_info("%s(1): pgoff=0x%lx 0x%lx %lu\n", __func__, */
	/* vma->vm_pgoff, size, (unsigned long) file->private_data); */
...
static int __init dram_memtest_interface_init(void)
{
	...
	if (get_emi_base == NULL) {
		pr_no_info("%s: mt_emi_base_get is NULL\n", __func__);
		return -1;
	}

	emi_base = get_emi_base();
	if (emi_base == NULL) {
		pr_no_info("%s: can't find EMI base\n", __func__);
		return -1;
	}

	memtest_dir = debugfs_create_dir("memtest", NULL);
	if (!memtest_dir) {
		pr_no_info("%s: create dir fail\n", __func__);
		return -1;
	}
	...
	if (!read_mr4) {
		pr_no_info("%s: create read_mr4 interface fail\n", __func__);
		return -1;
	}
	...
	if (!read_mr5) {
		pr_no_info("%s: create read_mr5 interface fail\n", __func__);
		return -1;
	}
	...
	if (!read_dram_addr) {
		pr_no_info("%s: create read_dram_addr interface fail\n", __func__);
		return -1;
	}
	...
	if (!memtest_result) {
		pr_no_info("%s: create result interface fail\n", __func__);
		return -1;
	}
	...
	if (!memtest_v2p) {
		pr_no_info("%s: create v2p interface fail\n", __func__);
		return -1;
	}
	...
	if (memtest_rank0_addr) {
		...
		if (!memtest_mem0) {
			pr_no_info("%s: create mem0 interface fail\n", __func__);
			return -1;
		}
	}
	...
	if (memtest_rank1_addr) {
		...
		if (!memtest_mem1) {
			pr_no_info("%s: create mem1 interface fail\n", __func__);
			return -1;
		}
	}
