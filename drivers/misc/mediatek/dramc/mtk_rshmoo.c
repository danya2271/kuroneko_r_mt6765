...
void dram_rshmoo_timer_callback(unsigned long data)
{
	...
	if (res < 0) {
		pr_no_info("arch_reset, get wd api error %d\n", res);
		while (1)
			cpu_relax();
	} else {
		pr_no_info("exception reboot\n");
...
static int dram_rshmoo_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "rank",
			&rshmoo_test_rank);
	if (ret < 0) {
		pr_no_info("Fail to find rank information\n");
		return ret;
	}

	ret = of_property_read_u32(pdev->dev.of_node, "ch", &rshmoo_test_ch);
	if (ret < 0) {
		pr_no_info("Fail to find ch information\n");
		return ret;
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node, "byte",
			&rshmoo_test_byte);
	if (ret < 0) {
		pr_no_info("Fail to find byte information\n");
		return ret;
	}
	...
	if (ret < 0) {
		pr_no_info("Fail to find txvrange information\n");
		return ret;
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node, "txvref",
			&rshmoo_test_txvref);
	if (ret < 0) {
		pr_no_info("Fail to find txvref information\n");
		return ret;
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node, "delay",
			&rshmoo_test_delay);
	if (ret < 0) {
		pr_no_info("Fail to find delay information\n");
		return ret;
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node, "ongoing",
			&rshmoo_test_ongoing);
	if (ret < 0) {
		pr_no_info("Fail to find ongoing information\n");
		return ret;
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node, "done",
			&rshmoo_test_done);
	if (ret < 0) {
		pr_no_info("Fail to find done information\n");
		return ret;
	}
	...
	if (ret) {
		pr_no_info("fail to create the rshmoo_test_done sysfs files\n");
		return ret;
	}
	...
	if (ret) {
		pr_no_info("fail to create the rshmoo_test_info sysfs files\n");
		return ret;
	}
...
static int __init dram_rshmoo_init(void)
{
	...
	if (ret) {
		pr_no_info("[DRAMC] init fail, ret 0x%x\n", ret);
		return ret;
	}
