...
int acquire_dram_ctrl(void)
{
	...
	do {
		...
		cnt--;
		/* pr_no_info("[DRAMC] wait for SPM HW SEMAPHORE\n"); */
		udelay(1);
	} while (cnt > 0);

	if (cnt == 0) {
		spin_unlock_irqrestore(&dramc_lock, save_flags);
		pr_no_info("[DRAMC] can NOT get SPM HW SEMAPHORE!\n");
		return -1;
	}

	/* pr_no_info("[DRAMC] get SPM HW SEMAPHORE success!\n"); */
...
int release_dram_ctrl(void)
{
	...
	if ((readl(PDEF_SPM_AP_SEMAPHORE) & 0x1) == 0x0) {
		pr_no_err("[DRAMC] has NOT acquired SPM HW SEMAPHORE\n");
		/* BUG(); */
	}

	writel(0x1, PDEF_SPM_AP_SEMAPHORE);
	if ((readl(PDEF_SPM_AP_SEMAPHORE) & 0x1) == 0x1) {
		pr_no_err("[DRAMC] release SPM HW SEMAPHORE fail!\n");
		/* BUG(); */
	}
	/* pr_no_info("[DRAMC] release SPM HW SEMAPHORE success!\n"); */
	return 0;
}
