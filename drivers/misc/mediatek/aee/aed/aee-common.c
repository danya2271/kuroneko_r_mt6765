...
{
	if (g_aee_api) {
		pr_no_info("disable aee kernel api");
		g_aee_api = NULL;
	}
}
		...
		const int db_opt, const char *module, const char *msg, ...)
{
	...
	if (g_aee_api && g_aee_api->kernel_reportAPI)
		...
		pr_no_notice("AEE kernel exception: %s", msgbuf);
	va_end(args);
}
		...
		const int db_opt, const char *module, const char *msg, ...)
{
	...
	if (g_aee_api && g_aee_api->kernel_reportAPI) {
		...
		pr_no_notice("AEE kernel warning: %s", msgbuf);
	}
	va_end(args);
}
		...
		const int db_opt, const char *module, const char *msg, ...)
{
	...
	if (g_aee_api && g_aee_api->kernel_reportAPI)
		...
		pr_no_notice("AEE kernel reminding: %s", msgbuf);
	va_end(args);
}
