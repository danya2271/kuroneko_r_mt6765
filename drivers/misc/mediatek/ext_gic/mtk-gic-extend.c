...
void mt_irq_set_pending_for_sleep(unsigned int irq)
{
	...
	if (irq < 16) {
		pr_no_info("Fail to set a pending on interrupt %d\n", irq);
		return;
	}
...
u32 mt_irq_get_pol_hw(u32 hwirq)
{
	...
	if (hwirq < 32) {
		pr_no_info("Fail to set polarity of interrupt %d\n", hwirq);
		return 0;
	}
...
void mt_irq_unmask_for_sleep(unsigned int virq)
{
	...
	if (irq < 16) {
		pr_no_info("Fail to enable interrupt %d\n", irq);
		return;
	}
...
static void mt_irq_mask(struct irq_data *data)
{
	...
	if (irq < NR_GIC_SGI) {
		...
		if (irq != FIQ_DBG_SGI)
			pr_no_info("Fail to disable interrupt %d\n", irq);
		return;
	}
...
static void mt_irq_set_sens(unsigned int irq, unsigned int sens)
{
	...
	if (irq < (NR_GIC_SGI + NR_GIC_PPI)) {
		pr_no_info("Fail to set sensitivity of interrupt %d\n", irq);
		return;
	}
...
static void fiq_isr(struct fiq_glue_handler *h, void *regs, void *svc_sp)
{
	...
	for (i = 0; i < ARRAY_SIZE(irqs_to_fiq); i++) {
		if (irqs_to_fiq[i].irq == irq) {
			if (irqs_to_fiq[i].handler) {
				...
				pr_no_info("Unexpected interrupt %d received!\n",
				       irq);
			}
			break;
		}
	...
	if (i == ARRAY_SIZE(irqs_to_fiq))
		pr_no_info("Interrupt %d triggers FIQ but it is not registered\n",
		       irq);
...
static int __init_fiq(void)
{
	...
	if (ret)
		pr_no_info("fail to register fiq_glue_handler\n");
	else
		fiq_glued = 1;
...
static int __init mtk_gic_ext_init(void)
{
	...
	if (!node) {
		...
		if (!node) {
			pr_no_info("[gic_ext] find arm,gic node failed\n");
			return -EINVAL;
		}
	}
	...
	if (!node) {
		pr_no_info("[gic_ext] find mediatek,mt6577-sysirq node failed\n");
		return -EINVAL;
	}
