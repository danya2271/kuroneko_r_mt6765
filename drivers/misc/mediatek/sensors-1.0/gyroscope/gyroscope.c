...
static void gyro_work_func(struct work_struct *work)
{
	...
	if (cxt->gyro_data.get_temperature) {
		...
		if (err)
			pr_no_info("get gyro temperature fails!!\n");
		else
	...
	if (cxt->gyro_data.get_data != NULL)
		...
		pr_no_err("gyro driver not register data path\n");

	if (err) {
		pr_no_err("get gyro data fails!!\n");
		goto gyro_loop;
	} else {
		cxt->drv_data.x = x;
		cxt->drv_data.y = y;
...
static struct gyro_context *gyro_context_alloc_object(void)
{
	...
	pr_no_debug("%s start\n", __func__);
	if (!obj) {
		pr_no_err("Alloc gyro object error!\n");
		return NULL;
	}
...
static int gyro_enable_and_batch(void)
{
	...
	if (cxt->power == 1 && cxt->enable == 0) {
		...
		if (cxt->is_active_data == false &&
			...
			pr_no_debug("gyro stop polling done\n");
		}
		/* turn off the power */
		if (cxt->is_active_data == false &&
			...
			if (err) {
				pr_no_err("gyro turn off power err = %d\n",
					    err);
				return -1;
			}
			pr_no_debug("gyro turn off power done\n");
		}

		cxt->power = 0;
		cxt->delay_ns = -1;
		pr_no_debug("GYRO disable done\n");
		return 0;
	}
	/* power off -> power on */
	if (cxt->power == 0 && cxt->enable == 1) {
		pr_no_debug("GYRO enable\n");
		if (true == cxt->is_active_data ||
		    true == cxt->is_active_nodata) {
			err = cxt->gyro_ctl.enable_nodata(1);
			if (err) {
				pr_no_err("gyro turn on power err = %d\n",
					    err);
				return -1;
			}
			pr_no_debug("gyro turn on power done\n");
		}
		cxt->power = 1;
		pr_no_debug("GYRO enable done\n");
	}
	/* rate change */
	if (cxt->power == 1 && cxt->delay_ns >= 0) {
		pr_no_debug("GYRO set batch\n");
		...
		if (err) {
			pr_no_err("gyro set batch(ODR) err %d\n", err);
			return -1;
		}
		pr_no_debug("gyro set ODR, fifo latency done\n");
		...
		if (cxt->is_active_data == true &&
			...
			pr_no_debug("gyro set polling delay %d ms\n",
				 atomic_read(&cxt->delay));
		}
		pr_no_debug("GYRO batch done\n");
	}
	return 0;
}
#endif
					...
					const char *buf, size_t count)
{
	...
	pr_no_debug("gyro_store_enable nodata buf=%s\n", buf);
	...
	if (!strncmp(buf, "1", 1)) {
		...
		pr_no_info(" gyro_store enable nodata cmd error !!\n");
		err = -1;
		goto err_out;
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	pr_no_debug("gyro show active not support now\n");
	div = cxt->gyro_data.vender_div;
	pr_no_debug("gyro vender_div value: %d\n", div);
				...
				size_t count)
{
	...
	err = sscanf(buf, "%d,%d,%lld,%lld", &handle, &flag, &cxt->delay_ns,
		     &cxt->latency_ns);
	if (err != 4) {
		pr_no_info("%s param error: err = %d\n", __func__, err);
		return -1;
	}
				...
				size_t count)
{
	...
	if (err < 0)
		pr_no_info("gyro enable flush err %d\n", err);
	mutex_unlock(&gyro_context_obj->gyro_op_mutex);
	if (err)
		return err;
	else
			       ...
			       size_t count)
{
	...
	if (cxt->gyro_ctl.set_cali != NULL)
		...
		pr_no_info(
			"GYRO DRIVER OLD ARCHITECTURE DON'T SUPPORT GYRO COMMON VERSION FLUSH\n");
	if (err < 0)
		pr_no_info("gyro set cali err %d\n", err);
	mutex_unlock(&gyro_context_obj->gyro_op_mutex);
	vfree(cali_buf);
	return count;
}
...
static int gyro_real_driver_init(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_CHOOSE_GYRO_NUM; i++) {
		...
		if (gyroscope_init_list[i] != 0) {
			pr_no_debug("gyro try to init driver %s\n",
				 gyroscope_init_list[i]->name);
			err = gyroscope_init_list[i]->init(pdev);
			if (err == 0) {
				pr_no_debug("gyro real driver %s probe ok\n",
					 gyroscope_init_list[i]->name);
				break;
			}
		}
...
int gyro_driver_add(struct gyro_init_info *obj)
{
	...
	if (!obj) {
		pr_no_err("%s fail, gyro_init_info is NULL\n", __func__);
		return -1;
	}

	for (i = 0; i < MAX_CHOOSE_GYRO_NUM; i++) {
		if ((i == 0) && (gyroscope_init_list[0] == NULL)) {
			pr_no_debug("register gyro driver for the first time\n");
...
static int gyro_misc_init(struct gyro_context *cxt)
{
	...
	if (err)
		pr_no_err("unable to register gyro misc device!!\n");
...
int gyro_register_data_path(struct gyro_data_path *data)
{
	...
	pr_no_debug("gyro register data path vender_div: %d\n",
		 cxt->gyro_data.vender_div);
	if (cxt->gyro_data.get_data == NULL) {
		pr_no_debug("gyro register data path fail\n");
		return -1;
	}
	if (cxt->gyro_data.get_temperature == NULL)
		pr_no_debug("gyro not register temperature path\n");
	return 0;
}
...
int gyro_register_control_path(struct gyro_control_path *ctl)
{
	...
	if (cxt->gyro_ctl.batch == NULL ||
		...
		pr_no_debug("gyro register control path fail\n");
		return -1;
	}
	...
	if (err) {
		pr_no_info("unable to register gyro misc device!!\n");
		return -2;
	}
	err = sysfs_create_group(&gyro_context_obj->mdev.this_device->kobj,
				 &gyro_attribute_group);
	if (err < 0) {
		pr_no_info("unable to create gyro attribute file\n");
		return -3;
	}
...
static int check_repeat_data(int x, int y, int z)
{
	...
	if (pc > 100) {
		pr_no_info("Gyro sensor output repeat data\n");
		pc = 0;
	}
...
static int gyro_probe(void)
{
	...
	if (err) {
		pr_no_err("gyro real driver init fail\n");
...
static int __init gyro_init(void)
{
	...
	if (gyro_probe()) {
		pr_no_err("failed to register gyro driver\n");
		return -ENODEV;
	}
