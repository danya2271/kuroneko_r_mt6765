	...
	if ((*head) == target) {
		*head = target->next;
		...
		if (temp->next == target) {
			...
			pr_no_info("VCODEC free history %p not found",
				target->handle);
			return -1;
		}
	}
...
#if SHOW_ALGO_INFO
	pr_no_info("%s first_idx %d(%lld) prev_idx %d(%lld)",
		...
		struct codec_job *job, struct codec_history *head)
{
...
#if SHOW_ALGO_INFO
	pr_no_info("%s find_hist %p handle %p\n", __func__, hist,
	...
	if (hist == 0) {
		...
		*t_us = now_us;
#if SHOW_ALGO_INFO
		pr_no_info("%s not history yet, full speed\n", __func__);
#endif
	} else {
...
#if SHOW_ALGO_INFO
		pr_no_info("%s deadline %llu, kcy %d\n", __func__, deadline, *kcy);
#endif
	}

#if SHOW_ALGO_INFO
	pr_no_info("%s now_us %lld, target_us %lld, min_mhz %d\n",
...
int update_hist_item(struct codec_job *job, struct codec_history *hist)
{
	...
	if (job->handle != hist->handle) {
		pr_no_info("VCODEC dvfs job - history mismatch\n");
		return -1;
	}
...
#if SHOW_ALGO_INFO
		pr_no_info("%s %p, gap (%lld), reset hist\n",
...
#if SHOW_ALGO_INFO
		pr_no_info("%s 2 kcy %d, time %llu, cnt %d\n",
...
#if SHOW_ALGO_INFO
	pr_no_info("%s %p, mhz %d, sub %lld, start %lld, end %lld\n",
...
#if SHOW_ALGO_INFO
		pr_no_info("%s new history %p head %p\n", __func__, target, *head);
#endif
	}
...
#if SHOW_ALGO_INFO
		pr_no_info("VCODEC dvfs start over for handle %p", job->handle);
#endif
	}
...
int add_job_(struct codec_job *job, struct codec_job **head)
{
	...
	while (last_job->next != 0) {
		if (last_job->handle == job->handle) {
			pr_no_info("VCODEC dvfs multiple jobs from same instance");
			return -1;
		}
...
int est_freq(void *handle, struct codec_job **job, struct codec_history *head)
{
	...
	if (target_job == 0) {
		pr_no_info("%s job not found!\n", __func__);
		return DEFAULT_MHZ;
	}

	if (target_job != *job)
		pr_no_info("%s target_job != job queue head\n", __func__);

	est_res = est_next_job(cur_time, &end_time, &kcy, &min_mhz, target_job,
				head);

#if SHOW_ALGO_INFO
	pr_no_info("%s res %d, min_mhz %d\n", __func__, est_res, min_mhz);
#endif
