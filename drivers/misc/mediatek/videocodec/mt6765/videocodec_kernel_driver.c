...
void vdec_polling_status(void)
{
	...
	while ((u4CgStatus != 0) && (u4DataStatusMain & (1<<15)) &&
		...
		if (u4Counter++ > DRAM_DONE_POLLING_LIMIT) {
			...
			pr_no_notice("[ERROR] Leftover data before power down");
			for (i = 45; i < 72; i++) {
				if (i == 45 || i == 46 || i == 52 || i == 58 ||
					i == 59 || i == 61 || i == 62 ||
					i == 63 || i == 71){
...
void vdec_power_on(void)
{
	...
	if (ret) {
		...
		pr_no_info("[VDEC] MT_SCP_SYS_DISP is not on, ret = %d\n",
				ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VCODEC] MT_CG_SMI_COMM0 is not on, ret = %d\n",
				ret);
		}
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VCODEC] MT_CG_SMI_COMM1 is not on, ret = %d\n",
				ret);
		}
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VCODEC] MT_CG_SMI_COMMON is not on, ret = %d\n",
				ret);
		}
#endif
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VCODEC] MT_SCP_SYS_VDE is not on, ret = %d\n", ret);
	}

	ret = clk_prepare_enable(clk_MT_CG_VDEC);
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VCODEC] MT_CG_VDEC is not on, ret = %d\n", ret);
	}
				...
				const char *buf, size_t count)
{
	if (sscanf(buf, "%du", &vcodecDebugMode) == 1) {
		...
		pr_no_debug("[VCODEC][%s] Input is stored\n", __func__);
	}
	return count;
}
...
void vdec_power_off(void)
{
	...
	if (gu4VdecPWRCounter == 0) {
		pr_no_debug("[VCODEC] gu4VdecPWRCounter = 0\n");
	} else {
		vdec_polling_status();
		/* VCODEC_SEL reset */
		do {
	...
	/* pr_no_debug("[PMQoS] vdec_power_off reset to 0"); */
...
void venc_power_on(void)
{
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VENC] MT_SCP_SYS_DISP is not on, ret = %d\n",
				ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VENC] MT_CG_SMI_COMM0 is not on, ret = %d\n",
				ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VENC] MT_CG_SMI_COMM1 is not on, ret = %d\n",
				ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VENC] MT_CG_SMI_COMMON is not on, ret = %d\n",
				ret);
	}
#endif
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VENC] MT_SCP_SYS_VEN is not on, ret = %d\n",
				ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_no_info("[VENC] MT_CG_VENC is not on, ret = %d\n",
				ret);
	}
#if defined(CONFIG_MACH_MT6761)
	/* GF14 type SRAM  power on config */
...
void venc_power_off(void)
{
	...
	if (gu4VencPWRCounter == 0) {
		pr_no_debug("[VENC] gu4VencPWRCounter = 0\n");
	} else {
		gu4VencPWRCounter--;
	...
	/* pr_no_debug("[PMQoS] venc_power_off reset to 0"); */
...
void vdec_break(void)
{
	...
	if (i >= 5000) {
		...
		pr_no_info("Leftover data access before powering down\n");

		for (j = 68; j < 80; j++) {
			u4DataStatus = VDO_HW_READ(KVA_VDEC_MISC_BASE+(j*4));
			pr_no_info("[VCODEC][DUMP] MISC_%d = 0x%08x",
				j, u4DataStatus);
		}
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(45*4));
		pr_no_info("[VCODEC][DUMP] VLD_45 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(46*4));
		pr_no_info("[VCODEC][DUMP] VLD_46 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(52*4));
		pr_no_info("[VCODEC][DUMP] VLD_52 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(58*4));
		pr_no_info("[VCODEC][DUMP] VLD_58 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(59*4));
		pr_no_info("[VCODEC][DUMP] VLD_59 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(61*4));
		pr_no_info("[VCODEC][DUMP] VLD_61 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(62*4));
		pr_no_info("[VCODEC][DUMP] VLD_62 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(63*4));
		pr_no_info("[VCODEC][DUMP] VLD_63 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_BASE+(71*4));
		pr_no_info("[VCODEC][DUMP] VLD_71 = 0x%08x", u4DataStatus);
		u4DataStatus = VDO_HW_READ(KVA_VDEC_MISC_BASE+(66*4));
		pr_no_info("[VCODEC][DUMP] MISC_66 = 0x%08x", u4DataStatus);
	}
...
void dec_isr(void)
{
	...
	if ((u4DecDoneStatus & (0x1 << 16)) != 0x10000) {
		pr_no_info("[VDEC] DEC ISR, Dec done is not 0x1 (0x%08x)",
				u4DecDoneStatus);
		return;
	}
	...
	if (u4TempDecISRCount != u4TempLockDecHWCount) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES:
		 * pr_no_info("[INFO] Dec ISRCount: 0x%x, "
		 * "LockHWCount:0x%x\n",
	...
	if (eValRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VDEC] ISR set DecIsrEvent error\n");
	}
...
void enc_isr(void)
{
	...
	if (u4TempEncISRCount != u4TempLockEncHWCount) {
		/* Add one line comment for avoid kernel coding style,
		 * WARNING:BRACES:
		 * pr_no_info("[INFO] Enc ISRCount: 0x%xa, "
		 *	"LockHWCount:0x%x\n",
	...
	if (CodecHWLock.pvHandle == 0) {
		pr_no_info("[VENC] NO one Lock Enc HW, please check!!\n");
	...
	if (CodecHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {/* hardwire */
		...
		pr_no_info("[VENC] Invalid lock holder driver type = %d\n",
	...
	if (eValRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VENC] ISR set EncIsrEvent error\n");
	}
}
...
{
	pr_no_info("VCODEC_LOCKHW, HWLockEvent TimeOut, CurrentTID = %d\n",
			current->pid);
	if (FirstUseDecHW != 1) {
		mutex_lock(&HWLock);
		if (CodecHWLock.pvHandle == 0) {
			 ...
			 * WARNING:BRACES:
			 */
			pr_no_info("VCODEC_LOCKHW, mediaserver may restarted");
		} else {
			/* Add one line comment for avoid kernel coding style,
			 * WARNING:BRACES:
			 */
			pr_no_info("VCODEC_LOCKHW, someone use HW");
		}
		mutex_unlock(&HWLock);
	}
...
{
	pr_no_info("VENC_LOCKHW HWLockEvent TimeOut, CurrentTID = %d\n",
			current->pid);

	if (FirstUseEncHW != 1) {
		mutex_lock(&HWLock);
		if (CodecHWLock.pvHandle == 0) {
			pr_no_info("VENC_LOCKHW, mediaserver may restarted");
		} else {
			pr_no_info("VENC_LOCKHW, someone use HW (%d)\n",
				 gLockTimeOutCount);
			++gLockTimeOutCount;

			if (gLockTimeOutCount > 30) {
				pr_no_info("VENC_LOCKHW - ID %d fail\n",
						current->pid);
				pr_no_info(ENC_LOCK_FLOG,
...
static long vcodec_lockhw_vdec(struct VAL_HW_LOCK_T *pHWLock, char *bLockedHW)
{
	...
	/* pr_no_debug("cur_job's handle %p", cur_job->handle); */
	...
	while ((*bLockedHW) == VAL_FALSE) {
		...
		if (HWLockEvent.u4TimeoutMs == 1) {
			pr_no_info("VDEC_LOCKHW, First Use Dec HW!!\n");
			FirstUseDecHW = 1;
		} else {
			FirstUseDecHW = 0;
		}
		...
		if (CodecHWLock.pvHandle ==
			...
			pr_no_info("[VDEC] Same inst double lock\n");
			pr_no_info("may timeout!! inst = 0x%lx, TID = %d\n",
				(unsigned long)CodecHWLock.pvHandle,
				current->pid);
		}
		mutex_unlock(&HWLock);
		...
		if (FirstUseDecHW == 0) {
			/*
			 * pr_no_debug("[VDEC] Not first time use, timeout = %d\n",
		...
		if (eValRet == VAL_RESULT_INVALID_ISR) {
			...
			if (ret) {
				pr_no_info("[VDEC] lockhw_dec_fail: %lu\n",
						ret);
				return -EFAULT;
			}
		} else if (eValRet == VAL_RESULT_RESTARTSYS) {
			pr_no_info("[VDEC] ERESTARTSYS when LockHW\n");
			return -ERESTARTSYS;
		}
		...
		if (CodecHWLock.pvHandle == 0) {
			...
			while (is_entering_suspend == 1) {
				...
				if (suspend_block_cnt > 100000) {
					 ...
					 * for too long
					 */
					pr_no_info("VDEC blocked by suspend");
					suspend_block_cnt = 0;
				}
			...
			/*
			 * pr_no_debug("VDEC_LOCKHW, free to use HW\n");
			 * pr_no_debug("Inst=0x%lx TID=%d, Time(s, us)=%d, %d",
			 *		handle,
			 *		current->pid,
			...
			*bLockedHW = VAL_TRUE;
			...
			/* pr_no_info("VDEC cur_job freq %u, %llu, %d",
			...
			u4TimeInterval = (((((rCurTime.u4Sec -
					...
					1000);
			/*
			 * pr_no_debug("VDEC_LOCKHW, someone use HW\n");
			 * pr_no_debug("Time(ms) = %d, TimeOut(ms)) = %d\n",
			 *		u4TimeInterval, pHWLock->u4TimeoutMs);
			 * pr_no_debug("Lock Inst=0x%lx, TID=%d, CurrTID=%d\n",
			 *	(unsigned long)CodecHWLock.pvHandle,
			 *	gu4VdecLockThreadId,
			 *	current->pid);
			 * pr_no_debug("rLockedTime(%ds,%dus), rCurTime(%ds,%dus)",
			 *		CodecHWLock.rLockedTime.u4Sec,
			 *		CodecHWLock.rLockedTime.u4uSec,
...
static long vcodec_lockhw_venc(struct VAL_HW_LOCK_T *pHWLock, char *bLockedHW)
{
	...
	while ((*bLockedHW) == VAL_FALSE) {
		...
		mutex_lock(&HWLockEventTimeoutLock);
		if (HWLockEvent.u4TimeoutMs == 1) {
			pr_no_info("VENC_LOCKHW, First Use Enc HW %d!!\n",
					pHWLock->eDriverType);
			FirstUseEncHW = 1;
		} else {
			FirstUseEncHW = 0;
		...
		if (CodecHWLock.pvHandle ==
			(void *)pmem_user_v2p_video(handle)) {
			pr_no_info("VENC_LOCKHW, Some inst double lock");
			pr_no_info("may timeout inst=0x%lx, TID=%d, type:%d\n",
				(unsigned long)CodecHWLock.pvHandle,
		...
		if (CodecHWLock.pvHandle == 0) {
			...
			while (is_entering_suspend == 1) {
				...
				if (suspend_block_cnt > 100000) {
					 ...
					 */
					pr_no_info("VENC blocked by suspend");
					suspend_block_cnt = 0;
				}
				usleep_range(1000, 2000);
			}
			...
			eVideoGetTimeOfDay(&CodecHWLock.rLockedTime,
					sizeof(struct VAL_TIME_T));
			/*
			 * pr_no_debug("VENC_LOCKHW, free to use HW\n");
			 * pr_no_debug("VENC_LOCKHW, handle = 0x%lx\n",
			 *	(unsigned long)CodecHWLock.pvHandle);
			 * pr_no_debug("Inst=0x%lx TID=%d, Time(%ds, %dus)",
			 *	(unsigned long)CodecHWLock.pvHandle,
			 *	current->pid,
			 *	CodecHWLock.rLockedTime.u4Sec,
			 *	CodecHWLock.rLockedTime.u4uSec);
			...
			/* pr_no_info("VENC cur_job freq %u, %llu, %d",
			...
			*bLockedHW = VAL_TRUE;
			...
			/*
			 * pr_no_debug("VENC_LOCKHW, someone use enc HW");
			 * pr_no_debug("Time(ms) = %d, TimeOut(ms) = %d\n",
			 *	 u4TimeInterval, pHWLock->u4TimeoutMs);
			 * pr_no_debug("rLockedTime(%ds,%dus),rCurTime(%ds,%dus)",
			 *	CodecHWLock.rLockedTime.u4Sec,
			 *	CodecHWLock.rLockedTime.u4uSec,
			 *	rCurTime.u4Sec, rCurTime.u4uSec);
			 * pr_no_debug("LockInst=0x%lx, Inst=0x%lx, TID=%d\n",
			 *	 (unsigned long)CodecHWLock.pvHandle,
			 *	 pmem_user_v2p_video(handle),
			 *	 current->pid);
			 */
			++gLockTimeOutCount;
			pr_no_info("VENC_LOCKHW Ret %d Count %d First %d Lock %d ,Try %d Timeout %d",
				eValRet,
				gLockTimeOutCount,
				FirstUseEncHW,
				CodecHWLock.eDriverType,
				...
				HWLockEvent.u4TimeoutMs);
			if (gLockTimeOutCount > 30) {
				pr_no_info("VENC_LOCKHW %d fail 30 times",
						current->pid);
				pr_no_info("no TO 0x%lx,%lx,0x%lx,type:%d\n",
				(unsigned long)CodecHWLock.pvHandle,
				pmem_user_v2p_video(handle),
				handle,
				pHWLock->eDriverType);
		...
		if (*bLockedHW == VAL_TRUE) {
			/*
			 * pr_no_debug("VENC_LOCKHW handle:0x%lx,va:%lx,type:%d",
			 *	(unsigned long)CodecHWLock.pvHandle,
...
static long vcodec_lockhw(unsigned long arg)
{
	...
	/* pr_no_debug("VCODEC_LOCKHW + tid = %d\n", current->pid); */

	user_data_addr = (unsigned char *)arg;
	ret = copy_from_user(&rHWLock, user_data_addr, sizeof(struct
							VAL_HW_LOCK_T));
	if (ret) {
		pr_no_info("[VCODEC] LOCKHW copy_from_user failed: %lu\n",
				ret);
		return -EFAULT;
	}

	/* pr_no_debug("[VCODEC] LOCKHW eDriverType=%d\n", rHWLock.eDriverType); */
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		/*
		 * pr_no_debug("VCODEC_LOCKHW, get locked - ObjId =%d\n",
		 *		current->pid);
		 */

		/* pr_no_debug("VCODEC_LOCKHWed - tid = %d\n", current->pid); */
	} else {
		pr_no_info("[WARNING] VCODEC_LOCKHW Unknown instance type %d timeout %u\n",
			rHWLock.eDriverType,
			rHWLock.u4TimeoutMs);
		return -EFAULT;
	}
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		u4VcodecSel = 0x0;
		pr_no_info("[WARNING] Unknown driver type\n");
	}
	...
	if (VDO_HW_READ(KVA_VDEC_GCON_BASE + 0x20) == 0) {
		...
		pr_no_info("[WARNING] VCODEC_SEL is not 0\n");
	}
	if (u4VcodecSel == 0x2)
...
static long vcodec_unlockhw(unsigned long arg)
{
	...
	/* pr_no_debug("VCODEC_UNLOCKHW + tid = %d\n", current->pid); */

	user_data_addr = (unsigned char *)arg;
	ret = copy_from_user(&rHWLock, user_data_addr,
				sizeof(struct VAL_HW_LOCK_T));
	if (ret) {
		pr_no_info("[VCODEC] UNLOCKHW, copy_from_user failed: %lu\n",
				ret);
		return -EFAULT;
	}

	/* pr_no_debug("VCODEC_UNLOCKHW eDriverType = %d\n",
	 *		rHWLock.eDriverType);
	 */
	eValRet = VAL_RESULT_INVALID_ISR;
	handle = (unsigned long)rHWLock.pvHandle;
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		if (CodecHWLock.pvHandle ==
			...
			cur_job = codec_jobs;
			if (cur_job == 0) {
				pr_no_info("VDEC job is null when unlock, error");
				...
				pr_no_info("VCODEC wrong job at dec done %p, %p",
					cur_job->handle,
					CodecHWLock.pvHandle);
			}
			mutex_unlock(&VcodecDVFSLock);
			...
			pr_no_info("[ERROR] VDEC_UNLOCKHW\n");
			pr_no_info("VDEC Not owner trying to unlock 0x%lx\n",
					pmem_user_v2p_video(handle));
			mutex_unlock(&HWLock);
			return -EFAULT;
		}
		...
		if (CodecHWLock.pvHandle ==
			...
			if (rHWLock.eDriverType != VAL_DRIVER_TYPE_JPEG_ENC) {
				...
				cur_job = codec_jobs;
			if (cur_job == 0) {
				pr_no_info("VENC job is null when unlock, error");
			} else if (cur_job->handle ==
				CodecHWLock.pvHandle) {
				cur_job->end = get_time_us();
				update_hist(cur_job, &codec_hists);
				...
				pr_no_info("VCODEC wrong job at enc done %p, %p",
					cur_job->handle,
					CodecHWLock.pvHandle);
			}
			mutex_unlock(&VcodecDVFSLock);
			...
			/* [TODO] error handling */
			pr_no_info("[ERROR] VENC_UNLOCKHW\n");
			pr_no_info("VENC Not owner trying to unlock 0x%lx\n",
				pmem_user_v2p_video(handle));
			mutex_unlock(&HWLock);
			return -EFAULT;
			}
		...
		pr_no_info("[WARNING] VCODEC_UNLOCKHW Unknown instance\n");
		return -EFAULT;
	}
...
static long vcodec_waitisr(unsigned long arg)
{
	...
	/* pr_no_debug("VCODEC_WAITISR + tid = %d\n", current->pid); */

	user_data_addr = (unsigned char *)arg;
	ret = copy_from_user(&val_isr, user_data_addr,
				sizeof(struct VAL_ISR_T));
	if (ret) {
		pr_no_info("[VCODEC] WAITISR, copy_from_user failed: %lu\n",
				ret);
		return -EFAULT;
	}
	...
	if (val_isr.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		if (bLockedHW == VAL_FALSE) {
			pr_no_info("VDEC_WAITISR, DO NOT have HWLock, fail\n");
			return -EFAULT;
		}
		...
		if (eValRet == VAL_RESULT_INVALID_ISR) {
			...
			pr_no_info("VDEC_WAITISR, ERESTARTSYS\n");
			return -ERESTARTSYS;
		}
		...
		if (bLockedHW == VAL_FALSE) {
			pr_no_info("VENC_WAITISR, DO NOT have HWLock, fail\n");
			return -EFAULT;
		}
		...
		if (eValRet == VAL_RESULT_INVALID_ISR) {
			...
			pr_no_info("VENC_WAITISR, ERESTARTSYS\n");
			return -ERESTARTSYS;
		}
		...
		if (val_isr.u4IrqStatusNum > 0) {
			...
			ret = copy_to_user(user_data_addr, &val_isr,
					sizeof(struct VAL_ISR_T));
			if (ret) {
				pr_no_info("VENC_WAITISR, cp status fail: %lu",
						ret);
				return -EFAULT;
			}
		}
	} else {
		pr_no_info("[WARNING] VCODEC_WAITISR Unknown instance\n");
		return -EFAULT;
	}
...
static long vcodec_set_frame_info(unsigned long arg)
{
	...
	/* pr_no_debug("VCODEC_SET_FRAME_INFO + tid = %d\n", current->pid); */
	user_data_addr = (unsigned char *)arg;
	ret = copy_from_user(&rFrameInfo, user_data_addr,
				sizeof(struct VAL_FRAME_INFO_T));
	if (ret) {
		pr_no_debug("%s, copy_from_user failed: %lu\n",
			__func__, ret);
		return -EFAULT;
	}
 ...
 */
	if (rFrameInfo.driver_type == VAL_DRIVER_TYPE_H264_DEC ||
		...
		if (gVDECBWRequested == 0) {
			...
			default:
				emi_bw = 0;
				pr_no_debug("Unsupported decoder type for BW");
			}

			if (rFrameInfo.is_compressed != 0)
				emi_bw = emi_bw * 6 / 10;

			/* pr_no_debug("UFO %d, emi_bw %ld",
			...
			/* pr_no_debug("input_size %d, w %d, h %d emi_bw %ld",
			...
			/* pr_no_info("VDEC mbytes/s emi_bw %ld", emi_bw); */
		...
		if (gVENCBWRequested == 0) {
			...
			default:
				emi_bw = 0;
				pr_no_debug("Unsupported encoder type for BW");
			}
			...
			/* pr_no_info("VENC mbytes/s emi_bw %ld", emi_bw); */
 ...
 */
	...
	{
		pr_no_info("[INFO] VCODEC_SET_THREAD_ID empty");
	}
	break;

	case VCODEC_ALLOC_NON_CACHE_BUFFER:
	{
		pr_no_info("[INFO] VCODEC_ALLOC_NON_CACHE_BUFFER  empty");
	}
	break;

	case VCODEC_FREE_NON_CACHE_BUFFER:
	{
		pr_no_info("[INFO] VCODEC_FREE_NON_CACHE_BUFFER  empty");
	}
	break;

	case VCODEC_INC_DEC_EMI_USER:
	{
		/* pr_no_debug("VCODEC_INC_DEC_EMI_USER + tid = %d\n",
		 *		current->pid);
		 */

		mutex_lock(&DecEMILock);
		gu4DecEMICounter++;
		pr_no_debug("[VCODEC] DEC_EMI_USER = %d\n",
				gu4DecEMICounter);
		user_data_addr = (unsigned char *)arg;
		ret = copy_to_user(user_data_addr, &gu4DecEMICounter,
				sizeof(unsigned int));
		if (ret) {
			pr_no_info("INC_DEC_EMI_USER, copy_to_user fail: %lu",
					ret);
	...
	{
		/* pr_no_debug("VCODEC_DEC_DEC_EMI_USER + tid = %d\n",
		 *		current->pid);
		 */
		...
		ret = copy_to_user(user_data_addr, &gu4DecEMICounter,
				sizeof(unsigned int));
		if (ret) {
			pr_no_info("DEC_DEC_EMI_USER, copy_to_user fail: %lu",
					ret);
			mutex_unlock(&DecEMILock);
			return -EFAULT;
		}
	...
	{
		/* pr_no_debug("VCODEC_INC_ENC_EMI_USER + tid = %d\n",
		 *		current->pid);
		 */

		mutex_lock(&EncEMILock);
		gu4EncEMICounter++;
		pr_no_debug("[VCODEC] ENC_EMI_USER = %d\n",
				gu4EncEMICounter);
		user_data_addr = (unsigned char *)arg;
		ret = copy_to_user(user_data_addr, &gu4EncEMICounter,
				sizeof(unsigned int));
		if (ret) {
			pr_no_info("INC_ENC_EMI_USER, copy_to_user fail: %lu",
					ret);
	...
	{
		/* pr_no_debug("VCODEC_DEC_ENC_EMI_USER + tid = %d\n",
		 *		current->pid);
		 */

		mutex_lock(&EncEMILock);
		gu4EncEMICounter--;
		pr_no_info("[VCODEC] ENC_EMI_USER = %d\n",
				gu4EncEMICounter);
		user_data_addr = (unsigned char *)arg;
		ret = copy_to_user(user_data_addr, &gu4EncEMICounter,
					sizeof(unsigned int));
		if (ret) {
			pr_no_info("DEC_ENC_EMI_USER, copy_to_user fail: %lu",
					ret);
			mutex_unlock(&EncEMILock);
			return -EFAULT;
		}
	...
	case VCODEC_LOCKHW:
	{
		...
		if (ret) {
			pr_no_info("[ERROR] VCODEC_LOCKHW failed! %lu\n",
					ret);
			return ret;
		}
	}
	...
	case VCODEC_UNLOCKHW:
	{
		...
		if (ret) {
			pr_no_info("[ERROR] VCODEC_UNLOCKHW failed! %lu\n",
					ret);
			return ret;
		}
	}
	...
	{
		/* pr_no_info("[INFO] VCODEC_INC_PWR_USER empty"); */
	}
	break;

	case VCODEC_DEC_PWR_USER:
	{
		/* pr_no_info("[INFO] VCODEC_DEC_PWR_USER empty"); */
	}
	break;
	...
	case VCODEC_WAITISR:
	{
		...
		if (ret) {
			pr_no_info("[ERROR] VCODEC_WAITISR failed! %lu\n",
					ret);
			return ret;
		}
	}
	...
	{
		pr_no_info("[INFO] VCODEC_INITHWLOCK empty");
	}
	break;

	case VCODEC_DEINITHWLOCK:
	{
		pr_no_info("[INFO] VCODEC_DEINITHWLOCK empty");
	}
	break;

	case VCODEC_GET_CPU_LOADING_INFO:
	{
		pr_no_info("[INFO] VCODEC_GET_CPU_LOADING empty");
	}
	break;

	case VCODEC_GET_CORE_LOADING:
	{
		pr_no_info("[INFO] VCODEC_GET_CORE_LOADING empty");
	}
	break;

	case VCODEC_GET_CORE_NUMBER:
	{
		/* pr_no_debug("VCODEC_GET_CORE_NUMBER + - tid = %d\n",
		 *		current->pid);
		 */
		...
		ret = copy_to_user(user_data_addr, &temp_nr_cpu_ids,
				sizeof(int));
		if (ret) {
			pr_no_info("GET_CORE_NUMBER, copy_to_user failed: %lu",
					ret);
			return -EFAULT;
		}
		/* pr_no_debug("VCODEC_GET_CORE_NUMBER - - tid = %d\n",
	...
	{
		pr_no_info("[INFO] VCODEC_SET_CPU_OPP_LIMIT empty");
	}
	break;
	...
	{
		/* pr_no_debug("VCODEC_SET_LOG_COUNT + tid = %d\n",
		 *		current->pid);
		 */
		...
		ret = copy_from_user(&rIncLogCount, user_data_addr,
				sizeof(char));
		if (ret) {
			pr_no_info("SET_LOG_COUNT, copy_from_user failed: %lu",
					ret);
	...
	case VCODEC_SET_FRAME_INFO:
	{
		...
		if (ret) {
			pr_no_debug("[ERROR] VCODEC_SET_FRAME_INFO failed! %lu\n",
				ret);
			return ret;
		}
	}
	...
	default:
	{
		pr_no_info("[ERROR] vcodec_ioctl default case %u\n", cmd);
	}
	break;
...
{
	long ret = 0;
	/* pr_no_debug("vcodec_unlocked_compat_ioctl: 0x%x\n", cmd); */
	switch (cmd) {
	case VCODEC_ALLOC_NON_CACHE_BUFFER:
	case VCODEC_FREE_NON_CACHE_BUFFER:
	{
...
static int vcodec_open(struct inode *inode, struct file *file)
{
	...
	pr_no_info("%s pid = %d, Driver_Open_Count %d\n",
...
{
	pr_no_debug("%s, curr_tid =%d\n", __func__, current->pid);
	pr_no_debug("%s pid = %d, Driver_Open_Count %d\n",
			__func__, current->pid, Driver_Open_Count);
...
static int vcodec_release(struct inode *inode, struct file *file)
{
	...
	if (Driver_Open_Count == 0) {
		...
		if (CodecHWLock.pvHandle != 0) {
			pr_no_info("err %s %d, type = %d, 0x%lx\n",
				__func__, current->pid,
				CodecHWLock.eDriverType,
				(unsigned long)CodecHWLock.pvHandle);
			pr_no_info("err VCODEC_SEL 0x%x\n",
...
static int vcodec_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (((length > VENC_REGION) || (pfn < VENC_BASE) ||
		...
		for (u4I = 0; u4I < VCODEC_INST_NUM_x_10; u4I++) {
			if ((ncache_mem_list[u4I].ulKVA != -1L) &&
				...
				if ((length == ulSize) && (pfn == ulAddr)) {
					pr_no_debug("[VCODEC] cache idx %d\n",
							u4I);
					break;
				}
			}
		...
		if (u4I == VCODEC_INST_NUM_x_10) {
			pr_no_info("mmap region error: Len(0x%lx),pfn(0x%lx)",
	...
	pr_no_debug("mmap vma->start 0x%lx, vma->end 0x%lx, vma->pgoff 0x%lx\n",
...
{
	if (CodecHWLock.pvHandle != 0) {
		pr_no_info("%s fail due to videocodec activity", __func__);
		return -EBUSY;
	}
	pr_no_debug("%s ok", __func__);
	return 0;
}

static int vcodec_resume(struct platform_device *pDev)
{
	pr_no_info("%s ok", __func__);
	return 0;
}
					...
					unsigned long action, void *data)
{
	...
	pr_no_info("%s ok action = %ld", __func__, action);
	switch (action) {
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (ret) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC] Can't Get Major number\n");
	}
	...
	if (request_irq(VDEC_IRQ_ID, (irq_handler_t)video_intr_dlr,
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC][ERROR] error to request dec irq\n");
	} else {
		pr_no_debug("[VCODEC] success to request dec irq: %d\n",
				VDEC_IRQ_ID);
	}
	...
	if (request_irq(VENC_IRQ_ID, (irq_handler_t)video_intr_dlr2,
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_debug("[VCODEC][ERROR] error to request enc irq\n");
	} else {
		pr_no_debug("[VCODEC] success to request enc irq: %d\n",
				VENC_IRQ_ID);
	}
	...
	if (IS_ERR(clk_MT_CG_SMI_COMMON)) {
		pr_no_info("[VCODEC] Unable to get MT_CG_MM_SMI_COMM0");
	...
	if (IS_ERR(clk_MT_CG_SMI_COMM1)) {
		pr_no_info("[VCODEC] Unable to get MT_CG_SMI_COMM1");
		return PTR_ERR(clk_MT_CG_SMI_COMM1);
	}

	clk_MT_CG_SMI_COMMON = devm_clk_get(&dev->dev, "MT_CG_MM_SMI_COMMON");
	if (IS_ERR(clk_MT_CG_SMI_COMMON)) {
		pr_no_info("[VCODEC] Unable to get MT_CG_MM_SMI_COMMON");
	...
	ret = mmdvfs_qos_get_freq_steps(PM_QOS_VDEC_FREQ,
					&g_dec_freq_steps[0], &dec_step_size);
	if (ret < 0)
		pr_no_debug("Vdec get DVFS freq steps failed: %d\n", ret);
	...
	ret = mmdvfs_qos_get_freq_steps(PM_QOS_VENC_FREQ,
					&g_enc_freq_steps[0], &enc_step_size);
	if (ret < 0)
		pr_no_debug("Venc get DVFS freq steps failed: %d\n", ret);
...
static int __init vcodec_driver_init(void)
{
...
#if defined(CONFIG_MACH_MT6761)
		pr_no_debug("[VCODEC] VENC(0x%p), VDEC(0x%p), GCON(0x%p), MBIST(0x%p)",
			KVA_VENC_BASE, KVA_VDEC_BASE,
			KVA_VDEC_GCON_BASE, KVA_MBIST_BASE);
#elif defined(CONFIG_MACH_MT6765)
		pr_no_debug("[VCODEC] VENC(0x%p), VDEC(0x%p), VDEC_GCON(0x%p)",
			KVA_VENC_BASE, KVA_VDEC_BASE, KVA_VDEC_GCON_BASE);
#endif
		pr_no_debug("[VCODEC] VDEC_IRQ_ID(%d), VENC_IRQ_ID(%d)",
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC][ERROR] create vcodec hwlock event error\n");
	}
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC][ERROR] create dec isr event error\n");
	}
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC][ERROR] create enc isr event error\n");
	}
#if USE_WAKELOCK == 0
	is_entering_suspend = 0;
#endif
	...
	if (!vcodec_debug_kobject) {
		pr_no_debug("Faile to create and add vcodec kobject");
		return -ENOMEM;
	}
	...
	if (error) {
		pr_no_debug("Faile to create/add debug file in /sys/vcodec/");
		return error;
	}
#endif
 ...
 */
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC][ERROR] close dec hwlock event error\n");
	}
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC][ERROR] close enc hwlock event error\n");
	}
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC][ERROR] close dec isr event error\n");
	}
	...
	if (eValHWLockRet != VAL_RESULT_NO_ERROR) {
		 ...
		 * WARNING:BRACES:
		 */
		pr_no_info("[VCODEC][ERROR] close enc isr event error\n");
	}
