...
finished:
	if (rd->props.io_log_en) {
		...
		pr_no_info("RT_REGMAP [WRITE] reg0x%04x  [Data] 0x%s\n",
							reg, wri_data);
	}
	return 0;
ERR:
			...
			int bytes, void *dest)
{
	...
	if (rd->props.io_log_en)
		pr_no_info("RT_REGMAP [READ] reg0x%04x\n", reg);
			     ...
			     size_t count, loff_t *ppos)
{
	...
	case RT_DBG_REG:
		...
		down(&rd->semaphore);
		if (rio.index < 0) {
			pr_no_info("this is an invalid or hiden register\n");
...
static int rt_regmap_check(struct rt_regmap_device *rd)
{
	...
	/* check name property */
	if (!rd->props.name) {
		pr_no_info("there is no node name for rt-regmap\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < num; i++) {
		...
		if (rm[i]->size < 1 || rm[i]->size > 24) {
			pr_no_info("rt register size error at reg 0x%02x\n",
				rm[i]->addr);
			return -EINVAL;
		}
	}
	...
	for (i = 0; i < num - 1; i++) {
		...
		if (rm[i]->addr >= rm[i + 1]->addr) {
			pr_no_info("sequence format error at reg 0x%02x\n",
				rm[i]->addr);
			return -EINVAL;
		}
	}
			...
			void *client, int slv_addr, void *drvdata)
{
	...
	if (!props) {
		pr_no_err("%s rt_regmap_properties is NULL\n", __func__);
		return NULL;
	}
	if (!rops) {
		pr_no_err("%s rt_regmap_fops is NULL\n", __func__);
		return NULL;
	}

	pr_no_info("regmap_device_register: name = %s\n", props->name);
	rd = devm_kzalloc(parent, sizeof(struct rt_regmap_device), GFP_KERNEL);
	if (!rd) {
		pr_no_info("rt_regmap_device memory allocate fail\n");
		return NULL;
	}
	...
	if (ret) {
		pr_no_info("rt register map format error\n");
	...
	if (ret) {
		pr_no_info("rt-regmap dev register fail\n");
		devm_kfree(parent, rd);
		return NULL;
	}
	...
	if (ret < 0) {
		pr_no_info(" rt cache data init fail\n");
		goto err_cacheinit;
	}
...
{
	pr_no_info("Init Richtek RegMap %s\n", RT_REGMAP_VERSION);
#ifdef CONFIG_DEBUG_FS
	rt_regmap_dir = debugfs_create_dir("rt-regmap", 0);
	if (IS_ERR(rt_regmap_dir)) {
		pr_no_err("rt-regmap debugfs node create fail\n");
		return -EINVAL;
	}
#endif /* CONFIG_DEBUG_FS */
	return 0;
