...
static int dt_parse_atf_logger_buf(void)
{
	...
	if (!atf_logger_rmem_np) {
		pr_no_info("No atf_logger reserved memory\n");
		return -EINVAL;
	}
	...
	pr_no_notice("%s, start: %pa, size: %pa\n",
		ATF_LOG_RESERVED_MEMORY_KEY,
...
#ifdef ATF_LOGGER_DEBUG
	pr_no_notice("[%s]param:0x%lx, count:%zu, ret:%ld\n",
...
start:
	while (1) {
		/* pr_no_notice("atf_log_read: wait in wq\n"); */
...
{
	pr_no_notice("ATF reserved memory, start: %pa, size: %pa\n",
				&atf_buf_phy_ctrl, &atf_buf_len);
	pr_no_notice("atf_buf_phy_ctrl: %pa\n", &atf_buf_phy_ctrl);
	pr_no_notice("atf_buf_len: %pa\n", &atf_buf_len);
	pr_no_notice("atf_buf_vir_ctrl: %pa\n", &atf_buf_vir_ctrl);
	pr_no_notice("atf_log_vir_addr: %pa\n", &atf_log_vir_addr);
	pr_no_notice("atf_log_len: %u\n", atf_log_len);
	pr_no_notice("atf_log_addr(%pa) = 0x%x\n",
			&(atf_buf_vir_ctrl->info.atf_log_addr),
			atf_buf_vir_ctrl->info.atf_log_addr);
	pr_no_notice("atf_log_size(%pa) = 0x%x\n",
			&(atf_buf_vir_ctrl->info.atf_log_size),
			atf_buf_vir_ctrl->info.atf_log_size);
	pr_no_notice("atf_write_offset(%pa) = %u\n",
			&(atf_buf_vir_ctrl->info.atf_write_offset),
			atf_buf_vir_ctrl->info.atf_write_offset);
	pr_no_notice("atf_read_offset(%pa) = %u\n",
			&(atf_buf_vir_ctrl->info.atf_read_offset),
			atf_buf_vir_ctrl->info.atf_read_offset);
}
	...
	uint64_t nbytes, const char *name)
{
	...
	for (i = 0; i < nlines; i++) {
		 ...
		 */
		pr_no_notice("%#lx ", ((uintptr_t)p) & 0xffff);
		for (j = 0; j < 8; j++) {
			u32	data;
...
static int __init atf_logger_probe(struct platform_device *pdev)
{
	...
	pr_no_notice("atf_log: inited");
	err = dt_parse_atf_logger_buf();
	if (unlikely(err)) {
		pr_no_info("No atf_log_buffer\n");
		return -1;
	}

	err = misc_register(&atf_log_dev);
	if (unlikely(err)) {
		pr_no_info("atf_log: failed to register device");
		return -1;
	}

	err = misc_register(&atf_raw_buf_dev);
	if (unlikely(err)) {
		pr_no_info("atf_raw_buf: failed to register device");
		return -1;
	}
	...
	if (atf_log_proc_dir == NULL) {
		pr_no_info("atf_log proc_mkdir failed\n");
		return -ENOMEM;
	}
	...
	if (atf_log_proc_file == NULL) {
		pr_no_info("atf_log proc_create failed at atf_log\n");
		return -ENOMEM;
	}
	...
	if (atf_raw_buf_proc_file == NULL) {
		pr_no_info("atf_raw_buf proc_create failed at atf_log\n");
		return -ENOMEM;
	}
...
static int __init atf_log_init(void)
{
	...
	if (ret)
		pr_no_info("atf logger init FAIL, ret 0x%x\n", ret);
	return ret;
}
