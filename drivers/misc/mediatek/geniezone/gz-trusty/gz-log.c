...
{
	if (!rmem) {
		pr_no_info("[%s] ERROR: invalid reserved memory\n", __func__);
		return -EFAULT;
	}
	glctx.paddr = rmem->base;
	glctx.size = rmem->size;
	glctx.flag = STATIC;
	pr_no_info("[%s] rmem:%s base(0x%llx) size(0x%zx)\n",
...
static int gz_log_page_init(void)
{
	...
	if (glctx.flag == STATIC) {
		...
		if (!glctx.virt) {
			pr_no_info("[%s] ERROR: ioremap failed, use dynamic\n",
				__func__);
			glctx.flag = DYNAMIC;
			goto dynamic_alloc;
		}

		pr_no_info("[%s] set by static, virt addr:%p, sz:0x%zx\n",
...
dynamic_alloc:
		...
		pr_no_info("[%s] set by dynamic, virt:%p, sz:0x%zx\n",
			__func__, glctx.virt, glctx.size);
	}
		       ...
		       unsigned long *size, unsigned long *start)
{
	...
	if (!glctx.virt) {
		*addr = *paddr = *size = *start = 0;
		pr_no_info("[%s] ERR gz_log init failed\n", __func__);
		return;
	}
	*addr = (unsigned long)glctx.virt;
	*paddr = (unsigned long)glctx.paddr;
	pr_no_info("[%s] virtual address:0x%lx, paddr:0x%lx\n",
				   ...
				   unsigned long action, void *data)
{
	 ...
	 * though this is racy.
	 */
	pr_no_info("trusty-log panic notifier - trusty version %s",
...
static bool trusty_supports_logging(struct device *device)
{
	...
	if (ret == SM_ERR_UNDEFINED_SMC) {
		pr_no_info("trusty-log not supported on secure side.\n");
		return false;
	} else if (ret < 0) {
		pr_no_info("trusty std call (GZ_SHARED_LOG_VERSION) failed: %d\n",
		       ret);
		return false;
	}

	if (ret == TRUSTY_LOG_API_VERSION) {
		pr_no_info("trusty-log API supported: %d\n", ret);
		return true;
	}

	pr_no_info("trusty-log unsupported api version: %d, supported: %d\n",
			  ...
			  char __user *buf, size_t size)
{
	...
	if (alloc - get > log->sz) {
		pr_no_notice("trusty: log overflow, lose some msg.");
		get = alloc - log->sz;
	}
	...
	while (get != put) {
		...
		ret = copy_to_user(buf + copy_chars, gls->line_buffer,
				   read_chars);
		if (ret) {
			pr_no_notice("[%s] copy_to_user failed ret %d\n",
				  __func__, ret);
			break;
		}
		get += read_chars;
...
static int trusty_gz_log_remove(struct platform_device *pdev)
{
	...
	ret = trusty_std_call32(gls->trusty_dev,
			...
			(u32)glctx.paddr, (u32)((u64)glctx.paddr >> 32), 0);
	if (ret)
		pr_no_info("std call(GZ_SHARED_LOG_RM) failed: %d\n", ret);
