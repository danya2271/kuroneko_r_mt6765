...
void switch_irtx_gpio(int mode)
{
	...
	if (mode >= (ARRAY_SIZE(irtx_gpio_cfg))) {
		pr_no_notice("%s() [PinC](%d) fail!! - invalid parameter!\n",
			__func__, mode);
		return;
	}

	if (IS_ERR(ppinctrl_irtx)) {
		pr_no_notice("%s() [PinC] ppinctrl_irtx:%p Error! err:%ld\n",
	...
	if (mt_irtx_dev.buck != NULL) {
		if (mode == IRTX_GPIO_MODE_LED_SET) {
			if (!regulator_is_enabled(mt_irtx_dev.buck)
				...
				pr_no_notice("%s() regulator_enable fail!\n",
					__func__);
				return;
			}
		} else {
			if (regulator_is_enabled(mt_irtx_dev.buck)
				...
				pr_no_notice("%s() regulator_disable fail!\n",
					__func__);
				return;
			}
		}
...
static int dev_char_open(struct inode *inode, struct file *file)
{
	...
	ret = get_ir_device();
	if (ret) {
		pr_no_notice("%s() IRTX device busy.\n", __func__);
		goto exit;
	}

	pr_no_info("%s() IRTX open by %s\n", __func__, current->comm);
...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	ret = put_ir_device();
	if (ret) {
		pr_no_notice("%s() IRTX device close without open\n", __func__);
		goto exit;
	}

	pr_no_info("%s() IRTX close by %s\n", __func__, current->comm);
exit:
	return ret;
}
	...
	size_t count, loff_t *ppos)
{
	...
	pr_no_info("%s() irtx write len=0x%x, pwm=%d\n", __func__,
	...
	if (!wave_vir) {
		pr_no_notice("%s() IRTX alloc memory fail\n", __func__);
		ret = -ENOMEM;
		goto exit_1;
	}
	...
	if (ret) {
		pr_no_notice("%s() IRTX copy from user fail %d\n", __func__, ret);
		goto exit_2;
	}

	/* invert bit */
	if (mt_irtx_dev.pwm_data_invert) {
		pr_no_notice("%s() IRTX invert data\n", __func__);
		for (i = 0; i < count; i++) {
	...
	pr_no_info("irtx ret:%d, period:%d, count:%zu, total:%d, duty:%d/%d\n",
		ret, h_l_period, count, total_time,
		pwm_ir.duty_cycle, pwm_ir.cycle);
#ifdef IRTX_DEBUG
	pr_no_info("%s() irtx pwm buf size = %d\n", __func__, buf_size);
	for (i = 0; i < buf_size; i++) {
		if (i && i % 16 == 0)
			pr_no_info("\n");
		pr_no_info("[%d]0x%x, ", i, buf_ptr[i]);
	}
	pr_no_info("\n");
#endif
	if (total_time <= 0) {
		...
		unsigned long arg)
{
	...
	case IRTX_IOC_SET_IRTX_LED_EN:
		ret = copy_from_user(&para, (void __user *)arg,
			sizeof(unsigned int));
		if (ret) {
			pr_no_notice("%s() IRTX SET LED EN,copy_from_user fail\n",
				__func__);
			ret = -EFAULT;
		} else {
			/* en: bit 12; */
			...
			en = (para & 0xF);
			pr_no_info("%s IRTX SET LED EN:0x%x,gpioid:%ul,en:%ul\n",
	...
	case IRTX_IOC_SET_CARRIER_FREQ:
		ret = copy_from_user(&para, (void __user *)arg,
			sizeof(unsigned int));
		if (ret) {
			pr_no_notice("IRTX_IOC_SET_CARRIER_FREQ fail\n");
			ret = -EFAULT;
		} else {
			pwm_ir.carrier = para;
			pr_no_info("irtx carrier freq = %d\n", pwm_ir.carrier);
		}
		break;
	case IRTX_IOC_SET_DUTY_CYCLE:
		ret = copy_from_user(&para, (void __user *)arg,
			sizeof(unsigned int));
		if (ret) {
			pr_no_notice("IRTX_IOC_SET_DUTY_CYCLE fail\n");
			ret = -EFAULT;
		} else {
			pwm_ir.cycle = para & 0xFFFF;
			pwm_ir.duty_cycle = (para >> 16) & 0xFFFF;
			pr_no_info("irtx duty cycle = %d/%d\n",
				pwm_ir.duty_cycle, pwm_ir.cycle);
		}
		break;
	default:
		pr_no_notice("%s() IRTX invalid ioctl cmd 0x%x\n", __func__, cmd);
		ret = -ENOTTY;
		break;
	}
	return ret;
		...
		unsigned long arg)
{
	if (!file->f_op || !file->f_op->unlocked_ioctl) {
		pr_no_info("irtx file has no f_op or no unlocked_ioctl.\n");
		return -ENOTTY;
	}
	...
	case COMPAT_IRTX_IOC_SET_CARRIER_FREQ:
		...
		break;
	default:
		pr_no_info("irtx compat_ioctl : No such command!! 0x%x\n", cmd);
		return -ENOIOCTLCMD;
	}
}
#endif
...
static int irtx_probe(struct platform_device *plat_dev)
{
	...
	if (plat_dev->dev.of_node == NULL) {
		pr_no_notice("%s() irtx OF node is NULL!\n", __func__);
		ret = -1;
		goto exit;
	}
	...
	pr_no_info("%s() device tree info: major=%d pwm=%d invert=%d\n", __func__,
	...
	if (IS_ERR(mt_irtx_dev.ppinctrl_irtx)) {
		pr_no_notice("%s() [PinC]cannot find pinctrl! ptr_err:%ld.\n",
	...
	if (IS_ERR(mt_irtx_dev.buck)) {
		...
		pr_no_notice("%s() irtx_ldo regulator not found\n", __func__);
	} else {
		ret = regulator_set_voltage(mt_irtx_dev.buck, 2800000, 2800000);
		if (ret < 0) {
			pr_no_notice("%s() regulator_set_voltage fail! ret:%d.\n",
				__func__, ret);
			goto exit;
		}
	}
	...
	if (!major) {
		...
		if (ret) {
			pr_no_notice("%s() alloc_chrdev_region fail! ret=%d.\n",
				__func__, ret);
			goto exit;
		} else {
		...
		if (ret) {
			pr_no_notice("%s() register_chrdev_region fail! ret=%d.\n",
				__func__, ret);
			goto exit;
		}
	}
	...
	if (ret) {
		pr_no_notice("%s() cdev_add fail! ret=%d\n", __func__, ret);
		goto exit;
	}
	...
	if (IS_ERR(dev)) {
		ret = PTR_ERR(dev);
		pr_no_notice("%s() device_create fail! ret=%d\n", __func__, ret);
		goto exit;
	}
	pr_no_info("%s() Done.\n", __func__);
	return 0;
...
static int __init irtx_init(void)
{
	...
	if (ret) {
		pr_no_notice("%s() platform driver register fail ret:%d.\n",
			__func__, ret);
		goto exit;
	}
