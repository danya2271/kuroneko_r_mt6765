...
void scp_A_ipi_handler(void)
{
	...
	/*pr_no_debug("scp A ipi handler %d\n", scp_id);*/
	if (scp_id >= SCP_NR_IPI || scp_id <= 0) {
		/* ipi id abnormal*/
		pr_no_debug("[SCP] A ipi handler id abnormal, id=%d\n", scp_id);
		...
		pr_no_debug("[SCP] A ipi handler is null or abnormal, id=%d\n"
								, scp_id);
	}
	...
	/*pr_no_debug("scp_ipi_handler done\n");*/
}
...
void scp_A_ipi_init(void)
{
	...
	pr_no_debug("[SCP] scp_rcv_obj[A] = 0x%p\n", scp_rcv_obj[SCP_A_ID]);
	pr_no_debug("[SCP] scp_send_obj[A] = 0x%p\n", scp_send_obj[SCP_A_ID]);
	...
	unsigned int  len, unsigned int wait, enum scp_core_id scp_id)
{
	...
	if (scp_id >= SCP_CORE_TOTAL) {
		pr_no_err("[SCP] %s: scp_id:%d wrong\n", __func__, scp_id);
	...
	if (in_interrupt()) {
		if (wait) {
			pr_no_err("[SCP] %s: cannot use in isr\n", __func__);
			scp_ipi_desc[id].error_count++;
			return SCP_IPI_ERROR;
		}
	}
	...
	if (id >= SCP_NR_IPI) {
		pr_no_err("[SCP] %s: ipi id %d wrong\n", __func__, id);
		return SCP_IPI_ERROR;
	}
	if (is_scp_ready(scp_id) == 0) {
		/* pr_no_err("[SCP] %s: %s not enabled, id=%d\n", __func__
		 *					, core_ids[scp_id], id);
		 */
		pr_no_notice("[SCP] %s: %s not ready\n", __func__,
			core_ids[scp_id]);
		scp_ipi_desc[id].error_count++;
		return SCP_IPI_ERROR;
	}
	if (len > sizeof(scp_send_obj[scp_id]->share_buf) || buf == NULL) {
		pr_no_err("[SCP] %s: %s buffer error\n", __func__,
			core_ids[scp_id]);
		scp_ipi_desc[id].error_count++;
		return SCP_IPI_ERROR;
	}
	...
	if (atomic_read(&scp_reset_status) == RESET_STATUS_START) {
		pr_no_notice("[SCP] %s: %s reset start\n", __func__,
			core_ids[scp_id]);
		scp_ipi_desc[id].error_count++;
		return SCP_IPI_ERROR;
	}
	...
	if (mutex_trylock(&scp_ipi_mutex[scp_id]) == 0) {
		...
		if ((scp_ipi_id_record_count % PRINT_THRESHOLD == 0) ||
			...
			pr_no_err("[SCP] %s:%s %d mutex busy, owner=%d\n",
	...
	if (scp_awake_lock(scp_id) == -1) {
		...
		pr_no_err("[SCP] %s: %s ipi error, awake scp fail\n", __func__,
			core_ids[scp_id]);
		scp_ipi_desc[id].error_count++;
		return SCP_IPI_ERROR;
	}
	...
	if ((readl(SCP_GIPC_IN_REG) & (1<<scp_id)) > 0) {
		...
		if ((scp_ipi_id_record_count % PRINT_THRESHOLD == 0) ||
			(scp_ipi_id_record_count % PRINT_THRESHOLD == 1)) {
			pr_no_err("[SCP] %s:%s %d ap->scp busy,last time=%d\n",
				__func__, core_ids[scp_id], id,
				scp_ipi_owner[scp_id]);
			scp_A_dump_regs();
		}
		if (scp_awake_unlock(scp_id) == -1)
			pr_no_debug("[SCP] %s: ap->scp busy awake unlock -1\n",
				__func__);
		scp_ipi_desc[id].busy_count++;
		mutex_unlock(&scp_ipi_mutex[scp_id]);
		return SCP_IPI_BUSY;
	...
	if (rb_id != id) {
		pr_no_debug("[SCP]ERR: write/read id failed, %d, %d\n", id, rb_id);
		WARN_ON(1);
	}
...
void scp_ipi_info_dump(enum ipi_id id)
{
	...
	for (i = 0; i < SCP_IPI_ID_STAMP_SIZE; i++) {
		if (scp_ipi_desc[id].recv_timestamp[i] != 0) {
			pr_no_debug("[SCP]scp->ap recv count:%u, ap recv:%llu, handler fin:%llu\n",
	...
	for (i = 0; i < SCP_IPI_ID_STAMP_SIZE; i++) {
		if (scp_ipi_desc[id].send_timestamp[i] != 0) {
			pr_no_debug("ap->scp send count:%u send time:%llu\n",
...
void scp_ipi_status_dump_id(enum ipi_id id)
{
	...
	pr_no_debug("[SCP]id\trecv\tsuccess\tbusy\terror\tname\n\r");
...
void scp_ipi_status_dump(void)
{
	...
	pr_no_debug("ap->scp total=%u scp->ap total=%u\n\r",
			ap_to_scp_ipi_count,
			scp_to_ap_ipi_count);
}
...
void mt_print_scp_ipi_id(void)
{
	...
	switch (id) {
	case IPI_SENSOR:
		pr_no_info("[SCP] ipi(%d) id/type/action/event/reserve = %d/%d/%d/%d/%d\n",
				*ipi_count, id, buf[0], buf[1], buf[2], buf[3]);
		break;
	default:
		pr_no_info("[SCP] ipi id = %d\n", id);
		break;
	}
}
