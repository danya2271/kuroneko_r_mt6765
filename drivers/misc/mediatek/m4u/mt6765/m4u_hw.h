...
{
	if (likely((unsigned int)portID < (unsigned int)gM4u_port_num))
...
{
	int i;
	if (m4u_id == 0) tf_id &= F_MMU0_INT_ID_TF_MSK;
...
{
	return likely((unsigned int)port < (unsigned int)gM4u_port_num) ? gM4uPort[port].larb_port : 0;
}

static inline int m4u_port_2_larb_id(M4U_PORT_ID port)
{
	return likely((unsigned int)port < (unsigned int)gM4u_port_num) ? gM4uPort[port].larb_id : 0xff;
}
...
{
	return likely((unsigned int)port < (unsigned int)gM4u_port_num) ? gM4uPort[port].m4u_id : 0;
}

static inline int m4u_port_2_m4u_slave_id(M4U_PORT_ID port)
{
	return likely((unsigned int)port < (unsigned int)gM4u_port_num) ? gM4uPort[port].m4u_slave : 0;
}
