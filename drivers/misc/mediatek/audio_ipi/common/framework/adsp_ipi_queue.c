...
#define scp_debug(x...) pr_no_info(x)
#else
#define scp_debug(x...)
#endif
...
#define DUMP_IPC_MSG(description, p_scp_msg) \
	do { \
		...
		if (p_scp_msg->len >= IPI_MSG_HEADER_SIZE) { \
			...
			pr_no_info("%s, ipi_id: %u, buf %p, len: %u", \
				description, \
				p_scp_msg->ipi_id, \
				p_scp_msg->buf, \
				p_scp_msg->len); \
...
int scp_ipi_queue_init(uint32_t dsp_id)
{
	...
	if (dsp_id >= NUM_OPENDSP_TYPE) {
		pr_no_info("dsp_id: %u error!!", dsp_id);
		return -EFAULT;
	}
	...
	uint32_t wait_ms)
{
	...
	if (buf == NULL || len > SCP_MSG_BUFFER_SIZE) {
		pr_no_info("buf: %p, len: %u!! return", buf, len);
		return -EFAULT;
	}
	if (is_audio_dsp_ready(dsp_id) == false) {
		pr_no_info("dsp_id: %u not ready!! ipi_id: %u",
			dsp_id, ipi_id);
		return 0;
	}
	...
	if (in_interrupt() && wait_ms != 0) {
		pr_no_info("in_interrupt()!! wait_ms %u => 0!!", wait_ms);
		wait_ms = 0;
	}
	...
	spin_unlock_irqrestore(&msg_queue->queue_lock, flags);
	if (retval != 0) {
		pr_no_info("dsp_id: %u, push fail!!", dsp_id);
		return retval;
	}
	...
	for (try_cnt = 0; try_cnt < k_max_try_cnt; try_cnt++) {
		...
		if (retval > 0) {
			pr_no_notice("wait ret %d, sig %d, enable %d",
				  retval,
		...
		if (retval == 0) { /* timeout */
			pr_no_info("wait %u ms timeout, will still send to scp later!!",
				wait_ms);
			break;
		}
	...
	void (*ipi_handler)(int ipi_id, void *buf, unsigned int len))
{
	...
	if (ipi_handler == NULL) {
		pr_no_info("NULL!! ipi_handler: %p", ipi_handler);
		return -EFAULT;
	}
	...
	if (stop_time > 1000) { /* 1 ms */
		pr_no_notice("IPI Q push %lld us too long!! push %lld wake %lld, ktime api %lld",
			  stop_time,
			  push_time,
			  wake_time - push_time,
			  stop_time - wake_time);
...
static void scp_dump_msg_in_queue(struct scp_msg_queue_t *msg_queue)
{
	...
	pr_no_info("dsp_id: %u, idx_r: %u, idx_w: %u, queue(%u/%u)",
		msg_queue->dsp_id,
		msg_queue->idx_r,
		msg_queue->idx_w,
	...
	while (idx_dump != msg_queue->idx_w) {
		...
		if (n < 0 || n > sizeof(dump_str))
			pr_no_info("error to get string dump_str\n");
	...
	uint32_t *p_queue_counter)
{
	...
	if (msg_queue == NULL || buf == NULL ||
		...
		pr_no_info("NULL!! msg_queue: %p, buf: %p, p_idx_msg: %p, p_queue_counter: %p",
...
static int scp_pop_msg(struct scp_msg_queue_t *msg_queue)
{
	...
	if (scp_check_queue_empty(msg_queue) == true) {
		pr_no_info("dsp_id: %u, queue is empty, idx_r: %u, idx_w: %u",
			msg_queue->dsp_id,
			msg_queue->idx_r,
			msg_queue->idx_w);
		return -1;
	...
	uint32_t *p_idx_msg)
{
	if (msg_queue == NULL || pp_scp_msg == NULL || p_idx_msg == NULL) {
		pr_no_info("NULL!! msg_queue: %p, pp_scp_msg: %p, p_idx_msg: %p",
	...
	*p_idx_msg = 0xFFFFFFFF;
	...
	/* check queue empty */
	if (scp_check_queue_empty(msg_queue) == true) {
		pr_no_info("dsp_id: %u, queue empty, idx_r: %u, idx_w: %u",
			msg_queue->dsp_id,
	...
	if (is_empty == true) {
		for (try_cnt = 0; try_cnt < k_max_try_cnt; try_cnt++) {
			...
			if (retval == 0) { /* got msg in queue */
				pr_no_notice("wait ret 0, empty %d, enable %d",
					  scp_check_queue_empty(msg_queue),
					  msg_queue->enable);
				break;
			}
	...
	const uint32_t scp_path)
{
	...
	if (dsp_id >= NUM_OPENDSP_TYPE || scp_path >= SCP_NUM_PATH) {
		pr_no_info("dsp_id: %u, scp_path: %u error!!",
			dsp_id, scp_path);
		return -EFAULT;
	}
	...
	/* check double init */
	if (msg_queue->init) {
		pr_no_info("dsp_id: %u already init!!", dsp_id);
		return 0;
	}
	msg_queue->init = true;
	...
	if (scp_path == SCP_PATH_A2S) {
		...
		if (n < 0 || n > sizeof(thread_name))
			pr_no_info("error to get string thread_name\n");
	...
	if (IS_ERR(msg_queue->scp_thread_task)) {
		pr_no_info("can not create %s kthread", thread_name);
		WARN_ON(1);
...
static int scp_process_msg_thread(void *data)
{
	...
	while (msg_queue->thread_enable && !kthread_should_stop()) {
		...
		if (retval != 0) {
			pr_no_info("scp_get_queue_element retval %d", retval);
			continue;
		}
		if (idx_msg >= MAX_SCP_MSG_NUM_IN_QUEUE) {
			pr_no_info("scp_get_queue_element idx_msg %u > %u",
	...
	pr_no_info("thread exit, dsp_id %u, scp_path %u, idx_r %d, idx_w %d",
	...
	struct scp_msg_t *p_scp_msg)
{
	...
	if (msg_queue == NULL || p_scp_msg == NULL) {
		pr_no_info("NULL!! msg_queue: %p, p_scp_msg: %p",
			msg_queue, p_scp_msg);
		return -EFAULT;
	}
	...
	case AUDIO_OPENDSP_USE_CM4_B:
		...
		if ((enum scp_core_id)dsp_id >= SCP_CORE_TOTAL) {
			pr_no_notice("dsp_id %u/%u not support!!",
		...
		retval = 0;
#else
		pr_no_notice("dsp_id %u not build!!", dsp_id);
		retval = -ENODEV;
#endif
		break;
	...
	case AUDIO_OPENDSP_USE_HIFI3_B:
		...
		break;
	default:
		pr_no_notice("dsp_id %u not support!!", dsp_id);
		retval = -ENODEV;
	}
	if (retval != 0)
		return retval;
	...
	struct scp_msg_t *p_scp_msg)
{
	...
	if (p_scp_msg->ipi_handler == NULL) {
		pr_no_info("NULL!! p_scp_msg->ipi_handler: %p",
			p_scp_msg->ipi_handler);
		return -EFAULT;
	}

	if (p_scp_msg->len == 0) {
		pr_no_info("p_scp_msg->len: %u", p_scp_msg->len);
		return -EFAULT;
	}
