...
{
	if (!g_dma[dsp_id]) {
		pr_no_notice("g_dma is NULL!!!, return.");
		return NULL;
	}
...
int init_audio_ipi_dma(const uint32_t dsp_id)
{
	...
	if (g_dma[dsp_id] != NULL) {
		pr_no_info("dsp_id(%u) already init. return", dsp_id);
		ret = -ENOMEM;
	...
	if (ret != 0 || mem_id == 0xFFFFFFFF || size == 0) {
		pr_no_info("dsp_id(%u), mem_id(%u), size(%u) ret %d error!!",
			dsp_id, mem_id, size, ret);
		goto IPI_DMA_INIT_EXIT;
	}

	pr_no_info("dsp_id(%u), mem_id(%u), size(%u)", dsp_id, mem_id, size);

	/* share mem for IPI DMA */
	dma = (struct audio_ipi_dma_t *)get_reserve_mem_virt(dsp_id, mem_id);
	if (dma == NULL) {
		pr_no_notice("scp_get_reserve_mem_virt(%d) fail!!", mem_id);
		ret = -ENOMEM;
		goto IPI_DMA_INIT_EXIT;
	}
	...
	if (dma->base_phy.addr_val == 0) {
		pr_no_notice("get_reserve_mem_phys(%d) fail!!", mem_id);
		ret = -ENOMEM;
		goto IPI_DMA_INIT_EXIT;
	}
	...
	if (dma->size < dma->pool_offset) {
		pr_no_notice("size %u < pool_offset %u",
	...
	/* pool */
	pr_no_info("+gen_pool_create, g_cache_alilgn_order[%d] = %d",
		dsp_id, g_cache_alilgn_order[dsp_id]);
	g_dma_pool[dsp_id] =
		gen_pool_create(g_cache_alilgn_order[dsp_id], -1);
	if (g_dma_pool[dsp_id] == NULL) {
		pr_no_notice("gen_pool_create fail");
		ret = -ENOMEM;
		goto IPI_DMA_INIT_EXIT;
	}
	pr_no_info("-gen_pool_create, g_dma_pool[%u] = %p",
	...
	if (gen_pool_add_virt(
		    ...
		    -1) != 0) {
		pr_no_notice("dsp_id(%u) gen_pool_add fail", dsp_id);
		ret = -ENOMEM;
...
int audio_ipi_dma_init_dsp(const uint32_t dsp_id)
{
	...
	if (!g_dma[dsp_id]) {
		pr_no_info("g_dma[%u] is NULL!!! return -EFAULT", dsp_id);
		return -EFAULT;
	}
...
#endif
	pr_no_info("dsp_id: %u, task: %d, ret: %d", dsp_id, task, ret);
	return ret;
}
				 ...
				 phys_addr_t phy_addr_val)
{
	...
	if (g_dma[dsp_id] == NULL) {
		pr_no_notice("g_dma[%u] is NULL!!!", dsp_id);
		return NULL;
	}
	if (phy_addr_val == 0)
		return NULL;
	...
	const uint32_t size)
{
	...
	if (g_dma[dsp_id] == NULL) {
		pr_no_info("g_dma[%u] is NULL!!!", dsp_id);
		return -ENODEV;
	}
	if (g_dma_pool[dsp_id] == NULL) {
		pr_no_info("dma_pool[%u] is NULL!!!", dsp_id);
		return -ENOMEM;
	}
	if (phy_addr == NULL || virt_addr == NULL || size == 0) {
		pr_no_info("arg err, %p, %p, %u", phy_addr, virt_addr, size);
		return -EINVAL;
	}
	...
	if (new_addr == 0) {
		pr_no_notice("task %d, gen_pool_alloc(0x%x) fail, (%zu/%zu)",
			  task,
			  size,
	...
	*phy_addr = gen_pool_virt_to_phys(g_dma_pool[dsp_id], new_addr);
	if (*phy_addr == 0) {
		pr_no_notice("task %d, gen_pool_virt_to_phys() fail, g_dma_pool[%u] %p, new_addr %zu",
			  task,
			  dsp_id,
			  g_dma_pool[dsp_id],
			  new_addr);
	...
	*virt_addr = get_audio_ipi_dma_vir_addr(
	...
	pr_no_info("task %d, size 0x%x, (%zu/%zu)",
		task,
		size,
		gen_pool_avail(g_dma_pool[dsp_id]),
		gen_pool_size(g_dma_pool[dsp_id]));
		       ...
		       phys_addr_t phy_addr, const uint32_t size)
{
	...
	if (phy_addr == 0 || size == 0) {
		pr_no_info("arg err, %llu, %u", phy_addr, size);
		return -EINVAL;
	}
			       ...
			       const uint32_t dsp_to_ap_size)
{
	...
	if (dsp_id >= NUM_OPENDSP_TYPE) {
		pr_no_info("dsp_id(%u) invalid. return", dsp_id);
		return -ENODEV;
	}
	if (dsp_id == AUDIO_OPENDSP_USE_CM4_B) {
		pr_no_info("ipi dma is not supported in dsp_id(%u), task = %d",
			dsp_id, task);
		return -ENODEV;
	}

	if (g_dma[dsp_id] == NULL) {
		pr_no_info("g_dma[%u] is NULL!!!task = %d", dsp_id, task);
		return -ENODEV;
	}
	if (g_dma_pool[dsp_id] == NULL) {
		pr_no_info("dma_pool[%u] is NULL!!!task = %d", dsp_id, task);
		return -ENOMEM;
	}
	if (task >= TASK_SCENE_SIZE) {
		pr_no_info("task: %d", task);
		return -EOVERFLOW;
	}
	task_ctrl = get_audio_controller_task(dsp_id);
	if (task_ctrl == TASK_SCENE_INVALID) {
		pr_no_info("ipi dma is not supported in dsp_id(%u), ctrl = %d",
			dsp_id, task_ctrl);
		return -ENODEV;
	}
	...
	if (g_region_reg_flag[task] == true) {
		pr_no_notice("task: %d already register", task);
		return -EEXIST;
	}
	...
	if (ret == 0) {
		pr_no_info("task %d, a2d sz 0x%x, offset 0x%x, d2a sz 0x%x, offset 0x%x, (%zu/%zu)",
			task,
...
int audio_ipi_dma_free_region(const uint8_t task)
{
	...
	if (g_region_reg_flag[task] == false) {
		pr_no_info("task: %d already unregister", task);
		return -ENODEV;
	}
		...
		uint32_t count)
{
	...
	if (!region) {
		pr_no_info("region is NULL!!! return -EFAULT");
		return -EFAULT;
	}
	if (!linear_buf) {
		pr_no_info("linear_buf is NULL!!! return -EFAULT");
		return -EFAULT;
	}
	if (!dma_vir_base(dsp_id)) {
		pr_no_info("dma_vir_base(%d) is NULL!!! return -EFAULT",
			dsp_id);
		return -EFAULT;
	}
	...
	if (count_align <= w2e) {
		...
		pr_no_info("count(%u), dump size(%u), buf(%p)(%s)"
			       ...
			       uint32_t *write_idx)
{
	...
	if (!data_buf || !write_idx) {
		pr_no_info("buf: %p or idx: %p is NULL!!!", data_buf, write_idx);
		return -EFAULT;
	}
	if (data_size == 0) {
		pr_no_info("task: %d, data_size = 0", task);
		return -ENODATA;
	}
			      ...
			      uint32_t read_idx)
{
	...
	if (!data_buf) {
		pr_no_info("buf: %p is NULL!!!", data_buf);
		return -EFAULT;
	}
	if (data_size == 0) {
		pr_no_info("task: %d, data_size = 0", task);
...

{
	...
	if (drop_size == 0) {
		pr_no_info("task: %d, drop_size = 0", task);
		return -ENODATA;
	}
...
static void hal_dma_dump_msg_in_queue(struct hal_dma_queue_t *msg_queue)
{
	...
	pr_no_info("idx_r: %u, idx_w: %u, queue(%u/%u)",
		msg_queue->idx_r,
		msg_queue->idx_w,
	...
	uint32_t *p_idx_msg)
{
	...
	if (msg_queue == NULL || p_ipi_msg == NULL || p_idx_msg == NULL) {
		pr_no_info("NULL!! msg_queue: %p, p_ipi_msg: %p, p_idx_msg: %p",
	...
	if (hal_dma_check_queue_to_be_full(msg_queue) == true) {
		pr_no_info("task: %d, msg_id: 0x%x, queue overflow, idx_r: %u, idx_w: %u, drop it",
	...
	if (hal_dma_check_idx_msg_valid(msg_queue, msg_queue->idx_w) == false) {
		pr_no_info("idx_w %u is invalid!! return", msg_queue->idx_w);
		return -1;
	}

	/* get dma data */
	if (p_ipi_msg->dma_info.data_size > MAX_DSP_DMA_WRITE_SIZE) {
		pr_no_notice("data_size %u > %u!!",
	...
	for (i = 0; i < p_ipi_msg->dma_info.data_size; i++)
		pr_no_info("%d", msg_queue->tmp_buf_d2k[i]);
#endif
...
static int hal_dma_pop(struct hal_dma_queue_t *msg_queue)
{
	...
	if (msg_queue == NULL) {
		pr_no_info("NULL!! msg_queue: %p", msg_queue);
		return -EFAULT;
	}

	/* check queue empty */
	if (hal_dma_check_queue_empty(msg_queue) == true) {
		pr_no_info("queue is empty, idx_r: %u, idx_w: %u",
			msg_queue->idx_r,
			msg_queue->idx_w);
		return -1;
	}
	...
	uint32_t *p_idx_msg)
{
	...
	if (msg_queue == NULL || pp_ipi_msg == NULL || p_idx_msg == NULL) {
		pr_no_info("NULL!! msg_queue: %p, pp_ipi_msg: %p, p_idx_msg: %p",
	...
	*p_idx_msg = 0xFFFFFFFF;
	...
	/* check queue empty */
	if (hal_dma_check_queue_empty(msg_queue) == true) {
		pr_no_info("queue empty, idx_r: %u, idx_w: %u",
			msg_queue->idx_r, msg_queue->idx_w);
		return -ENOMEM;
	}
	...
	if (hal_dma_check_idx_msg_valid(msg_queue, msg_queue->idx_r) == false) {
		pr_no_info("idx_r %u is invalid!! return",
			msg_queue->idx_r);
		return -1;
	}
...
int audio_ipi_dma_msg_to_hal(struct ipi_msg_t *p_ipi_msg)
{
	...
	if (p_ipi_msg == NULL || msg_queue == NULL) {
		pr_no_info("p_ipi_msg(%p) or msg_queue(%p) is NULL!! return",
			p_ipi_msg, msg_queue);
		return -EFAULT;
	}
...
size_t audio_ipi_dma_msg_read(void __user *buf, size_t count)
{
	...
	if (buf == NULL || count == 0 || msg_queue == NULL) {
		pr_no_info("arg!! %p %zu %p, return", buf, count, msg_queue);
		msleep(500);
		return 0;
	}
	...
	if (retval != 0) {
		pr_no_info("hal_dma_get_queue_msg retval %d", retval);
		msleep(100);
		return 0;
	}
